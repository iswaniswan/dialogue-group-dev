    ...
    <div class="col-lg-12">
        <div class="panel panel-info">
            <div class="panel-heading">
                <i class="fa fa-pencil mr-2"></i> <?= $title; ?> <a href="#" onclick="show('<?= $folder; ?>/cform/index/<?= $dfrom . "/" . $dto; ?>','#main'); return false;" class="btn btn-info btn-sm mr-2 pull-right"><i class="fa fa-list mr-2"></i></i> <?= $title_list; ?></a>
            </div>
...
<script>
    ...
    $("#addrow").on("click", function() {
        ...
        cols += `<td><input type="text" id="e_satuan${i}" class="form-control input-sm" autocomplete="off" name="e_satuan${i}" readonly></td>`;
        cols += `<td class="text-center"><input type="text" id="nquantity${i}" class="form-control text-right input-sm inputitem" autocomplete="off" name="nquantity${i}" onblur=\'if(this.value==""){this.value="0";}\' onfocus=\'if(this.value=="0"){this.value="";}\' value="0" onkeyup="angkahungkul(this); sumi();"></td>`;
        cols += `<td><input type="text" class="form-control input-sm" name="eremark${i}" id="eremark${i}" placeholder="Isi keterangan jika ada!"/></td>`;
        cols += `<td><button type="button" title="Delete" class="ibtnDel btn btn-circle btn-danger"><i class="ti-close"></i></button></td></tr>`;
        newRow.append(cols);
    ...
    function sumi() {
        var sumiqty = 0;
        $("#tabledatay tbody tr .inputitem").each(function(index) {
            sumiqty += parseFloat($(this).val()); //console.log(index + ": " + $(this).val());
        });
        $("#total").html(sumiqty);
    }
</script>