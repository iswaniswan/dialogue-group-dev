        ...
        <div class="col-sm-12">
            <div class="table-responsive">
                <table id="tabledatay" class="table color-table inverse-table table-bordered class" cellpadding="8" cellspacing="1" width="100%">
                    ...
                    <tbody>
                        <?php $jmlbayar = 0; foreach ($datadetail as $key) { 
                            $jmlbayar += $key->v_jumlah;
                            $i++; ?>
                            <tr>
                                <td class="text-center"><spanx id="snum<?=$i;?>"><?=$i;?></spanx></td>
                                <td>
                                    <input type="hidden" id="idnota<?=$i;?>" value="<?= $key->id_nota;?>"/>
                                    <input type="hidden" id="inota<?=$i;?>" value="<?= $key->i_nota;?>"/>
                                ...
                                <td><input type="text" readonly class="form-control input-sm text-right" id="vnilai<?=$i;?>" value="<?= number_format($key->v_sisa_nota);?>"/></td>
                                <td><input type="text" readonly class="form-control text-right input-sm" id="vbayar<?=$i;?>" value="<?= number_format($key->v_jumlah);?>"></td>
                                <td><input type="text" readonly class="form-control input-sm text-right" id="vsisa<?=$i;?>" value="0"/></td>
                    ...
                    <tfoot>
                        <tr class="bold">
                            ...
                            <td>
                                ...
                                <input type="hidden" id="vjumlahbayar" class="form-control input-sm text-right" value="<?= number_format($jmlbayar);?>" readonly>
...
<script>
    ...
    function approve() {        
        ...
        if (parseInt(formatulang($('#vjumlahbayar').val())) > parseInt(formatulang($('#vjumlah').val()))) {
            swal('Maaf :(','Total Bayar = '+$('#vjumlahbayar').val()+' Tidak Boleh lebih dari Jumlah KN = '+$('#vjumlah').val()+' !','error');
            return false
        } else {
            for (var i = 1; i <= $('#jml').val(); i++) {
