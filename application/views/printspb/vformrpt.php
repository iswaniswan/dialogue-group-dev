<?php 
  ...
  foreach($isi as $row){
    ...
    $ab = str_repeat(" ",9);
    $ipp  = new PrintIPP();
    $ipp->setHost($host); #IP Address
    $ipp->setPrinterURI($uri);#Printer Address
    $ipp->setRawText();
    $ipp->unsetFormFeed();
    ...
    $cetak.=CHR(18)."\n";   
    $cetak.=$nor.NmPerusahaan."                                     ".CHR(27).CHR(71).$prioritas.CHR(27).CHR(72)."\n\n";    
    ...
    $pjg=strlen("(".$row->i_customer.")".$row->e_customer_name)+floatval($nor);
    $jrk=42-$pjg;
...
#   $cetak.=$ab."Saldo Piutang                     : RP ".number_format($row->v_saldo)."\n";
    ...
    $cetak.=$ab."Jumlah SPB              : RP ".number_format($nilai_spb)."\n";
