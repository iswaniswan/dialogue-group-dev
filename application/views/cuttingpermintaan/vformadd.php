                ...
                <input readonly type = "hidden" name = "dfrom" value = "<?= $dfrom ;?>"><input readonly type = "hidden" name = "dto" value = "<?= $dto ;?>">
                    <div class="col-md-12">
                        <div class="form-group row">
                            ...
                            <label class="col-md-6">Tanggal Permintaan</label>
                            <div class="col-sm-3">
                            ...
                            <div class="col-sm-3">
                                <div class="input-group">
                                    <input type="text" name="ispbb" id="ispbb" readonly="" autocomplete="off" onkeyup="gede(this);" placeholder="<?= $number;?>" maxlength="25" class="form-control input-sm" value="" aria-label="Text input with dropdown button">
                            ...
                            <div class="col-sm-3">
                                <input type="text" name="dspbb" id="dspbb" class="form-control date" onchange="number();" value="<?= date('d-m-Y') ;?>" readonly>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-2">
                                <input readonly type="text" name="dschedule" id="dschedule" class="form-control" value="" readonly>
                            </div>

                            <div class="col-sm-6">
                                <textarea name="eremarkh" class="form-control"></textarea>
                            </div>
                        </div>    
                    </div>
                    <div class="col-md-12">
...
$(document).ready(function () {
    ...
    $('#send').click(function(event) {
        ...
        for (let x = 0; x < strArray.length; x++) {
            statuschange('<?= $folder;?>',strArray[x],'2','<?= $dfrom."','".$dto;?>');
            //document.write("<p>" + strArray[x] + "</p>");
        }
        // alert();
        ...
        $.ajax({
            ...
            if((data['data'][0]!=null && data['brgop'] != null)){ 
                ...
                for (let a = 0; a < data['brgop'].length; a++) {
                    ...
                    var id_company     = data['brgop'][a]['id_company'];
                    ...
                    if(group==""){
                        cols2 += '<td class = "bg-success text-white" colspan = "11" style=\"font-size:16px;\"><b>'+i_product_wip+' ('+e_product_wipname+') - '+e_color_name+'<input style="width:100px;" class="form-control inputitem" type="text" id="nquantity'+a+'" name="nquantity'+a+'" value="'+n_quantity_sisa+'" onkeyup="hitungnilai3(this.value,'+product2+'); validasiqty(this.value,'+product2+'); angkahungkul(this);" onblur=if(this.value==""){this.value="0";hitungnilai3(this.value,'+product2+');} onfocus=if(this.value=="0"){this.value="";hitungnilai3(this.value,'+product2+');}></b><input style="width:60px;" class="form-control" type="hidden" id="nquantitysisa'+a+'" name="nquantitysisa'+a+'" value="'+n_quantity_sisa+'"><label>Sisa Schedule '+n_quantity_sisa+'</label></td>';
                    }else{
                        if((group!=product)){
                            cols2 += '<td class = "bg-success text-white" colspan = "11" style=\"font-size:16px;\"><b>'+i_product_wip+' ('+e_product_wipname+') - '+e_color_name+'<input style="width:100px;" class="form-control inputitem" type="text" id="nquantity'+a+'" name="nquantity'+a+'" value="'+n_quantity_sisa+'" onkeyup="hitungnilai3(this.value,'+product2+'); validasiqty(this.value,'+product2+'); angkahungkul();" onblur=if(this.value==""){this.value="0";hitungnilai3(this.value,'+product2+');} onfocus=if(this.value=="0"){this.value="0";hitungnilai3(this.value,'+product2+');}></b><input style="width:60px;" class="form-control" type="hidden" id="nquantitysisa'+a+'" name="nquantitysisa'+a+'" value="'+n_quantity_sisa+'"><label>Sisa Schedule '+n_quantity_sisa+'</label></td>';
                        }
                    }
...
function getgudang(igudang) {
    ...
    function validasiqty(qty,kode){
        for(var a=0;a<$('#jml').val();a++){
            i=a;  
            sisa   = formatulang(document.getElementById("nquantitysisa"+i).value);
            qty    = formatulang(document.getElementById("nquantity"+i).value);
            if (parseFloat(qty) > parseFloat(sisa)) {
                swal("Tidak Boleh melebihi Sisa Schedule ( " + sisa +" )");
                $("#nquantity"+i).val(sisa);
                //qty = sisa;
            }
            if(parseFloat(qty) == '0' || parseFloat(qty) == null || parseFloat(qty) == ''){
                swal("Quantity tidak boleh 0 atau kosong");
                $("#nquantity"+i).val(sisa);
                hitungnilai3(sisa,kode);
            }
        }
    }
        ...
        if ($('#jml').val()==0) {
            ...
            $("#tabledata tbody tr").each(function() {
                $(this).find("td .inputitem").each(function() {
                    for(var a=0;a<$('#jml').val();a++){
                        sisa    = formatulang(document.getElementById("nquantitysisa"+a).value);
