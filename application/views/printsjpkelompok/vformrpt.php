<?php 
  ...
  foreach($isi as $row){
    ...
    $iarea  = $row->i_area;
    $isjtype= '01';
    $query  = $this->db->query(" select * from tm_sjp_item where i_sjp='$sj' and i_area='$iarea'",false);
    $jml  = $query->num_rows();
    $detail = $this->mmaster->bacadetail($row->i_sjp,$row->i_area);
    foreach($detail as $rowi){
      ...
      if($j>10){
        ...
        $hal=$hal+1;
        CetakHeader($row,$hal,$nor,$abn,$ab,$ipp,$cetak);
        $j  = 0;
      }
    }
