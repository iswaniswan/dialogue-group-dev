        ...
        <div class="col-lg-12">
            <div class="panel panel-info">
                <div class="panel-heading">
                    <i class="fa fa-plus"></i> &nbsp; <?= $title; ?> Budgeting <a href="#" onclick="show('<?= $folder; ?>/cform/index/<?= $dfrom . "/" . $dto; ?>','#main'); return false;" class="btn btn-info btn-sm pull-right"><i class="fa fa-list"></i>&nbsp;<?= $title_list; ?></a>
                </div>
                ...
                <table id="tabledatax" class="table color-table table-bordered class" cellpadding="8" cellspacing="1" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center link" style="width: 10%;">Kode</th>
                            <th class="text-center link" style="width: 30%;">Barang</th>
                            <th class="text-center link" style="width: 12%;">Satuan</th>
                            <!-- <th class="text-center" style="width: 10%;">Sisa</th> -->
                            <th class="text-center link" style="width: 10%;">Jml</th>
                            <th hidden class="text-center link" style="width: 20%;">Supplier</th>
                            <th hidden class="text-center link" style="width: 10%;">Harga Supp</th>
                            <!-- <th class="text-center" style="width: 10%;">Harga Adj</th> -->
                            <th class="text-center inputan" style="width: 20%;">Keterangan</th>
                            <th class="text-center informasi" style="width: 3%;"><input type="checkbox" id="checkAll" class="form-control input-sm"></th>
                        </tr>
                    </thead>
                    <tbody>
                    </tbody>
                    ...
                    if (!ada) {
                        swal({
                            ...
                            $.ajax({
                                type: "POST",
                                data: $("form").serialize(),
                                url: '<?= base_url($folder . '/cform/simpan_budgeting/'); ?>',
                                dataType: "json",
                                success: function(data) {
                                    if (data.sukses == true) {
                                        $('#id').val(data.id);
                ...
                success: function(data) {
                    if (data.length > 0) {
                        ...
                        for (let i = 0; i < data.length; i++) {
                            ...
                            var cols = "";
                            cols += '<td style="text-align: center;"><spanx id="snum' + i + '">' + no + '</spanx></td>';
                            cols += '<td><input type="text" id="i_material' + i + '" class="form-control input-sm" name="imaterial' + i + '" readonly value="' + data[i]['i_material'] + '"></td>';
                            cols += '<td><input type="text" id="e_material_name' + i + '" class="form-control input-sm" name="e_material_name' + i + '" readonly value="' + data[i]['e_material_name'] + '"></td>';
                            cols += '<td><input type="hidden" id="isatuan' + i + '" name="isatuan' + i + '" value="' + data[i]['i_satuan_code'] + '"><input type="text" readonly id="esatuan' + i + '" class="form-control input-sm" name="esatuan' + i + '" value="' + data[i]['e_satuan_name'] + '"></td>';
                            cols += '<td><input type="text" readonly id="nquantity' + i + '" class="form-control text-right input-sm inputitem" autocomplete="off" name="nquantity' + i + '" onblur=\'if(this.value==""){this.value="0";}\' onfocus=\'if(this.value=="0"){this.value="";}\' value="' + data[i]['n_sisa'] + '" onkeyup="angkahungkul(this);"></td>';
                            cols += '<td hidden><select disabled data-urut="' + i + '" id="isupplierex' + i + '" class="form-control input-sm" name="isupplierex' + i + '"><option value="' + data[i]['id_supplier'] + '" selected >' + data[i]['i_supplier'] + ' - ' + data[i]['e_supplier_name'] + '</option></select></td>';
                            // cols += '<td><input type="text" id="nsisa'+i+'" class="form-control text-right input-sm" name="nsisa' + i + '" readonly value="0"></td>';
                            cols += '<td hidden><input type="text" readonly id="harga_sup' + i + '" class="form-control text-right input-sm" autocomplete="off" name="harga_sup' + i + '" value="' + data[i]['v_price'] + '"></td>';
                            cols += '<td hidden><input type="text" readonly id="harga_adj' + i + '" class="form-control text-right input-sm inputitem" autocomplete="off" name="harga_adj' + i + '" onblur=\'if(this.value==""){this.value="0";}\' onfocus=\'if(this.value=="0"){this.value="";}\' value="' + data[i]['v_price_adj'] + '" onkeyup="angkahungkul(this);"></td>';
                            cols += '<td><input type="text" id="eremark' + i + '" class="form-control input-sm" name="eremark' + i + '" value="' + data[i]['e_remark'] + '"/><input type="hidden" id="ikode' + i + '" name="ikode' + i + '"/ value="' + data[i]['i_kode_kelompok'] + '" ><input type="hidden" id="id' + i + '" name="id' + i + '"/ value="' + data[i]['id'] + '" ><input type="hidden" id="isupplier' + i + '" name="isupplier' + i + '"/ value="' + data[i]['id_supplier'] + '" ></td>';
                            cols += '<td><input type="checkbox" id="check' + i + '" class="form-control input-sm" name="check' + i + '"/></td>';
                            newRow.append(cols);
                            $("#tabledatax").append(newRow);
                            $('#isupplierex' + i).select2({
                                placeholder: 'Cari Kode / Nama Supplier',
                                allowClear: true,
                                width: '100%',
                                type: "POST",
