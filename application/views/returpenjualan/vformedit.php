                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-3">
                                <div class="input-group">
                                    ...
                                    <input type="hidden" name="idocumentold" id="ireturold" value="<?= $data->i_document;?>">
                                    <input type="text" name="idocument" required="" id="iretur" readonly="" autocomplete="off" onkeyup="gede(this);" placeholder="<?= $number;?>" maxlength="25" class="form-control input-sm" value="<?= $data->i_document;?>" aria-label="Text input with dropdown button">
                                ...
                                <select name="ialasan" id="ialasan" class="form-control select2" required="" data-placeholder="Pilih Alasan Retur!">
                                    ...
                                    <?php if ($retur) {
                                        foreach ($retur as $row):?>
                                            <option value="<?= $row->id;?>" <?php if ($row->id == $data->id_alasan_retur) {?> selected <?php } ?>>
                                                <?= $row->e_alasan_name;?>
                                            </option>
                                        <?php endforeach; 
                                    } ?>
                ...
                <table id="tabledatay" class="table color-table success-table table-bordered class" cellpadding="8" cellspacing="1" width="100%">
                    ...
                    <tbody>
                        <?php $no = 0; $group = ""; foreach ($datadetail as $key) {
                            ...
                            if ($group=="") {?>
                                <tr class='hejo'>
                                    <td colspan="11">Nomor Nota : <b><?= $key->i_document;?></b> &nbsp;&nbsp;-&nbsp;&nbsp; Tanggal Nota : <b><?= $key->d_document;?></b> &nbsp;&nbsp;-&nbsp;&nbsp; Nomor SJ : <b><?= $key->i_sj;?></b></td>
                                </tr>
                            <?php }else{
                                if($group!=$key->id_referensi.$key->id_document){?>
                                <tr class='hejo'>
                                    <td colspan="11">Nomor Nota : <b><?= $key->i_document;?></b> &nbsp;&nbsp;-&nbsp;&nbsp; Tanggal Nota : <b><?= $key->d_document;?></b> &nbsp;&nbsp;-&nbsp;&nbsp; Nomor SJ : <b><?= $key->i_sj;?></b></td>
                                </tr>
                                <?php $no = 1; }
                            ...
                            $group = $key->id_referensi.$key->id_document;
                            ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <input name="iddocument<?=$i;?>" type="hidden" value="<?= $key->id_referensi;?>" readonly>
                                    <input name="iddocumentdetail<?=$i;?>" type="hidden" value="<?= $key->id_referensi_detail;?>" readonly>
                                </td>
                                <td><input type="text" id="nquantitysisa<?=$i;?>" class="form-control text-right input-sm" value="<?= $key->n_quantity_sisa_reff;?>" readonly></td>
                                <td><input type="text" id="nquantity<?=$i;?>" class="form-control text-right input-sm inputitem" autocomplete="off" name="nquantity<?=$i;?>" onblur="if(this.value==''){this.value='0';}" onfocus="if(this.value=='0'){this.value='';}" value="<?= $key->n_quantity;?>" onkeyup="angkahungkul(this); cekqty(<?=$i;?>); hitungtotal();"></td>
                                ...
                                <td><input type="text" readonly class="form-control input-sm text-right" name="vdiscount<?=$i;?>" id="vdiscount<?=$i;?>" value="<?= number_format($key->v_diskon_tambahan);?>"/></td>
                                <td><input type="text" readonly class="form-control input-sm text-right" name="vtotal<?=$i;?>" id="vtotal<?=$i;?>" value="<?= number_format($key->n_quantity * $key->v_price);?>"/><input type="hidden" readonly class="form-control input-sm text-right" name="vtotaldiskon<?=$i;?>" id="vtotaldiskon<?=$i;?>" value="<?= $key->v_diskon_total;?>"/></td>
                                <td><input type="text" class="form-control input-sm" name="eremark<?=$i;?>" id="eremark<?=$i;?>" value="<?= $key->e_remark;?>" placeholder="Jika Ada!"/></td>
                            </tr>
                        <?php $i++; } ?>
                    </tbody>
                    <tfoot>
                    ...
                    if (data['detail']!=null) {
                        ...
                        for (let i = 0; i < data['detail'].length; i++) {
                            ...
                            if (group=="") {
                                ...
                                if(group!=data['detail'][i]['id_document']+data['detail'][i]['id_document']){
                                    cils += `<td colspan="11">Nomor Nota : <b>${data['detail'][i]['i_document']}</b> &nbsp;&nbsp;-&nbsp;&nbsp; Tanggal Nota : <b>${data['detail'][i]['d_document']}</b>&nbsp;&nbsp;-&nbsp;&nbsp; Nomor SJ : <b>${data['detail'][i]['i_sj']}</b></td>`;
                                    no = 1; 
                                }
                            ...
                            group = data['detail'][i]['id_document']+data['detail'][i]['id_document'];
                            ...
                            cols += `<td><input type="text" id="nquantitysisa${i}" class="form-control text-right input-sm" value="${data['detail'][i]['n_quantity']}" readonly></td>`;
