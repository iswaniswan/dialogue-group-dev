    ...
    <div class="table-responsive">
        <table id="sitabel" class="table color-table inverse-table table-bordered class sitabel" cellpadding="8" cellspacing="1" width="100%">
            ...
            <tbody id="tableBody">
                ...
                <!-- <?php foreach($datadetailprod as $key) { ?>
                    <tr class="table-warning xx<?= $z; ?>">
                        <td class="text-center">
                            <spanlistx id="snum<?= $z; ?>"><b><?= $z; ?></b></spanlistx>
                        </td>
                        <td><?= $key->i_product_wip; ?></td>
                        <td colspan="8"><?= $key->e_product_wipname; ?></td>
                    </tr>
                    <tr class="tr table-info del<?= $z; ?>" id="tr<?= $z; ?>">
                        <td class="text-center"><a href="#" onclick="toge(<?= $z; ?>); return false;" class="toggler<?= $z; ?>" data-icon-name="fa-eye" data-prod-cat="eye_<?= $z; ?>"><i class="fa fa-eye fa-lg text-success"></i></a></td>
                        <td colspan="9" class="font"><b>LIST BARANG MATERIAL</b></td>
                    ...
                    <?php foreach($datadetail as $key2) { ?>
                        <?php if($key->id_product == $key2->id_product) { ?> 
                            <tr id="trdetail<?= $z . $i; ?>" class="table-success add<?= $z; ?> del<?= $z; ?> cat_eye_<?= $z; ?>">
                                ...
                                <?php if($key2->i_material_keluar != null) { ?>
                                    <td><?= $key2->i_material_keluar ?></td>
                                    <td><?= $key2->e_material_name_keluar . ' ' . $key2->e_satuan_name_keluar; ?></td>
                                    <td class="text-right"><?= $key2->n_quantity_keluar; ?></td>
                                <?php } else { ?>
                                    <td colspan="3"></td>
                                <?php } ?>
                                <?php if($key2->i_material_masuk != null) { ?>
                                    <td><?= $key2->i_material_masuk ?></td>
                                    <td><?= $key2->e_material_name_masuk . ' ' . $key2->e_satuan_name_masuk; ?></td>
                                    <td class="text-right"><?= $key2->n_quantity_masuk; ?></td>
                                <?php } else { ?>
                                    <td colspan="3"></td>
                                <?php } ?>
                                <td><?= $key2->e_remark; ?></td>
