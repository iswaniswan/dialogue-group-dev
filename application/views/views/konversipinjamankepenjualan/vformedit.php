                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-3">
                            <div class="input-group">
                                ...
                                <input type="hidden" name="ikppold" id="ikppold" value="<?= $data->i_document;?>">
                                <input type="text" name="idocument" required="" id="ikpp" readonly="" autocomplete="off" onkeyup="gede(this);" placeholder="<?= $number;?>" maxlength="16" class="form-control input-sm" value="<?= $data->i_document;?>" aria-label="Text input with dropdown button">
                        ...
                        <div class="col-sm-3">
                            <select name="ipartner" required="" id="ipartner" class="form-control select2">
                                <option value="<?= $data->id_partner."|".$data->e_type_partner;?>"><?= $data->e_partner_name;?></option>
                            </select>
                        </div>
                    </div> 
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <select name="ireff[]" id="ireff" required="" multiple="multiple" class="form-control input-sm select2">
                                <?php if ($referensi) {
                ...
                <tbody>
                    <?php $no = 0; $group = ""; foreach ($datadetail as $key) {
                        ...
                        <tr>
                            ...
                            <td><input readonly class="form-control input-sm" type="text" value="<?= $key->i_material;?>">
                                <input type="hidden" name="iddocument<?=$i;?>" value="<?= $key->id_document_reff;?>">
                                <input type="hidden" name="idmaterial<?=$i;?>" value="<?= $key->id_material;?>">
                            </td>
                            <td><input readonly class="form-control input-sm" type="text" value="<?= $key->e_material_name;?>"></td>
                            ...
                            <td><input class="form-control input-sm text-right" type="text" id="nquantity<?=$i;?>" name="nquantity<?=$i;?>" value="<?= $key->n_quantity;?>" placeholder="0" autocomplete="off" onkeyup="angkahungkul(this);cekjml(<?=$i;?>)"></td>
                            <td><input class="form-control input-sm" type="text" name="eremark<?=$i;?>" value="<?= $key->e_remark;?>" placeholder="Isi Keterangan Jika Ada!"></td>
                        </tr>
                        <?php $i++; 
                    } ?>
                </tbody>
...
<script>
                ...
                success: function (data) {
                    if (data['detail']!=null) {
                        ...
                        for (let x = 0; x < data['detail'].length; x++) {
                            ...
                            if(group==""){
                                cols1 += '<td colspan="8">Referensi : '+data['detail'][x]['i_document']+'</td>';
                            }else{
                                if(group!=data['detail'][x]['id_document']){
                                    cols1 += '<td colspan="8">Referensi : '+data['detail'][x]['i_document']+'</td>';
                                no = 1; }
                            }
                            newRow.append(cols1);
                            $("#tabledatay").append(newRow);
                            group = data['detail'][x]['id_document'];
                            var newRow = $("<tr>");
                            cols += '<td class="text-center">'+(x+1)+'</td>';
                            cols += '<td><input readonly class="form-control input-sm" type="text" value="'+data['detail'][x]['i_material']+'">';
                            cols += '<input type="hidden" name="iddocument'+x+'" value="'+data['detail'][x]['id_document']+'">';
                            cols += '<input type="hidden" name="idmaterial'+x+'" value="'+data['detail'][x]['id_material']+'"></td>';
                            cols += '<td><input readonly class="form-control input-sm" type="text" value="'+data['detail'][x]['e_material_name']+'"></td>';
                            ...
                            cols += '<td><input class="form-control input-sm text-right" type="text" id="nquantity'+x+'" name="nquantity'+x+'" value="'+data['detail'][x]['n_quantity_sisa']+'" placeholder="0" autocomplete="off" onkeyup="angkahungkul(this);cekjml('+x+')"></td>';
                            cols += '<td><input class="form-control input-sm" type="text" name="eremark'+x+'" placeholder="Isi Keterangan Jika Ada!"></td>';
                            newRow.append(cols);
                            $("#tabledatay").append(newRow);
                        }
                    }
    ...
    $('#ceklis').click(function(event) {
        if($('#ceklis').is(':checked')){
            $("#ikpp").attr("readonly", false);
        }else{
            $("#ikpp").attr("readonly", true);
    ...
    $( "#ikpp" ).keyup(function() {
        $.ajax({
            ...
            dataType: "json",
            success: function (data) {
                if (data==1 && ($('#ikpp').val() != $('#ikppold').val())) {
        ...
        if($("#jml").val()==0){
            ...
            for (var i = 0; i < $("#jml").val(); i++) {
                if($("#nquantity"+i).val()=='' || $("#nquantity"+i).val()==null || $("#nquantity"+i).val()==0){
                    swal('Maaf','Jumlah Konversi Harus Lebih Besar Dari 0!','error');
                    return false;
                }
            }
        }
