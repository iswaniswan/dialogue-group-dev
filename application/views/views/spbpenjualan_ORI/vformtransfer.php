                ...
                <table id="tabledatay" class="table color-table success-table table-bordered class" cellpadding="8" cellspacing="1" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" width="7%">Qty DO <br> Per Warna</th>
                    ...
                    <tbody>
                        <?php $no = 0; $group = ""; foreach ($datadetail as $key) {
                            ...
                            if($group==""){ ?>
                                <tr class="pudding">
                                    <td colspan="13">Nomor Referensi : <b><?= $key->i_op;?></b> &nbsp;&nbsp;-&nbsp;&nbsp; Tanggal Referensi : <b><?= $key->d_referensi;?></b> &nbsp;&nbsp;-&nbsp;&nbsp; Area : <b><?= $key->i_area.' - '.$key->e_area;?></b></td>
                                </tr>
                                <?php 
                            }else{
                                if($group!=$key->i_op){?>
                                ...
                                <tr>
                                    ...
                                    <td>    
                                         <input class="form-control input-sm text-right" type="text" name="harga<?=$i;?>" value="<?= number_format($key->v_price);?>">
                                    </td>
                                    <td><input class="form-control input-sm" type="text" name="eremark<?=$i;?>" value="<?= $key->e_op_remark;?>"></td>
                                    <td class="text-center"><input class="text-center" onclick="cek(<?=$i;?>);" type="checkbox" id="ceklis<?=$i;?>" name="ceklis<?=$i;?>" checked></td>
                                </tr>
                                <?php $i++; 
                            } ?>
                    </tbody>
...
<script>
    ...
    $( "#submit" ).click(function(event) {
        $.ajax({
            ...
            url: '<?= base_url($folder.'/cform/checkreferensi/'); ?>',
            dataType: "json",
            success: function (data) {
                if (data.sukses==true) {
                        ...
                        if (($('#ibagian').val()!='' || $('#ibagian').val()!=null) && ($('#idcustomer').val()!='' || $('#idcustomer').val()!=null)) {
                            if ($('#jml').val()==0) {
                                ...
                                for (var i = 0; i <= $('#jml').val(); i++) {
                                    if($('#ceklis'+i).is(':checked')){
                                        /* if (parseFloat($('#qty'+i).val()) % 1 != 0) {
                                            swal('Quantity Tidak Boleh Ada Desimal');
                                            return false;
                                        } else { */
                                            var i_product = $("#i_product"+i).val();
                                            var i_op = $("#iop"+i).val();
                                            var order = parseFloat($('#n_order'+i).val());
                                            //var qty = parseFloat($('#qty'+i).val());
                                            var sum = 0;
                                            $('.'+i_product+i_op).each(function(){
                                                
                                                var check = $(this).closest('tr').find('[type=checkbox]');
                                                //swal(""+check);
                                        ...
                                        // var row = $("#i_product"+i).closest('tr');
                    ...
                    swal("Maaf :(", "No Referensi : "+data.kode+", Sudah Ada Yang Input", "error"); 
                }
            },
            error: function () {
                swal("Maaf", "Data Gagal Disimpan :(", "error");
