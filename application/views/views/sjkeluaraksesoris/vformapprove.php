                ...
                <tbody>
                    <?php foreach ($datadetail as $key) {?>
                        <tr>
                            ...
                            <td>
                                <?= $key->i_material;?>
                                <input type="hidden" id="iproduct<?=$i;?>" name="iproduct<?=$i;?>" value="<?= $key->i_material;?>">
                            </td>
                            ...
                            <td class="text-right">
                                ...
                                <input type="hidden" id="nquantitysisa<?=$i;?>" name="nquantitysisa<?=$i;?>" value="<?= $key->n_quantity_sisa_reff;?>">
                            </td>
                            <td class="text-right">
                                <?= $key->n_quantity;?>
                                <input type="hidden" id="nquantity<?=$i;?>" name="nquantity<?=$i;?>" value="<?= $key->n_quantity;?>"></td>
                            <td><?= $key->e_remark;?></td>
                        </tr>
                    <?php $i++; } ?>
                </tbody>
...
<script>
    $('#approve').click(function(event) {
        ...
        for (var x = 0; x < $("#jml").val(); x++) {
            if (parseInt($('#nquantitysisa'+x).val()) < parseInt($('#nquantity'+x).val())) {
                var habis = true;
        ...
        if (habis==false) {
            ...
            swal("Yaaahhh :(", "Jml Kirim Kode "+iproduct.substr(0,n)+" Melebihi Jml Sisa :(", "error");
            return false;
        }
    });
</script>