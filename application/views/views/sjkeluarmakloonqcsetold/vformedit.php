            ...
            <div class="panel-body table-responsive">
                ...
                <div class="col-md-12">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-3">
                            <input type="text" id="ipermintaan" name="ipermintaan" class="form-control input-sm" maxlength="18" value="<?= $data->i_permintaan;?>" required="" placeholder="No Permintaan Harus Diisi!">
                        </div>
                        <div class="col-sm-2">
                            <input type="text" id="dpermintaan" name="dpermintaan" class="form-control input-sm tgl" value="<?= $data->d_perkiraan;?>" readonly>
                        </div>
                        <div class="col-sm-4">
                    ...
                    <span class="notekode">* Note : No Dokumen Reset Sesuai Tipe Makloon</span><br>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <select data-nourut="<?= $i ;?>" id="idproduct<?= $i ;?>" class="form-control select2 input-sm" name="idproduct<?= $i ;?>" >
                                    <option value="<?= $key->id_product_wip;?>"><?= $key->i_product_wip.' - '.$key->namabarang;?></option>
                                </select>
                            </td>
                            ...
                            <td><input type="text" class="form-control input-sm" name="eremark<?= $i ;?>" id="eremark<?= $i ;?>" value="<?= $key->e_remark;?>" placeholder="Isi keterangan jika ada!"/><input type="hidden" name="vprice<?= $i ;?>" id="vprice<?= $i ;?>" value="<?= $key->v_price;?>"/></td>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#ipartner').select2({
            ...
            /* GET DATA DISKON DAN PKP SUPPLIER */
            ...
            $.ajax({
                ...
                url: '<?= base_url($folder.'/cform/partnerdetail'); ?>',
                dataType: "json",
                success: function (data) {
                    $('#fpkp').val(data[0].f_pkp);
                    $('#ndiskon').val(data[0].n_diskon);
                },
                error: function () {
                    swal('Data kosong : (');
                }
    ...
    $( "#ddocument" ).change(function() {
        $( "#dperkiraan" ).val($(this).val());
    });

    $( "#dperkiraan" ).change(function() {
        ...
        if (dperkiraan!=null&& ddocument!=null) {
            if (dperkiraan<ddocument) {
                swal('Tanggal Perkiraan Tidak Boleh Lebih Kecil Dari Tanggal Document!!!');
                $('#dperkiraan').val('');
            }
        }
    });
