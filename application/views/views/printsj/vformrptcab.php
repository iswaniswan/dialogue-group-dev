<?php 
   ...
   foreach($isi as $row){
      ...
      $query   = $this->db->query(" select * from dgu.tm_nota_item where i_sj='$sj' and i_area='$iarea'",false);
      $jml  = $query->num_rows();
      $total=0;
      foreach($detail as $rowi){
        if($rowi->n_deliver>0){
          ...
          $group='';
          $plu='';
            $qu   = $this->db->query(" select i_customer_plugroup from dgu.tr_customer_plugroup
                                   where i_customer='$row->i_customer'");
          if($qu->num_rows()>0){
            foreach($qu->result() as $ts){
              $group=$ts->i_customer_plugroup;
            }
            $qx   = $this->db->query("select i_customer_plu from dgu.tr_customer_plu
...
####
      ...
      $cetak.=CHR(27).CHR(120).CHR(1).str_repeat(" ",7).CHR(179)."    BAGIAN KEUANGAN CABANG : (".$row->e_area_phone." )             ".CHR(179).CHR(27).CHR(120).CHR(0)."\n";
