    ...
    <div class="col-lg-12">
        <div class="panel panel-info">
            ...
            <div class="panel-body table-responsive">
                <?php echo $this->pquery->form_remote_tag(array('url' => site_url($folder.'/cform/Realisasi'), 'update' => '#main', 'type' => 'post', 'class' => 'form-horizontal')); ?>
                <div id="pesan"></div>
                <div class="col-md-6">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12">
                            <select name="istore" id="istore" class="form-control select2" onchange="getdetstore(this.value);"></select>
                            <input type="hidden" id="estorename" name="estorename" class="form-control" maxlength="30" readonly>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-md-12">Lokasi Gudang</label>
                        <div class="col-sm-12">
                            <input type="hidden" id="istorelocation" name="istorelocation" class="form-control" maxlength="30" readonly>
                            <input type="text" id="estorelocationname" name="estorelocationname" class="form-control" maxlength="30" readonly>
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-md-3">
                                ...
                                <?php if ($detail) {
                                    ...
                                    foreach ($detail as $row) { 
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input class="form-control" style="text-align:right;" type="text" id="ndeliver<?=$i;?>" name="ndeliver<?=$i;?>" value="0" onkeyup="hitungnilai(this.value)" onblur="hitungnilai(this.value)" onpaste="hitungnilai(this.value)" autocomplete="off">
                                                <input class="form-control" readonly type="hidden" id="eremark<?=$i;?>" name="eremark<?=$i;?>" value="<?= $row->e_remark; ?>">
                                                <input type="hidden" id="nstock<?=$i;?>" name="nstock<?=$i;?>" value="">
                                            </td>
                                        </tr>
                                    <?php  } ?>
...
<script>
        ...
        $('#istore').select2({
            ...
            ajax: {
                ...
                data: function (params) {
                    var fspbstokdaerah = '<?= $isi->f_spb_stockdaerah; ?>';
                    var query = {
                        q: params.term,
                        fspbstokdaerah: fspbstokdaerah
                    }
                    return query;
                },
    ...
    function getdetstore(istore){
        var ispb           = '<?= $isi->i_spb; ?>';
        var dspb           = '<?= $isi->d_spb; ?>';
        var iarea          = '<?= $isi->i_area; ?>';
        var fspbstokdaerah = '<?= $isi->f_spb_stockdaerah; ?>';
        $.ajax({
            type: "post",
            data: {
            ...
            success: function (data) {
                ...
                $('#estorename').val(data[0].e_store_name);
                $('#submit').click();       
            },
            error: function () {
                alert('Error :)');
