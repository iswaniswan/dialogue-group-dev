    ...
    <div class="col-lg-12">
        <div class="panel panel-info">
            ...
            <div class="panel-body table-responsive">
                ...
                <div id="pesan"></div>
                <?php if($isi){
                    $plapon = $this->mmaster->baca_plapon($isi->i_customer);
                    foreach ($plapon->result() as $isi_plapon) {
                        $saldo = $isi_plapon->v_flapond;
                    }
                    $nota_sisa = $this->mmaster->nota_sisa($isi->i_customer);
                    $saldobaru = ($saldo-$nota_sisa);
                    $this->mmaster->updatesaldo($isi->i_customer, $saldobaru);
                    ?>
                    <div class="col-md-6">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-12">
                                <input id="eapprove1" name="eapprove1" maxlength="100" class="form-control">
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-sm-offset-5 col-sm-8">
                                <?php $this->load->model('spbapprovesales/mmaster');
                                $plapon = $this->mmaster->baca_plapon($isi->i_customer);
                                foreach ($plapon->result() as $isi_plapon) {
                                    $v_saldo = $isi_plapon->v_saldo;
                                }
                                $nilaispb = $this->mmaster->baca_nilai($isi->i_customer);
                                foreach ($nilaispb->result() as $nilaispbb) {
                                    $nilaibersih = $nilaispbb->nilai_spb;
                                }
                                $v_saldo = ($v_saldo - $nilaibersih);
                                if($v_saldo <0 ){
                                    $areaa = $this->mmaster->baca_area_pusat()->num_rows();
                                    if($areaa > 0){ ?>
                                ...
                                <?php if ($detail) {
                                    ...
                                    foreach ($detail as $row) { 
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input class="form-control form-control-success" readonly style="text-align:right;" type="text" id="norder<?=$i;?>" name="norder<?=$i;?>" onkeyup="hitungnilai(this.value)" value="<?= $row->n_order; ?>">
                                            </td>
                                            <td>
                                                <input class="form-control form-control-success" readonly style="text-align:right;" type="text" id="ndeliver<?=$i;?>" name="ndeliver<?=$i;?>" onkeyup="hitungnilai(this.value)" value="<?= $row->n_deliver; ?>">
                                            </td>
                                            <td>
