                            ...
                            <tbody>
                                ...
                                if ($datadetail) {
                                    ...
                                    foreach ($datadetail as $row) {
                                        $ii++;
                                        $sisa_b = $row->nquantity_permintaan - $row->n_quantity_total_per_product;
                                        if ($group != $row->i_product_base) {
                                            $i++; ?>
                                            <tr class="tr list-item tr_first<?= $i ?>">
                                                ...
                                                <td colspan="2"></td>
                                                <td class="text-right"><?= $row->e_remark_op ?></td>
                                            </tr>
                                        <?php }
                                        $group = $row->i_product_base; ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <input type="hidden" class="form-control input-sm text-right" id="nquantitytotal<?= $i; ?>" name="nquantitytotal[]" value="<?= $row->n_quantity_total_per_product; ?>" readonly>
            ...
            if (!ada) {
                ...
                if (cek) {
                    swal({
                        ...
                        if (isConfirm) {
                            $.ajax({
                                url: '<?= base_url($folder . '/cform/insertspbnewtransfer/'); ?>',
                            ...
                            statuschangetransfer('<?= $folder; ?>', $('#id').val(), '6', '<?= $dfrom . "','" . $dto; ?>');
                        }
                    });
                } else {
                    statuschangetransfer('<?= $folder; ?>', $('#id').val(), '6', '<?= $dfrom . "','" . $dto; ?>');
