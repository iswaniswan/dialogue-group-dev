                    ...
                    <div class="col-md-12">
                        ...
                        <div class="row">
                                <?php if ($data->i_status == '1' || $data->i_status == '3' || $data->i_status == '7') { ?>
                                    <div class="col-sm-3">
                                        <button type="button" id="submit" class="btn btn-success btn-block btn-sm"><i class="fa fa-save fa-lg mr-3"></i>Update</button>
                                    </div>
                                <?php } ?>
                                    <div class="col-sm-3">
                                        <button type="button" class="btn btn-inverse btn-block btn-sm" onclick="show('<?= $folder; ?>/cform/index/<?= $dfrom . "/" . $dto; ?>','#main'); return false;"><i class="fa fa-arrow-circle-left fa-lg mr-3"></i>Kembali</button>
                                    </div>
                                <?php if ($data->i_status == '1') { ?>
                                     <div class="col-sm-3"><button type="button" id="send" class="btn btn-primary btn-block btn-sm"><i class="fa fa-paper-plane-o fa-lg mr-3"></i>Send</button></div>
                                     <div class="col-sm-3"><button type="button" id="hapus" class="btn btn-danger btn-block btn-sm"><i class="fa fa-trash fa-lg mr-3"></i>Delete</button></div>
                                <?php } elseif ($data->i_status == '2') { ?>
                                     <div class="col-sm-3"><button type="button" id="cancel" class="btn btn-primary btn-block btn-sm"><i class="fa fa-refresh fa-lg mr-3"></i>Cancel</button></div>
                                <?php } ?>
            ...
            <table id="sitabel" class="table color-table success-table table-bordered class" cellpadding="8" cellspacing="1" width="100%">
                <thead>
                    <tr>
                        ...
                        <th width="7%">WIP</th>
                        <th width="25%">Nama WIP</th>
                        <th width="7%">Material</th>
                        <th width="16%">Nama Material</th>
                        <th width="7%">Satuan</th>
                        <th class="text-right" width="7%">Qty Kirim</th>
                        <th class="text-right" width="7%">Qty Terima</th>
                        <th class="text-right" width="7%">Jml Gelar</th>
                        <th>Keterangan</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    if ($datadetail) {
                        foreach ($datadetail as $key) { ?>
                            <tr>
                                ...
                                <td><input class="form-control input-sm" readonly type="text" id="i_product_wip<?= $i; ?>" name="i_product_wip<?= $i; ?>" value="<?= $key->i_product_wip; ?>"><input type="hidden" id="id_product_wip<?= $i; ?>" name="id_product_wip<?= $i; ?>" value="<?= $key->id_product_wip; ?>"></td>
                                <td><input class="form-control input-sm" readonly type="text" id="e_product_wipname<?= $i; ?>" name="e_product_wipname<?= $i; ?>" value="<?= $key->e_product_wipname . ' - ' . $key->e_color_name; ?>"></td>
                                ...
                                <td><input class="form-control input-sm text-right" type="number" id="n_quantity<?= $i; ?>" name="n_quantity<?= $i; ?>" value="<?= $key->n_quantity; ?>" placeholder="0" onkeypress="return hanyaAngka(event);" onkeyup="ngetang(<?= $i; ?>);" onmouseup="ngetang(<?= $i; ?>);"></td>
                                <td>
                                    <input readonly class="form-control input-sm text-right" type="text" id="n_quantity_gelar<?= $i; ?>" name="n_quantity_gelar<?= $i; ?>" value="<?= $key->n_jumlah_gelar; ?>">
                                    <input type="hidden" id="v_gelar<?= $i; ?>" name="v_gelar<?= $i; ?>" value="<?= $key->v_gelar; ?>">
                                    <input type="hidden" id="v_set<?= $i; ?>" name="v_set<?= $i; ?>" value="<?= $key->v_set; ?>">
                                </td>
                                <td><input type="text" class="form-control input-sm" placeholder="Isi keterangan jika ada!" value="<?= $key->e_remark; ?>" name="e_remark_item<?= $i; ?>">
                                    <input type="hidden" class="form-control input-sm" placeholder="" value="<?= $key->id_item_keluar; ?>" name="id_item_keluar<?= $i; ?>"></td>
                            </tr>
                    <?php $i++;
                        }
                    } ?>
...
<script>
                ...
                success: function(data) {
                    if (data['detail'] != null) {
                        ...
                        for (let x = 0; x < data['detail'].length; x++) {
                            ...
                            cols += '<td><input class="form-control input-sm" readonly type="text" id="i_product_wip' + x + '" name="i_product_wip' + x + '" value="' + data['detail'][x]['i_product_wip'] + '"><input type="hidden" id="id_product_wip' + x + '" name="id_product_wip' + x + '" value="' + data['detail'][x]['id_product_wip'] + '"></td>';
                            cols += '<td><input class="form-control input-sm" readonly type="text" id="e_product_wipname' + x + '" name="e_product_wipname' + x + '" value="' + data['detail'][x]['e_product_wipname'] + ' - ' + data['detail'][x]['e_color_name'] + '"></td>';
                            ...
                            cols += '<td><input class="form-control input-sm text-right" type="number" id="n_quantity' + x + '" name="n_quantity' + x + '" value="' + data['detail'][x]['n_quantity_sisa'] + '" placeholder="0" onkeypress="return hanyaAngka(event);" onkeyup="ngetang(' + x + ');" onmouseup="ngetang(' + x + ');"></td>';
                            cols += '<td>';
                            cols += '<input class="form-control input-sm text-right" type="text" readonly id="n_quantity_gelar' + x + '" name="n_quantity_gelar' + x + '" value="' + data['detail'][x]['jumlah_gelar'] + '">';
                            cols += '<input type="hidden" readonly id="v_gelar' + x + '" name="v_gelar' + x + '" value="' + data['detail'][x]['v_gelar'] + '">';
                            cols += '<input type="hidden" readonly id="v_set' + x + '" name="v_set' + x + '" value="' + data['detail'][x]['v_set'] + '">';
                            cols += '</td>';
                            cols += '<td><input type="text" class="form-control input-sm" placeholder="Isi keterangan jika ada!" name="e_remark_item' + x + '"><input type="hidden" class="form-control input-sm" placeholder="" name="id_item_keluar' + x + '" value="' + data['detail'][x]['id'] + '"></td>';
                            newRow.append(cols);
    ...
    function ngetang(i) {
        ...
        $('#n_quantity_gelar' + i).val(parseFloat($('#n_quantity' + i).val())/parseFloat($('#v_gelar' + i).val()));
    }
</script>