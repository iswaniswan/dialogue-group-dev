    ...
    <div class="col-lg-12">
        <div class="panel panel-info">
            ...
            <div class="panel-body table-responsive">
                <?php echo $this->pquery->form_remote_tag(array('url' => site_url($folder.'/cform/updatespbpromo'), 'update' => '#pesan', 'type' => 'post', 'class' => 'form-horizontal')); ?>
                <?php 
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12">
                            <?php if ($isi->i_nota!='') {?>
                                <input id="ipromo" name="ipromo" type="hidden" value="<?= $isi->i_spb_program;?> ">
                                <select class="form-control" disabled="true">
                                    <?php if ($promo) {
                                        foreach ($promo as $key) { ?>
                                            <option value="<?= $key->i_promo;?>" <?php if ($key->i_promo==$isi->i_spb_program) { ?> selected <?php } ?>><?= $key->i_promo." - ".$key->e_promo_name;?></option> 
                                        <?php }
                                    } ?>   
                                </select>
                            <?php }else{ ?>
                                <select name="ipromo" id="ipromo" class="form-control" onchange="promo(this.value);">
                                    <?php if ($promo) {
                                        foreach ($promo as $key) { ?>
                                            <option value="<?= $key->i_promo;?>" <?php if ($key->i_promo==$isi->i_spb_program) { ?> selected <?php } ?>><?= $key->i_promo." - ".$key->e_promo_name;?></option> 
                                        <?php }
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12">
                            ...
                            <input id="eareaname" name="eareaname" type="hidden" value="<?= $isi->e_area_name; ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12">
                            ...
                            <?php }else{ ?>
                                <select name="icustomer" id="icustomer" class="form-control select2" onchange="getdetailpel(this.value);">
                                    <option value="<?= $isi->i_customer;?>"><?= $isi->e_customer_name; ?></option>
                                </select>
                            <?php } ?>
                            <input id="icustomergroup" name="icustomergroup" type="hidden" value="<?= $isi->i_customer_group; ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                ...
                <div class="form-group row">
                    <div class="col-sm-offset-5 col-sm-8">
                        <?php if(($isi->i_store =='') || ($isi->i_approve1=='') || ($isi->i_approve2=='')){?>
                    ...
                    <div class="col-md-6"> 
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-6">
                                <input id="vcustomerdiscount4" name="vcustomerdiscount4" class="form-control" required="" readonly value="<?= number_format($disc4); ?>">
                            </div>
                            <input type="hidden" readonly id="vcustomerdiscount4x" name="vcustomerdiscount4x" value="">
                        </div>
                        <div class="form-group row">
                            ...
                            <tbody>
                                ...
                                if($detail){
                                    foreach($detail as $row){ 
                                        ...
                                        <td>
                                            <input style="font-size: 12px;" class="form-control" id="eremark<?= $i;?>" name="eremark<?= $i;?>" value="<?= $row->e_remark;?>" maxlength="50"></td>
                                        </tr>
                                    <?php }
                                } ?>
...
<script>
    ...
    $("#addrow").on("click", function () {
        ...
        cols += '<td><input id="vproductretail'+xx+'" class="form-control" name="vproductretail'+xx+'"/ readonly><input readonly type="hidden" id="v_product_min'+xx+'" name="v_product_min'+xx+'" value="" readonly></td>';
        cols += '<td><input value="" id="norder'+xx+'" class="form-control" name="norder'+xx+'" onkeypress="return hanyaAngka(event)" onblur="cekminimal('+xx+');" onkeyup="hitungnilai(this.value,'+xx+')" autocomplete="off"><input type="hidden" id="nquantitystock'+xx+'" name="nquantitystock'+xx+'" value=""></td>';
        cols += '<td><input id="vtotal'+xx+'" class="form-control" name="vtotal'+xx+'"/ readonly></td>';
        ...
        $('#iproduct'+xx).select2({
            ...
            allowClear: true,
            ajax: {
                url: '<?= base_url($folder.'/cform/databrgpromo/'); ?>',
                dataType: 'json',
                delay: 250,
    ...
    function getdetailpel(icustomer){
        ...
        $.ajax({
            ...
            url: '<?= base_url($folder.'/cform/getdetailpelreguler'); ?>',
            dataType: "json",
...
function getharga(id){
    ...
    if(!ada){
        ...
        $.ajax({
            ...
            url: '<?= base_url($folder.'/cform/getdetailbarpromo'); ?>',
            dataType: "json",
            success: function (data) {
...
function hitungnilai(isi){
    ...
    if (isNaN(parseFloat(isi))){
        ...
        if(document.getElementById("ncustomerdiscount1").value=='0.00' && document.getElementById("vcustomerdiscount1").value>0){
            vtot =0;
            vtotx=0;
            for(i=1;i<=jml;i++){
            ...
            vtotdis=document.getElementById("vcustomerdiscount1").value;
