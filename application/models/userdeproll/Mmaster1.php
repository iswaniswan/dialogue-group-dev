...
class Mmaster extends CI_Model {
	...
	function data($i_menu, $idcompany, $folder){
        ...
        $datatables->query("select ROW_NUMBER () OVER (ORDER BY id_company) as no, id_company, username, i_departement, i_level, 
          i_apps, name, e_departement_name, e_level_name, status, i_menu, '$folder' as folder from (
          select distinct on(a.i_departement, a.i_level) a.id_company, a.username, a.i_departement, a.i_level, a.i_apps
                          , d.name, b.e_departement_name, c.e_level_name,'$i_menu' as i_menu, case when a.status = TRUE then 'Aktif' else 'Tidak Aktif' end as status
                            ...
                            inner join company d on a.id_company = d.id
                            where a.id_company = '$idcompany' and a.i_apps = '2'
                          ) as x", false);
...
// $ilevel/$idept/$icompany/$iapps
  ...
  public function getcustomer($idep, $ilev, $imenu, $isubmenu){
    ...
    $this->db->select("x.i_menu, x.id, x.e_menu, x.i_apps, x.e_name, sum(x.ada) as adaaja, x.e_name from(
...
public function getstock($lok,$iproduct){
  ...
  public function bacadepart(){
    $this->db->select(" * from public.tr_departement ",false);
    return $this->db->get();
  }
