...
/*(a.d_faktur_date_from ||' s/d '||a.d_faktur_date_to) as periode, */
  ...
  public function data($i_menu,$folder,$dfrom,$dto){
    ...
    if ($dfrom!='' && $dto!='') {
        ...
        $where = "AND a.d_retur BETWEEN '$dfrom' AND '$dto'";
    }else{
        $where = "";
    }
    ...
    $datatables->query("
                        ...
                        FROM 
                          ...
                          LEFT JOIN 
                            tm_retur_belibahanpembantu_item b ON (a.id = b.id_retur_beli AND a.id_company = b.id_company)
                          LEFT JOIN 
  ...
  public function cek_kode($kode,$ibagian){
    ...
    $this->db->from('tm_retur_belibahanpembantu');
  ...
  public function bacasupplier($cari){
    ...
    return $this->db->query("
                              ...
                              FROM
                                ...
                                JOIN 
                                   tm_btb_item c 
                                   ON (b.id = c.id_btb AND b.id_company = c.id_company)
                              WHERE
                                (a.i_supplier like '%$cari%' or b.e_supplier_name like '%$cari%')
                                AND a.id_company = '$this->idcompany'
                                AND b.i_status = '6'
                                AND c.n_retur_sisa <> '0'
                                AND b.i_bagian IN 
                                 (
                                    SELECT
                                       i_bagian 
  ...
  public function insertheader($id, $iretur, $dateretur, $ibagian, $isupplier, $esupplier, $vtot, $eremark){
    ...
    $this->db->insert('tm_retur_belibahanpembantu', $data);
  }

  public function insertdetail($id, $idbtb, $idnota, $isj, $imaterial, $isatuan, $nquantity, $vunitprice, $edesc){
    $idcompany  = $this->session->userdata('id_company');
    $data = array(
      'id_retur_beli'  => $id,
      'id_btb'         => $idbtb,
    ...
    $this->db->insert('tm_retur_belibahanpembantu_item', $data);
  }
    ...
    return $this->db->query("
                            SELECT DISTINCT
                                ...
                                FROM
                                  ...
                                  LEFT JOIN 
                                    tm_retur_belibahanpembantu_item c
  ...
  public function cek_datadetail($id, $iretur, $isupplier){
    return $this->db->query("
                            SELECT 
                                ...
                                f.i_sj_supplier
                              FROM
                                tm_retur_belibahanpembantu_item a 
                                LEFT JOIN 
                                  tm_retur_belibahanpembantu b 
  ...
  public function updatedetail($id, $idretur, $imaterial, $isatuan, $nquantity, $edesc){
    ...
    $this->db->where('id_company', $this->idcompany);
    $this->db->update('tm_retur_belibahanpembantu_item', $data);
  }
    ...
    if ($istatus=='6') {
        $query = $this->db->query("
            SELECT 
               ...
               n_quantity 
            FROM 
               tm_retur_belibahanpembantu_item
            WHERE 
               id_retur_beli = '$id'
               AND id_company = '$this->idcompany'
            ", FALSE);
