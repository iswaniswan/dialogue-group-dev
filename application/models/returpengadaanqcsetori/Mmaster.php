 ...
 /*  public function changestatus($id,$istatus){
  ...
  public function runningnumber($thbl,$tahun,$ibagian){
    //var_dump($thbl);
        ...
        $query  = $this->db->query("
            SELECT
                max(substring(i_retur, 10, 6)) AS max
            FROM
                tm_retur_keluar_pengadaan
            WHERE to_char (d_retur, 'yyyy') >= '$tahun'
            AND i_status <> '5'
            AND i_bagian = '$ibagian'
            AND substring(i_retur, 1, 3) = '$kode'
            AND substring(i_retur, 5, 2) = substring('$thbl',1,2)
            AND id_company = '".$this->session->userdata("id_company")."'
        ", false);
        if ($query->num_rows() > 0){          
  ...
  function insertdetail($id, $idproductwip, $idmaterial, $nquantitymat, $edesc, $nquantityset1 ,$eremarkset1){      
      $idcompany  = $this->session->userdata('id_company');
      $this->db->set(
                    array(
                          'id_retur_keluar'           => $id,
                          ...
                          'n_quantity_retur'          => $nquantitymat,
                          'n_sisa_retur'              => $nquantitymat,
                          'id_company'                => $idcompany,
                          'e_remark'                  => $edesc,
                          'n_quantity_set_return'     => $nquantityset1,
                          'e_remark_set_return'       => $eremarkset1,
                        )
                    );
