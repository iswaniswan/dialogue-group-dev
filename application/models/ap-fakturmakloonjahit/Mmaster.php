...
class Mmaster extends CI_Model
{
     ...
     public function getpartner($cari, $i_menu) {
        return $this->db->query("
                                ...
                                WHERE
                                	...
                                	AND a.id_company = '".$this->session->userdata('id_company')."' 
                                	AND a.i_status = '6'
                                	AND a.id NOT IN (SELECT id_referensi FROM tm_notamakloonjahit WHERE id_company = '".$this->session->userdata('id_company')."')
                                ORDER BY
                                	b.e_supplier_name   
                                ", FALSE);
    }

    public function getreferensi($cari, $idpartner){
        return $this->db->query("
                                SELECT DISTINCT
                                	b.id_document_reff,
                                	c.id,
                                ...
                                FROM
                                    tm_masuk_makloon_unitjahit a
                                	LEFT JOIN 
                                        tm_masuk_makloon_unitjahit_item b
                                		ON (a.id = b.id_document 
                                		AND a.id_company = b.id_company)
                                	LEFT JOIN 
                                		tm_keluar_makloon_unitjahit c
                                		ON (b.id_document_reff = c.id 
                                		AND b.id_company = c.id_company)
                                WHERE 
                                	a.i_status = '6'
                                	AND a.id_supplier = '$idpartner'
                                	AND c.id NOT IN (SELECT id_referensi FROM tm_notamakloonjahit WHERE i_status <> '5' AND id_company = '".$this->session->userdata('id_company')."')
                                	AND a.id_company = '".$this->session->userdata('id_company')."'
                                ", FALSE);
    }
    ...
    public function getdetailref($id, $idpartner, $i_menu){
        return $this->db->query("
                                ...
                                FROM
                                	...
                                	LEFT JOIN 
                                		tm_keluar_makloon_unitjahit_item b
                                	...
                                	INNER JOIN 
                                		...
                                		AND a.id_company = c.id_company)
                                	LEFT JOIN 
                                		tm_masuk_makloon_unitjahit d
                                		ON (a.id_document = d.id
                                		AND a.id_company = d.id_company)
                                	LEFT JOIN 
                                		tm_keluar_makloon_unitjahit e
    ...
    public function cekkode($kode,$ibagian)
    {
        ...
        $this->db->from('tm_notamakloonjahit');
    ...
    public function insertheader($id, $ibagian, $inota, $dnota, $dreceivefaktur, $ipartner, $ireferensi, $dreferensi, $ifaktursup, $dfaktursup, $ipajak, 
        ...
        $this->db->insert('tm_notamakloonjahit', $data);
    }
    ...
    public function insertdetail($id, $idreffmasuk, $idwip, $nquantity, $harga, $hargatotal, $edesc) {
        ...
        $this->db->insert('tm_notamakloonjahit_item', $data);
    }
    ...
    public function send($kode){
        ...
        $this->db->update('tm_notamakloonjahit', $data);
    }
    ...
    public function bacadetail($id){
        return $this->db->query("
                                SELECT 
                                	...
                                	e.e_color_name 
                                FROM
                                	tm_notamakloonjahit_item a
                                	LEFT JOIN 
                                		tm_notamakloonjahit b 
                                		ON (a.id_document = b.id
                                		AND a.id_company = b.id_company)
                                	LEFT JOIN 
                                		tm_masuk_makloon_unitjahit c
    ...
    public function updatedetail($id, $idreffmasuk, $idwip, $nquantity, $harga, $hargatotal, $edesc) {
        ...
        $this->db->where('id_company', $this->session->userdata('id_company'));
        $this->db->update('tm_notamakloonjahit_item', $data);
    }
