...
class Mmaster extends CI_Model
{
		...
		$datatables->add('action', function ($data) {
			...
			if (check_role($i_menu, 3) /*&& ($tahun . $bulan) > date('Ym')*/) {
				if ($i_status == '1' || $i_status == '3') {
					$data         .= "<a href=\"#\" title='Edit' onclick='show(\"$folder/cform/tambah/$i_bagian/$id_customer/$tahun/$bulan/$dfrom/$dto/$id/\",\"#main\"); return false;'><i class='ti-pencil-alt mr-3'></i></a>";
				}
			}

			if (check_role($i_menu, 7)) {
			    if ($i_status == '2' && $i_level == $this->session->userdata('i_level') ) {
  ...
  //     where a.e_type_name = 'Distributor' and a.id_company = '$idcompany' ) AND id_company = '$idcompany' order by e_customer_name", false);
	...
	public function dataheader($idcompany, $idcustomer, $periode)
	{
		$this->db->select("id, i_status from tm_forecast_distributor where id_company = '$idcompany' and id_customer = '$idcustomer' and periode = '$periode' ", false);
		return $this->db->get();
	}
