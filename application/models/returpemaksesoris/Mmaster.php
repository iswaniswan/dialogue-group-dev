...
/*(a.d_faktur_date_from ||' s/d '||a.d_faktur_date_to) as periode, */
    ...
    $datatables->query("
                       SELECT DISTINCT
                        ...
                        FROM 
                          ...
                          LEFT JOIN 
                            tm_retur_beliaksesories_item b ON (a.id = b.id_retur_beli)
                          LEFT JOIN 
                            tm_btb c ON (b.id_btb = c.id)
                          LEFT JOIN 
    ...
    $datatables->add('action', function ($data) {
      ...
      $data = '';
          ...
          if (check_role($i_menu, 7)) {
            if ($i_status == '2') {
            $data .= "<a href=\"#\" title='Approve' onclick='show(\"$folder/cform/approve/$id/$iretur/$isupplier/$dfrom/$dto/\",\"#main\"); return false;'><i class='ti-check-box'></i></a>&nbsp;&nbsp;";
            }
          }
  ...
  public function runningid(){
      ...
      $this->db->from('tm_retur_beliaksesories');
                              ...
                              WHERE
                                 ...
                                 (
                                    ...
                                    FROM
                                       ...
                                       LEFT JOIN
                                          tr_type b 
                                          ON (a.i_type = b.i_type) 
                                    WHERE
                                       b.i_departement = '$idept' 
  ...
  public function getmemo($idnota){
      $idcompany  = $this->session->userdata('id_company');
      return $this->db->query("SELECT to_char(a.d_btb, 'dd-mm-yyyy') as d_nota from tm_btb a where a.id = '$idnota' AND a.id_company = '$idcompany'", false);
  }

  public function getmemodetail($isupplier, $idnota){
      $idcompany  = $this->session->userdata('id_company');
      return $this->db->query("
                                SELECT DISTINCT
                                    a.id_btb,
                                  ...
                                  FROM
                                    tm_btb_item a
                                    JOIN
                                      tm_btb b ON (a.id_btb = b.id AND a.id_company = b.id_company)
                                    JOIN
                                      tr_material c ON (a.i_material = c.i_material AND a.id_company = c.id_company)
                                    JOIN 
                                      tr_satuan d ON (c.i_satuan_code = d.i_satuan_code AND c.id_company = d.id_company AND a.id_company = d.id_company)
                                  WHERE 
                                    b.i_status = '6'
                                    AND a.id_btb = '$idnota'
  ...
  public function insertheader($id, $iretur, $dateretur, $ibagian, $isupplier, $esupplier, $vtot, $eremark){
      ...
      $this->db->insert('tm_retur_beliaksesories', $data);
  }
  ...
  public function insertdetail($id, $idbtb, $isj, $imaterial, $isatuan, $nquantity, $vunitprice, $edesc){
      ...
      $this->db->insert('tm_retur_beliaksesories_item', $data);
  }
  ...
  public function cek_data($id, $iretur, $isupplier){
    ...
    return $this->db->query("
                              ...
                              FROM
                                ...
                                LEFT JOIN 
                                  tm_retur_beliaksesories_item c
  ...
  public function cek_datadetail($id, $iretur, $isupplier){
    ...
    return $this->db->query("
                           SELECT 
                              ...
                              f.i_sj_supplier
                            FROM
                              tm_retur_beliaksesories_item a 
                              LEFT JOIN 
                                tm_retur_beliaksesories b 
  ...
  public function cek_supplier($isupplier){
      $idcompany  = $this->session->userdata('id_company');
      $this->db->select("* from tr_supplier where i_supplier = '$isupplier' and id_company = '$idcompany'", false);
      return $this->db->get()->result();
  }

  public function cek_referensi($isupplier){
      $idcompany  = $this->session->userdata('id_company');
      $this->db->select("* from tm_notabtb where i_supplier='$isupplier' and id_company = '$idcompany'", false);
      return $this->db->get()->result();
  }
  ...
  public function updateheader($id, $idn, $ibagian, $dateretur, $isupplier, $eremark){
        $idcompany  = $this->session->userdata('id_company');
        $data = array(
                  'i_retur_beli'            => $idn,
                  'i_bagian'                => $ibagian,
  ...
  public function updatedetail($id, $ifaktur, $imaterial, $isatuan, $nquantity, $edesc){
    $idcompany  = $this->session->userdata('id_company');
    $data = array(
      'n_quantity'     => $nquantity,
      'e_remark'       => $edesc,
    );
    $this->db->where('id_retur_beli', $id);
    $this->db->where('id_btb', $ifaktur);
    $this->db->where('i_material', $imaterial);
    $this->db->where('i_satuan_code', $isatuan);
    $this->db->where('id_company', $idcompany);
    $this->db->update('tm_retur_beliaksesories_item', $data);
  }
  ...
  public function changestatus($id,$istatus){
        ...
        $query = $this->db->query("
            SELECT 
               ...
               n_quantity 
            FROM 
               tm_retur_beliaksesories_item
            WHERE 
               id_retur_beli = '$id' ", FALSE);
        if ($query->num_rows()>0) {
            foreach ($query->result() as $key) {
                $this->db->query("
                    ...
                    WHERE
                        id_btb = '$key->id_btb'
                        AND i_material = '$key->i_material'", FALSE);
            }
        }
        $data = array(
            'i_status'  => $istatus,
