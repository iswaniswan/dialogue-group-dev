...
class Mmaster extends CI_Model
{
    function data($i_menu, $folder, $dfrom, $dto)
    {
        ...
        $cek = $this->db->query("SELECT i_bagian FROM tm_stockopname_pengadaan_material
        ...
        $datatables->query("SELECT 0 as no, a.id, a.id_company, a.i_document,
            ...
            FROM tm_stockopname_pengadaan_material a
    ...
    public function runningid()
    {
        ...
        $this->db->from('tm_stockopname_pengadaan_material');
    ...
    public function runningnumber($thbl, $tahun, $ibagian)
    {
        $cek = $this->db->query("
          ...
          FROM tm_stockopname_pengadaan_material
          WHERE i_status <> '5'
        ...
        $query  = $this->db->query("
          ...
          FROM
            tm_stockopname_pengadaan_material
    ...
    {
        $this->db->select("id from tm_stockopname_pengadaan_material where id_company = '$idcompany' and i_bagian = '$ibagian' and i_document = '$i_document' ", false);
        return $this->db->get();
    }
    ...
    public function barang($cari, $ibagian, $ddocument)
    {
        ...
        $sql = "SELECT x.id_company, a.i_material, a.id, a.e_material_name, b.e_satuan_name 
                FROM f_mutasi_material('$id_company', '$i_periode', '$d_jangka_awal', '$d_jangka_akhir', '$dfrom', '$dto', '$ibagian') x
                INNER JOIN tr_material a ON (a.id_company = x.id_company AND a.id = x.id_material)
                INNER JOIN tr_satuan b ON (a.id_company = b.id_company AND a.i_satuan_code = b.i_satuan_code)
                WHERE a.i_kode_group_barang = 'GRB0005' AND (
                    a.i_material ILIKE '%$cari%' OR a.e_material_name ILIKE '%$cari%'
                )";
        // var_dump($sql); die();
    ...
    public function barang_grb05($cari, $ibagian)
    {
        $sql = "SELECT a.i_material, a.id, a.e_material_name, b.e_satuan_name 
                FROM tr_material a 
                INNER JOIN public.company c ON c.id = a.id_company
                INNER JOIN tr_satuan b ON (a.id_company = b.id_company AND a.i_satuan_code = b.i_satuan_code)
                WHERE a.f_status = 't' AND 
                    a.id_company = '$this->id_company' AND
                    a.i_kode_group_barang = 'GRB0005' AND (
                    a.i_material ILIKE '%$cari%' OR a.e_material_name ILIKE '%$cari%'
                )";
    ...
    public function simpan($id, $ibagian, $idocument, $ddocument, $iperiode, $eremarkh)
    {
        ...
        $this->db->insert('tm_stockopname_pengadaan_material', $data);
    }
    ...
    public function simpandetail($id, $idmaterial, $qty, $eremark)
    {
        ...
        $this->db->insert('tm_stockopname_pengadaan_material_item', $data);
    }
    ...
    /* public function changestatus($id, $istatus)
    {
        ...
        $this->db->update('tm_stockopname_pengadaan_material', $data);
    } */
        ...
        if ($istatus == '3' || $istatus == '6') {
            ...
            if ($istatus == '3') {
                ...
                $this->db->query("INSERT INTO tm_menu_approve (i_menu,i_level,i_document,e_approve,d_approve,e_database) VALUES
                    ('$this->i_menu','$this->i_level','$id','$this->username','$now','tm_stockopname_pengadaan_material');", FALSE);
            }
        } else {
            $data = array(
                'i_status'  => $istatus,
    ...
    public function datadetail_edit($id)
    {
        return $this->db->query("
          ...
          from tm_stockopname_pengadaan_material_item a
    ...
    public function hapusdetail($id)
    {
        ...
        $this->db->delete('tm_stockopname_pengadaan_material_item');
    }
    ...
    public function get_export_so_material()
    {
        $sql = "SELECT a.id, a.i_material, initcap(a.e_material_name) e_material_name , initcap(b.e_satuan_name) e_satuan_name  
                FROM tr_material a 
                INNER JOIN tr_satuan b ON (
                ...
                WHERE a.f_status = 't' 
                    AND a.id_company = '$this->id_company' 
                    and i_kode_group_barang = 'GRB0005'
                ORDER BY 3 ASC;";
