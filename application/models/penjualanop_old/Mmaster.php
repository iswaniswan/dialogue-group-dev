...
class Mmaster extends CI_Model {
    ...
    public function getdetailpel($icustomer, $per){
        ...
        //     ORDER BY
        //         a.i_customer",false);
        $this->db->select("a.*, c.n_customer_discount1, c.n_customer_discount2, c.n_customer_discount3, b.e_salesman_name, b.i_salesman
            from tr_branch a
    ...
    public function runningnumber($customergroup, $thbl){      
        $th     = substr($thbl,0,4);      
        $asal   = $thbl;      
        ...
        if ($query->num_rows() > 0){           
            ...
            $this->db->query("
                ...
                WHERE
                    i_modul = 'SP'
                    AND substr(e_periode, 1, 4)= '$th'
                    AND i_area = '$customergroup' ", false);            
