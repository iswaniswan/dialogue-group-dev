...
class Mmaster extends CI_Model {
      ...
      if ($dfrom!='' && $dto!='') {
		...
		$datatables->add('action', function ($data) {
            ...
            if(check_role($i_menu, 2)){
                $data .= "<a href=\"#\" title='View' onclick='show(\"$folder/cform/view/$iop/$idpp/$idop/$dfrom/$dto/\",\"#main\"); return false;'><i class='ti-eye'></i></a>&nbsp;&nbsp;";
            }
            if(check_role($i_menu, 3) && ($i_status == '1'|| $i_status == '2' || $i_status == '3' || $i_status == '7') && $f_op_faktur != 't'){
                $data .= "<a href=\"#\" title='Edit' onclick='show(\"$folder/cform/edit/$iop/$idpp/$idop/$dfrom/$dto/\",\"#main\"); return false;'><i class='ti-pencil-alt'></i></a>&nbsp;&nbsp;";
            ...
            if(check_role($i_menu, 7) && ($i_status=='2')){
               if ($i_status == '2') {
                  $data .= "<a href=\"#\" onclick='show(\"$folder/cform/approval/$iop/$idpp/$idop/$dfrom/$dto/\",\"#main\"); return false;'><i class='ti-check-box'></i></a>&nbsp;&nbsp;&nbsp;";
              }
            }
   ...
   public function data_pp($folder, $dfrom, $dto){
      if ($dfrom!='' && $dto!='') {
         $dfrom = date('Y-m-d', strtotime($dfrom));
         $dto   = date('Y-m-d', strtotime($dto));
         $where = "AND a.d_pp BETWEEN '$dfrom' AND '$dto' AND a.i_status != '9' AND a.i_status='6' AND c.n_sisa > 0";
      }else{
         $where = "AND a.i_status != '9' AND a.i_status = '6' AND c.n_sisa > 0";
      }
	   ...
	   $datatables->query("
                           ...
                           FROM 
                              ...
                              LEFT JOIN tm_pp_item c
                                ON (a.id = c.id_pp)
                                WHERE a.id_company = '".$this->session->userdata('id_company')."'
                              $where
                           group by a.id, b.e_bagian_name
                           ORDER BY
                          	   a.i_pp,
                              a.d_pp
                        ", FALSE);
		...
		$datatables->add('action', function ($data) {
            ...
            if($i_status != '9'){
               $data .= "<a href=\"#\" title='Edit' onclick='callswal(\"$ipp\",\"$ibagian\"); return false;'><i class=' ti-new-window'></i></a>&nbsp;&nbsp;";
            }
			return $data;
      });
      $datatables->hide('folder');
   ...
   public function gudang($cari, $idepart){
      return $this->db->query("SELECT * FROM tr_bagian WHERE (UPPER(i_bagian) LIKE '%$cari%' OR UPPER(e_bagian_name) LIKE '%$cari%') AND id_company = '".$this->session->userdata("id_company")."' ORDER BY e_bagian_name", FALSE);
   }

   public function getpp($dfrom1, $dto1, $igudang) {
      return $this->db->query("SELECT i_pp FROM tm_pp WHERE d_pp >= '$dfrom1' AND d_pp <=  '$dto1' AND i_status = '6'  AND i_bagian = '$igudang' AND id_company = '".$this->session->userdata("id_company")."'", FALSE);
   }

   public function getsup($ipp) {
      return $this->db->query("
                              ...
                              FROM
                                 tm_pp_item a
                                 LEFT JOIN tm_pp b
                                 ON (a.i_pp = b.i_pp AND a.id_pp = b.id)
                                 LEFT JOIN
                                 tr_supplier_materialprice c
                                 ON (a.i_material = c.i_material and a.id_company = c.id_company)
                                 LEFT JOIN tr_supplier d
                                 ON (c.i_supplier = d.i_supplier and a.id_company = d.id_company)
                              WHERE
                                 a.i_pp = '$ipp'
      ...
      return $this->db->query("
                              SELECT DISTINCT
                                 ...
                                 a.i_status,
                              	...
                              	c.i_pp,
                              	a.i_bagian,
                              	f.e_bagian_name as bagian_pembuat, 
                              	to_char(a.d_deliv,'dd-mm-yyyy') AS d_deliv,
                              	a.i_status_op,
                              	d.e_status_op,
                              	e.e_bagian_name as gudang,
                              	a.i_supplier,
                              	a.e_supplier_name,
                                g.e_supplier_address,
                                 a.e_remark,
                                 a.n_top,
                                 a.n_diskon,
                                 a.i_type_pajak,
                              ...
                              FROM 
                              	tm_opbb a
                              	LEFT JOIN 
                              		tm_opbb_item b ON (a.id = b.id_op)
                              	LEFT JOIN 
                              		tm_pp c ON (b.id_pp = c.id and a.id_company = c.id_company)
                              	LEFT JOIN 
                              		tr_status_op d ON (a.i_status_op = d.i_status_op)
                              	LEFT JOIN 
                              		tr_bagian e ON (c.i_bagian = e.i_bagian and a.id_company = e.id_company)
                              	LEFT JOIN 
                                    tr_bagian f ON (a.i_bagian = f.i_bagian and a.id_company = f.id_company) 
                                LEFT JOIN 
                                    tr_supplier g ON (g.i_supplier = a.i_supplier and a.id_company = g.id_company) 
                              WHERE
                              	a.i_op = '$iop' /*and a.i_status = '6'*/
                                AND a.id_company = '".$this->session->userdata("id_company")."'
                              ",FALSE);
   }

  function bacadetail($isupplier){
    $this->db->select(" * from duta_prod.tm_opbb_item a
                      join duta_prod.tm_opbb b on a.i_op=b.i_op
                      where i_supplier = '$isupplier' and f_op_cancel = 'f'", false);
    return $this->db->get();
  }
...
//and a.i_satuan=b.i_satuan
   ...
   public function get_item($idpp, $idop, $idcompany){
      return $this->db->query("
                              SELECT DISTINCT ON (a.i_material)
				                  	a.*, 
                                 b.*,
                                 ...
                                 g.e_status_op
				                   FROM 
				                  	...
				                  	LEFT JOIN 
				                  	tm_pp_item d ON (a.id_pp = d.id_pp)
				                  	LEFT JOIN 
  ...
  function get_harga1($ipp, $isupplier){  
      $idcompany  = $this->session->userdata('id_company');      
      return $this->db->query("
                              SELECT 
                                 x.*,
                              ...
                              (   
                                    ...
                                    WHERE
                                       ...
                                       and c.v_harga_konversi is not null
                                       AND a.n_sisa NOT IN 
                                       (
                                          '0.00'
                                       )
  ...
  public function importancestatus($cari){
   return $this->db->query("SELECT * FROM tr_status_op WHERE (UPPER(i_status_op) LIKE '%$cari%' OR UPPER(e_status_op) LIKE '%$cari%') ORDER BY i_status_op", FALSE); 
  }

  public function paymenttype($cari){
   return $this->db->query("SELECT * FROM tr_payment_type WHERE (UPPER(i_payment_type) LIKE '%$cari%' OR UPPER(e_payment_type_name) LIKE '%$cari%') ORDER BY i_payment_type", FALSE); 
  }
  ...
  public function runningid()
  {
   ...
   public function updateharga($id, $imaterial, $vprice){
      $this->db->set(
         array(          
            'v_price'  => $vprice,
         ));
      ...
      $this->db->where('i_material',$imaterial);
      $this->db->update('tm_opbbx_item');
   }

	public function insertheader($id, $iop, $dateop, $ibagian, $isupplier, $ntop, $ndiskon, $itypepajak, $importantstatus, $datedeliv, $eremark, $fpkp, $esuppliername){
        $idcompany  = $this->session->userdata('id_company');
  ...
  public function update($id, $iop, $dateop, $ibagian, $isupplier, $ntop, $ndiskon, $itypepajak, $importantstatus, $datedeliv, $eremark, $fpkp, $esuppliername){
        $idcompany  = $this->session->userdata('id_company');
        $data = array(
         'id'               => $id,              
         'i_op'             => $iop,
  ...
  public function appr2($iop){
    ...
    function getdataheader($id){
         $this->db->select("distinct a.i_op, d.i_pp, a.i_payment_type, a.d_op, a.e_remark, a.f_op_cancel, b.i_supplier, b.e_supplier_name, d.e_approve, a.d_deliv, c.i_kode_master, e.e_nama_master, a.e_approval, d_pp, a.important_status, f.e_status
                        from tm_opbb a
    ...
    function getdatadetail($id){
         $this->db->select("a.i_op, a.i_pp, a.i_material, a.i_satuan,a.e_remark, a.i_kode_master, a.n_pemenuhan, b.e_material_name, c.e_satuan, d.i_kode_master,  b.e_material_name, c.e_satuan, d.i_kode_master, e.e_nama_master, sum(v_price) as total, po.d_op, a.n_quantity
                        from tm_opbb_item a
                        ...
                        where a.i_op='$id'
                        group by a.i_op, a.i_pp, a.i_material, a.i_satuan,a.e_remark, a.i_kode_master, a.n_pemenuhan, b.e_material_name, c.e_satuan, d.i_kode_master, e.e_nama_master, po.d_op, a.n_quantity", false);
