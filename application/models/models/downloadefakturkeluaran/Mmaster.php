  ...
  function data($dto,$jenis,$partner){
   ...
   public function partner($cari,$jenis) {
        $cari = str_replace("'", "", $cari);
            ...
            select i_customer, e_customer_name, e_customer_npwp, urut from (
                select i_customer, e_customer_name, e_customer_npwp, '2' as urut from tr_customer where f_customer_pkp = 't'
                union all
                select 'semua' as i_customer, 'semua' as e_customer_name, 'semua' as e_customer_npwp, '1' as urut
            ) as x
            where e_customer_name ilike '%$cari%' or e_customer_npwp ilike '%$cari%' or i_customer ilike '%$cari%' order by e_customer_name", FALSE);
    }
}
