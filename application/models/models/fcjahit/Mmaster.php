    ...
    */
    ...
    public function data($folder, $i_menu, $dfrom, $dto)
    {
        ...
        $datatables->add('action', function ($data) {
            ...
            if (check_role($i_menu, 2)) {
                $data .= "<a href=\"#\" title='Detail' onclick='show(\"$folder/cform/view/$id/$dfrom/$dto\",\"#main\"); return false;'><i class='ti-eye fa-lg text-success mr-2'></i></a>";
            }

            if (check_role($i_menu, 3)) {
                if ($i_status == '1' || $i_status == '2' || $i_status == '3' || $i_status == '7') {
                    $data .= "<a href=\"#\" title='Edit' onclick='show(\"$folder/cform/edit/$id/$dfrom/$dto\",\"#main\"); return false;'><i class='ti-pencil-alt fa-lg mr-2'></i></a>";
                }
            }

            if (check_role($i_menu, 7) && ($i_status == '2')) {
                if (($i_level == $this->session->userdata('i_level')) || $this->session->userdata('i_level') == 1) {
                    $data .= "<a href=\"#\" title='Approve' onclick='show(\"$folder/cform/approval/$id/$dfrom/$dto\",\"#main\"); return false;'><i class='ti-check-box text-primary fa-lg mr-2'></i></a>";
                }
            }
    ...
    public function datareferensi($folder, $i_menu, $dfrom, $dto)
    {
        ...
        $datatables->query("SELECT
                ...
                CASE WHEN f_over_budget = TRUE THEN 'Ya' ELSE 'Tidak' END over_budget,
                c.e_bagian_name,
                e_remark,
                a.i_bagian,
                '$i_menu' AS i_menu,
            ...
            where to_date(a.periode, 'YYYYmm') BETWEEN to_date('$dfrom', '01-mm-yyyy') AND to_date('$dto', '01-mm-yyyy')
                and a.i_status = '6' and a.id_company = '$this->company'
                and a.id not in (select id_referensi from tm_fcjahit where i_status in ('1', '2', '3', '6'))
            ", FALSE);
        ...
        $datatables->add('action', function ($data) {
            ...
            if (check_role($i_menu, 1)) {
                $data .= "<a href=\"#\" title='Buat FC Jahit' onclick='show(\"$folder/cform/prosesdata/$id/$dfrom/$dto/$tahun/$bulan\",\"#main\"); return false;'><i class='ti-new-window fa-lg'></i></a>";
            }
    ...
    public function datadetail($id)
    {
        ...
        $periode = date('Ym');
        //$periode = $this->db->query("select periode from tm_forecast_produksi where id = '$id'")->row()->periode;
        return $this->db->query("SELECT
            ...
            INNER JOIN tm_forecast_produksi b ON (a.id_forecast = b.id)
            INNER JOIN tr_product_base e ON (e.id = a.id_product)
            INNER JOIN tr_product_wip d ON (
                d.i_product_wip = e.i_product_wip AND e.id_company = d.id_company AND d.i_color = e.i_color
            )
            INNER JOIN tr_color h ON (
                h.i_color = d.i_color AND d.id_company = h.id_company
            )
            INNER JOIN tr_item_type c ON (
                c.i_type_code = e.i_type_code AND e.id_company = c.id_company
            )
            LEFT JOIN (SELECT id_product_wip, id_company, n_saldo_akhir FROM produksi.f_mutasi_saldoawal_pengadaan_newbie($this->id_company, '$periode', '$jangkaawal', '$jangkaakhir', '$dfrom', '$dto', '')) cc ON
                (cc.id_product_wip = d.id AND cc.id_company = d.id_company)
            WHERE
                a.id_forecast = '$id'
            ORDER BY
                c.e_type_name,
    ...
    public function runningnumber($thbl, $tahun, $ibagian)
    {
        ...
        $query  = $this->db->query("
            SELECT
                max(substring(i_document, 10, 2)) AS max
            FROM
                tm_fcjahit
            WHERE 
                i_status <> '5'
    ...
    public function insertheader($id,$idocument,$ddocument,$ibagian,$idreferensi,$iperiode,$eremark)
    {
        ...
        $this->db->insert('tm_fcjahit', $data);
    }

    /*----------  SIMPAN DATA ITEM BASE ----------*/

    public function insertdetailbase($id,$idproduct,$n_quantity,$n_pengadaan,$n_fcjahit,$n_fcjahit_internal,$n_fcjahit_eksternal,$n_fcjahit_ade,$e_remark)
    {
        $data = array(
            'id_company'            => $this->company,
            'id_forecast'           => $id,
            ...
            'n_fc'                  => $n_quantity,
            'n_pengadaan'           => $n_pengadaan,
            'n_quantity'            => $n_fcjahit,
            'n_quantity_internal'   => $n_fcjahit_internal,
            'n_quantity_eksternal'  => $n_fcjahit_eksternal,
            'n_quantity_ade'        => $n_fcjahit_ade,
            'n_sisa'                => $n_fcjahit,
            'e_remark'              => $e_remark,
        );
        $this->db->insert('tm_fcjahit_item', $data);
    }
    ...
    public function editheader($id)
    {
        return $this->db->query("
            SELECT
                ...
                to_char(a.d_document, 'DD FMMonth YYYY') AS ddocument,
    ...
    public function edititembase($id)
    {
        ...
        return $this->db->query("SELECT distinct 
                ...
                n_fc , n_pengadaan, n_quantity, n_quantity_internal, n_quantity_eksternal, n_quantity_ade,
                initcap(trim(h.e_color_name)) AS e_color_name, a.e_remark,
                e_type_name
            FROM
                tm_fcjahit_item a
            INNER JOIN tr_product_wip d on (d.id = a.id_product_wip AND a.id_company = d.id_company)
            INNER JOIN tr_product_base b ON (b.i_product_wip = d.i_product_wip AND d.i_color = b.i_color AND b.id_company = d.id_company)
            INNER JOIN tr_item_type t ON (t.i_type_code = b.i_type_code AND b.id_company = t.id_company)
            INNER JOIN tr_color h on (h.i_color = d.i_color AND d.id_company = h.id_company)
            where a.id_forecast = '$id'
            ORDER BY e_type_name, d.i_product_wip, e_product_name, e_color_name
        ", FALSE);
    }
    ...
    public function delete($id)
    {
        ...
        $this->db->delete('tm_fcjahit_item');
    }
    ...
    public function updateheader($id,$idocument,$ddocument,$ibagian,$idreferensi,$iperiode,$eremark)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('tm_fcjahit', $data);
    }
        ...
        if ($istatus=='3' || $istatus== '6') {
            ...
            if ($istatus == '3') {
                ...
                $this->db->query("INSERT INTO tm_menu_approve (i_menu,i_level,i_document,e_approve,d_approve,e_database) VALUES
                    ('$this->i_menu','$this->i_level','$id','$this->username','$now','tm_fcjahit');", FALSE);
            }
        } else{
            $data = array(
                'i_status'  => $istatus,
