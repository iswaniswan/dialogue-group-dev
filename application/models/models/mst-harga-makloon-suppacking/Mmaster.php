...
class Mmaster extends CI_Model {
	...
	function data($i_menu, $dfrom){
		$datatables = new Datatables(new CodeigniterAdapter);
		
    $datatables->query("select a.i_supplier, b.e_supplier_name, a.i_material, c.e_material_name, d.e_satuan, a.v_price, 
                      a.i_type_code, e.e_type_name, a.d_berlaku, a.f_status_aktif, '$i_menu' as i_menu
                      FROM tm_price_makloon_supplier_packing a 
                      ...
                      WHERE c.f_status_aktif = 't'
                      AND (d_berlaku >= to_date('$dfrom','dd-mm-yyyy')
                      AND d_berlaku <= to_date('$dfrom','dd-mm-yyyy'))
        ...
        $datatables->add('action', function ($data) {
            ...
            if(check_role($i_menu, 2)){
                $data .= "<a href=\"#\" onclick='show(\"mst-harga-makloon-suppacking/cform/view/$isupplier/$kodebrg\",\"#main\"); return false;'><i class='fa fa-eye'></i></a>&nbsp;&nbsp;";
            }
            if(check_role($i_menu, 3) && $f_status_aktif != 'f'){
                $data .= "<a href=\"#\" onclick='show(\"mst-harga-makloon-suppacking/cform/edit/$isupplier/$kodebrg\",\"#main\"); return false;'><i class='fa fa-pencil'></i></a>";
            }
...
//var_dump($ikodekelompok, $ikodejenis);
     ...
     if($ikodejenis == 'AJB'){
      ...
      $this->db->select(" a.i_material as i_material, a.e_material_name, a.i_satuan_code, a.i_kode_kelompok, a.i_type_code, b.i_satuan, d.e_nama, b.e_satuan, c.e_type_name FROM tr_material a
                        ...
                        select i_material||i_supplier FROM tm_price_makloon_supplier_packing a)
  ...
  function get_supplier(){
      $this->db->select('*');
      $this->db->from('tr_supplier');
      $this->db->where('i_type_makloon', 'JNM0007');
  ...
  public function insert($isupplier, $kodebrg, $harga, $ipriceno, $dateberlaku, $igroupbrg, $ikodekelompok, $ikodejenis, $isatuan,  $itipe){
    ...
    $this->db->insert('tm_price_makloon_supplier_packing', $data);
  }
  ...
  public function update($isupplier, $kodebrg, $dsebelum, $dateberlaku){
        $dupdate = date("Y-m-d");
        ...
        $data = array(
         'd_akhir' => $dakhir
          ...
          $this->db->update('tm_price_makloon_supplier_packing', $data);
  }
