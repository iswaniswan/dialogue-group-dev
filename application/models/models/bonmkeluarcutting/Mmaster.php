...
class Mmaster extends CI_Model
{
        ...
        $datatables->query(
            "   
                SELECT
                    ...
                    '$dto' AS dto
                FROM
                    tm_keluar_produksibb a
                INNER JOIN tr_status_document b ON
                    (b.i_status = a.i_status)
                INNER JOIN tm_keluar_produksibb_item c ON
                    (c.id_document = a.id)
    ...
    public function changestatus($id,$istatus)
    {
        if ($istatus=='6') {
            $query = $this->db->query("
                ...
                INNER JOIN tm_keluar_produksibb_item b ON
        ...
        $this->db->where('id', $id);
        $this->db->update('tm_keluar_produksibb', $data);
    }

    public function cek_kode($kode,$ibagian)
    {
        $this->db->select('i_document');
        $this->db->from('tm_keluar_produksibb');
    ...
    public function getspbb_detail($id)
    {
        return $this->db->query(
            "
                ...
                JOIN tr_polacutting f ON
                    (d.i_material = f.i_material
                    AND d.id_company = f.id_company)
                JOIN tr_product_wip g ON
                    (c.id_product = g.id)                
                JOIN tr_color h ON
                    (g.i_color =h.i_color AND g.id_company = h.id_company)
                WHERE
                    c.id_spbb = '$id'
    ...
    public function insertheader($id,$idocument,$ddocument,$ibagian,$itujuan,$eremarkh,$ispbb)
    {
        ...
        $this->db->insert('tm_keluar_produksibb', $data);
    }
    ...
    public function insertbonkdetail($id,$idproduct,$nquantity,$ispbb)
    {
        ...
        $this->db->insert('tm_keluar_produksibb_item', $data);
    }
    ...
    public function insertbonkdetailitem($id,$idproduct,$idmaterial,$npemenuhan,$npanjangkain,$ispbb,$eremark)
    {
        ...
        $this->db->insert('tm_keluar_produksibb_itemdetail', $data);
    }
    ...
    public function cek_datadetail($id){
        return $this->db->query(
                                "SELECT
                                    ...
                                    INNER JOIN tm_keluar_produksibb_item d ON
                                        (d.id_document = a.id_document
                                        AND a.id_product_wip = d.id_product_wip)
                                    INNER JOIN tr_product_wip e ON 
                                        (a.id_product_wip = e.id)
                                    INNER JOIN tr_color f ON 
                                        (e.i_color = f.i_color AND e.id_company = f.id_company)
                                    INNER JOIN tm_spbb_item g ON
                                        (a.id_spbb = g.id_spbb AND a.id_material = g.id_material)
                                    WHERE a.id_document = '$id'
                                ", FALSE);
    }
    ...
    public function deletedetail($id)
    {
        ...
        $this->db->delete('tm_keluar_produksibb_item');

        $this->db->where('id_document', $id);
        $this->db->delete('tm_keluar_produksibb_itemdetail');
    }
