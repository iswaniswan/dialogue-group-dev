...
class Mmaster extends CI_Model {
  ...
  public function getkelompok($id){
    ...
    $query = $this->db->query("SELECT d_akhir FROM tm_price_makloon_supplier_print", FALSE);
    foreach($query->result() as $row){
      if($row->d_akhir != null){
        $dakhir = $row->d_akhir;
    ...
    $datatables->query("
                        ...
                        FROM(
                              SELECT ROW_NUMBER() OVER (ORDER BY a.i_supplier) as nomor,
                                 ...
                                 '$folder' as folder
                              FROM
                                 tm_price_makloon_supplier_print a 
                                 INNER JOIN
                                    tr_supplier b 
  ...
  public function delete($isupplier, $kodebrg){
    $this->db->query(" DELETE FROM tm_price_makloon_supplier_print WHERE i_supplier = '$isupplier' and kode_brg = '$kodebrg'");
  }
  ...
  public function get_supplier(){
    return $this->db->query("SELECT i_supplier, e_supplier_name FROM tr_supplier WHERE i_type_makloon = 'JNM0001' ORDER BY i_supplier", FALSE)->result();
  }
  ...
  public function insert($isupplier, $kodebrg, $harga, $ipriceno, $dberlaku, $igroupbrg, $itypemakloon, $ikodekelompok, $ikodejenis, $isatuan,  $itipe){
    ...
    $this->db->insert('tm_price_makloon_supplier_print', $data);
  }
  ...
  public function update($isupplier, $kodebrg, $harga, $itipe, $isatuan, $dsebelum, $dberlaku){
        ...
        $data = array(
         ...
         'd_update'      => $dupdate
          ...
          $this->db->update('tm_price_makloon_supplier_print', $data);
  }
