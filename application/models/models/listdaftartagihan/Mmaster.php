    ...
    public function data($dfrom, $dto, $iarea, $folder, $iperiode, $title){
        ...
        $datatables->query("
            SELECT
                DISTINCT
                a.i_dt,
                to_char(a.d_dt, 'dd-mm-yyyy') AS ddt,
                to_char(a.d_dt, 'yyyymm') AS dperiode,
                a.d_dt AS d_dt,
            ...
            ORDER BY
                a.i_dt", false);
        $datatables->edit('f_dt_cancel', function($data){
            if($data['f_dt_cancel'] == "t"){
                return "Ya";
            }else{
                return "Tidak";
            }
        ...
        $datatables->add('action', function ($data) {
            ...
            $title      = $data['title'];
            $idtc       = $data['idtc'];
            $fcancel    = $data['f_dt_cancel'];
            $data       = '';
            $data      .= "<a href=\"#\" onclick='show(\"$folder/cform/proses/$idt/$ddt/$iarea/$dfrom/$dto\",\"#main\"); return false;'><i class='fa fa-pencil'></i></a>";
            if (($idtc==null||$idtc=='') && $fcancel!='t' && $iperiode<=$dperiode) {
                /*$data  .= "&nbsp;&nbsp;<a href=\"#\" onclick='deleteCRUD2(\"$folder/cform/delete/$title/$idt/$iarea/\"); return false;'><i class='fa fa-trash'></i></a>";*/
                $data  .= "<a href=\"#\" onclick='hapus(\"$idt\",\"$iarea\"); return false;'><i class='fa fa-trash'></i></a>";
            }
            return $data;
        });
        ...
        $datatables->hide('dto');
        $datatables->hide('d_dt');
        $datatables->hide('idtc');
