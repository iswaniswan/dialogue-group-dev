...
class Mmaster extends CI_Model
{
    ...
    public function data($i_menu, $folder, $dfrom, $dto)
    {
        ...
        $datatables->query("
            ...
            INNER JOIN tr_bagian bg ON (bg.i_bagian = a.i_bagian AND bg.id_company = '$id_company')
            ...
            LEFT JOIN tr_bagian be ON (be.i_bagian = a.i_pengirim AND be.id_company = dt.id_company)
    ...
    public function pengirim($cari, $pembuat)
    {
         return $this->db->query("
           SELECT distinct a.i_bagian, b.e_bagian_name  from tm_stb_cutting a 
           inner join tr_bagian b on (a.id_company = b.id_company and a.i_bagian = b.i_bagian)
           where a.id_company_tujuan = '$this->id_company' AND b.e_bagian_name ILIKE '%$cari%';
        "); 
    ...
    public function simpandetail($id, $idpanel, $qty, $eremark, $qty_penyusun)
    {
        $data = array(
            'id_company'          => $this->id_company,
            'id_document'         => $id,
            ...
            'n_quantity'          => $qty,
            'n_quantity_penyusun' => $qty_penyusun,
            'e_remark'            => $eremark,
        );
    ...
    public function dataedit($id)
    {
        return $this->db->query("
            ...
            INNER JOIN tr_bagian ef ON (ef.i_bagian = a.i_pengirim AND ef.id_company = f.id_company)
            LEFT JOIN tr_jenis_barang_keluar je ON (a.id_jenis_barang_keluar = je.id)
            WHERE a.id = '$id'
        ", FALSE);
    }
    ...
    {
        return $this->db->query("SELECT a.id, k.i_document, c.id||p.id_material::varchar as id_product_wip, c.i_product_wip, c.e_product_wipname, d.e_color_name, ki.n_quantity as keluarfull, ki.n_quantity as keluar, 
                COALESCE(b.n_quantity, 0) as masuk, b.e_remark, p.id as id_panel, p.bagian, p.i_panel, b.n_quantity_penyusun, m.i_material, m.e_material_name
            FROM tm_masuk_qcset_new a
            INNER JOIN tm_masuk_qcset_item_new b ON (a.id = b.id_document)
            INNER JOIN tm_stb_cutting k ON (k.id = a.id_document_reff)
            INNER JOIN tm_stb_cutting_item ki ON (
                k.id = ki.id_document AND b.id_panel_item = ki.id_panel_item
            )
            INNER JOIN tm_panel_item p ON (b.id_panel_item = p.id)
            inner join tr_product_wip c on (
                p.id_product_wip = c.id and c.id_company = $this->id_company
            )
            INNER JOIN tr_color d ON (
                c.i_color = d.i_color and c.id_company = d.id_company
            )
            INNER JOIN tr_material m ON (m.id = p.id_material)
            WHERE a.id = '$id'
    ...
    public function getdetailrefeks($id)
    {
        ...
        ", FALSE); */
        return $this->db->query("SELECT a.i_document, a.id, 1 AS id_jenis_barang_keluar, b.id_panel_item, c.id||e.id_material::varchar AS id_product_wip,
                f.i_material, f.e_material_name,g.e_satuan_name,c.i_product_wip, c.e_product_wipname, d.e_color_name, b.n_quantity, 
                e.id AS id_panel, e.bagian, e.i_panel, b.n_quantity_penyusun
            FROM
                tm_stb_cutting a
            INNER JOIN tm_stb_cutting_item b ON (a.id = b.id_document)
            INNER JOIN tm_panel_item e ON (b.id_panel_item = e.id )
            ...
            INNER JOIN tr_material f ON (f.id = e.id_material)
            INNER JOIN tr_satuan g ON (
