...
//------------------------------------------------------------------------------------
    function bacaperiode($dfrom,$dto,$interval)
    {
      $sql =" a.kode, a.bln, sum(a.omset) as omset, sum(a.retur) as retur 
              from( 
              SELECT (a.i_area) as kode, to_number(to_char(a.d_nota, 'mm'), '99') as bln, sum(a.v_nota_netto) AS omset, 0 as retur 
              FROM tm_nota a 
              WHERE a.f_nota_cancel='f' AND NOT a.i_nota IS NULL
              AND (a.d_nota >= to_date('$dfrom', 'dd-mm-yyyy') AND a.d_nota <= to_date('$dto', 'dd-mm-yyyy')) 
              GROUP BY a.i_area, to_char(a.d_nota, 'mm') 
              union all 
              SELECT (a.i_area) as kode, to_number(to_char(a.d_kn, 'mm'), '99') as bln, 0 as omset, sum(a.v_netto) AS jumlah 
              FROM tm_kn a 
              WHERE a.f_kn_cancel='f' AND (a.d_kn >= to_date('$dfrom', 'dd-mm-yyyy') AND a.d_kn <= to_date('$dto', 'dd-mm-yyyy')) 
              GROUP BY a.i_area, to_char(a.d_kn, 'mm') 
              ) as a 
