...
class Mmaster extends CI_Model {
	function bacadetaildo($ido,$supplier,$ddtap){
	    $this->db->select("a.i_do, a.d_do, a.i_op, b.e_supplier_name, c.i_product, c.e_product_name, c.n_deliver, c.v_product_mill, b.f_supplier_pkp
    ...
    function insertheader($idtap,$iarea,$isupplier,$ipajak,$ddtap,$dduedate,$dpajak,$fpkp,$ndiscount,$vgross,$vdiscount,$vppn,$vnetto,$thbl){
		if($dpajak!=''){
			$this->db->set(
				array(
						'i_dtap'	=> $idtap,
    ...
    function insertdetail($idtap,$ido,$iop,$isupplier,$iarea,$iproduct,$iproductmotif,$eproductname,$ddtap,$njumlah,$vpabrik,$diskon,$ddo,$j,$thbl){
    ...
    function deleteheader($idtap,$iarea,$isupplier,$ddtap,$thbl){
  		$this->db->query("delete from tm_dtap where i_dtap='$idtap' and i_area='$iarea' and i_supplier='$isupplier' and n_dtap_year = '$thbl'");
  		$this->db->query("delete from tm_jurnal_transharian where i_refference='$idtap' and i_area='$iarea' and d_refference='$ddtap'");
  		$this->db->query("delete from tm_jurnal_transharianitem where i_refference='$idtap' and i_area='$iarea' and d_refference='$ddtap'");
  		$this->db->query("delete from tm_general_ledger where i_refference='$idtap' and i_area='$iarea' and d_refference='$ddtap'");
    }

    public function deletedetail( $idtap,$ido,$iop,$isupplier,$iarea,$iproduct,$iproductmotif,$njumlah,$vpabrik,$vppn,$ndiskon,$i,$thbl){
		$kotor	= $njumlah*$vpabrik;
		$diskon	= $ndiskon;
		$bersih = $kotor-$diskon;    
		if( ($vppn==0) || ($vppn=='') ){
		...
		$this->db->query("DELETE FROM tm_dtap_item 
						  ...
						  and i_op='$iop' and i_product='$iproduct' and i_product_motif='$iproductmotif' and n_dtap_year = '$thbl' ");
	}
