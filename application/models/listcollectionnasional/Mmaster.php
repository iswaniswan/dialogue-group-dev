...
function bacaperiode($dfrom,$dto,$interval)
    {
      ...
      $sql =" a.bln, sum(a.total) as total, sum(a.realisasi) as realisasi from( 
              select substring(b.e_periode, 5, 2) as bln, sum(b.v_target_tagihan) as total, sum(b.v_realisasi_tagihan) as realisasi 
              from tm_collection a, tm_collection_item b
              where b.e_periode>='$perfrom' and b.e_periode<='$perto' and a.e_periode=b.e_periode
              group by substring(b.e_periode, 5, 2)) as a 
              group by a.bln 
              order by a.bln";
