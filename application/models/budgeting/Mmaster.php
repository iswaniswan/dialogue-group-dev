        ...
        return $this->db->query("SELECT
            ...
            FROM
                ...
                SELECT
                    ...
                    case 
                          when f.i_kode_group_barang = 'GRB0004' then (greatest(a.n_fc_berjalan, a.qty_do  ) + a.n_quantity_fc - a.n_quantity_stock - coalesce(a.n_quantity_packing,0)) * e.n_quantity
                ...
                /* INNER JOIN tr_product_wip_item e ON
                    (e.id_product_wip = d.id) */
                INNER JOIN (
                    select a.id_product_wip, a.id_material, a.f_budgeting,
                    case 
                        when COALESCE(b.v_panjang_bis,0) > 0 then ((1/a.v_set) * a.v_gelar) + 
                        (COALESCE (v_bisbisan,0)/COALESCE (v_panjang_bis,0)) 
                        else ((1/a.v_set) * a.v_gelar) 
                    end as n_quantity
                    from tr_polacutting_new a
                    left join tr_material_bisbisan b on (b.id = a.id_bisbisan and a.id_material = b.id_material)
                    where a.f_budgeting = 't' and a.f_status = 't') e ON (e.id_product_wip = d.id)
                LEFT JOIN tr_material f ON
                    (f.id = e.id_material)
                LEFT JOIN tr_satuan g ON
                    (g.i_satuan_code = f.i_satuan_code
                ...
                LEFT JOIN (
                    ...
                    f_mutasi_packing('$this->id_company', '$i_periode_now', '9999-01-01', '9999-12-01', '$date_from', '$date_to', '')
                ) as mp on (a.id_product = mp.id_product_base)
                LEFT JOIN (
                    SELECT
                        *
                    FROM
                        f_mutasi_material ('$this->id_company','$i_periode_now',
                        '9999-12-01',
                        '9999-12-31',
                        '$date_from',
                        '$date_to','')) h ON
