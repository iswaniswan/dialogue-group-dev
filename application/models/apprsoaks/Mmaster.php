...
function data($i_menu, $from, $to, $ibank){
  ...
  public function cek_data($dfrom, $dto, $ikodemaster){
    $this->db->select("a.*, b.*, c.e_material_name, d.e_satuan
                      from tt_stok_opname_bahan_bakuaks_detail a
                      join tt_stok_opname_bahan_bakuaks b on a.i_stok_opname_bahanbakuaks = b.i_stok_opname_bahanbakuaks
  ...
  public function updateheader($ikodeso, $ikodemaster, $bl, $th){
    ...
    $this->db->where('i_stok_opname_bahanbakuaks', $ikodeso);
    $this->db->where('i_kode_master', $ikodemaster);
    // $this->db->where('d_bulan', $bl);
    // $this->db->where('d_tahun', $th);
    $this->db->update('tt_stok_opname_bahan_bakuaks', $data);
  } 
  ...
  public function updatedetail($ikodeso, $imaterial){
     ...
     $this->db->update('tt_stok_opname_bahan_bakuaks_detail', $data);
  }
}
