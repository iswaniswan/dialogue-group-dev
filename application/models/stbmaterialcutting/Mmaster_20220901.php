...
class Mmaster extends CI_Model
{
    ...
    {
        return $this->db->query("SELECT a.*, b.e_bagian_name, c.e_bagian_name||' - '||d.name e_bagian_receive_name, to_char(a.d_document, 'dd-mm-yyyy') AS date_document
    ...
    function data_schedule($i_menu, $folder, $dfrom, $dto)
    {
        ...
        $datatables->query("WITH CTE AS (SELECT DISTINCT 0 AS no, ab.i_periode, c.i_product_wip, initcap(c.e_product_wipname) e_product_wipname,  a.id_product_wip, e.e_color_name, 
            ...
            INNER JOIN (
                SELECT a.id_product_wip, id_material, n_quantity 
                FROM tr_product_wip_item a
                INNER JOIN tr_type_makloon b ON (b.id = ANY(a.id_type_makloon))
                WHERE id_type_makloon NOTNULL AND b.e_type_makloon_name ILIKE '%CUTTING%'
    ...
    function schedule_detail($folder, $dfrom, $dto)
    {
        if ($dfrom != '' && $dto != '') {
            $dfrom = date('Y-m-d', strtotime($dfrom));
            $dto   = date('Y-m-d', strtotime($dto));
        ...
        return $this->db->query("WITH CTE AS (SELECT DISTINCT 0 AS no, ab.i_periode, c.i_product_wip, initcap(c.e_product_wipname) e_product_wipname,  a.id_product_wip, e.e_color_name, 
            b.id_material, d.i_material, initcap(d.e_material_name) e_material_name, initcap(f.e_satuan_name) e_satuan_name, a.n_fc_cutting, b.n_quantity, round(b.n_quantity * a.n_fc_cutting,4) qty,
            string_agg(g.d_document::varchar,', ') tanggal_schedule, '$dfrom' dfrom, '$dto' dto, '$folder' folder, ROW_NUMBER() OVER (ORDER BY a.id) AS i, a.id
            FROM tm_fccutting_detail a
            INNER JOIN tm_fccutting ab ON (ab.id = a.id_forecast)
    ...
    public function dataeditdetail_schedule($id_material, $id_product_wip, $i_periode, $id)
    {
        $today = date('Y-m-d');
        $jangkaawal = date('Y-m-01');
        $jangkaakhir = date('Y-m-d', strtotime("-1 days"));
