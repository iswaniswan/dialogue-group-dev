...
class Mmaster extends CI_Model
{
    ...
    function data($i_menu, $folder)
    {
        ...
        $datatables->query("SELECT
            ...
            LEFT JOIN tr_marker e ON (e.id = a.id_marker)
            WHERE
        ...
        $datatables->edit(
            'e_marker_name',
            function ($data) {
                $f_marker_utama = $data['f_marker_utama'];
                $e_marker_name = $data['e_marker_name'];
                if ($f_marker_utama == 't') {
                    $data   = "<span class=\"label label-default text-info font-weight-bold\"><b>$e_marker_name</b></span>";
                } else {
                    $data   = "<span class=\"label label-default text-danger font-weight-bold\"><b>$e_marker_name</b></span>";
                }
                return $data;
            }
        );
        ...
        $datatables->add('action', function ($data) {
            ...
            $id_marker  = trim($data['id_marker']);
            $i_menu  = $data['i_menu'];
            $folder  = $data['folder'];
            $data    = '';
            if (check_role($i_menu, 2)) {
                $data .= "<a href=\"#\" onclick='show(\"$folder/cform/view/$product/$icolor/$id_marker\",\"#main\"); return false;'><i class='ti-eye fa-lg text-success mr-3'></i></a>";
            }
            if (check_role($i_menu, 3)) {
                $data .= "<a href=\"#\" onclick='show(\"$folder/cform/edit/$product/$icolor/$id_marker\",\"#main\"); return false;'><i class='ti-pencil-alt fa-lg mr-3'></i></a";
            }
        ...
        $datatables->hide('f_marker_utama');
    ...
    public function productwipref($cari, $i_product_wip, $i_color, $id_marker)
    {
        $idcompany  = $this->session->userdata('id_company');
        $colmar = $i_color.$id_marker;
        return $this->db->query("SELECT
                DISTINCT 
                a.i_product_wip,
                initcap(e_product_wipname) AS e_product_wipname,
            ...
            WHERE
                ...
                AND a.i_color||b.id_marker <> '$colmar'
    ...
    public function getdetailref($i_product_wip, $i_color, $id_marker)
    {
        $idcompany  = $this->session->userdata('id_company');
        return $this->db->query("SELECT
                DISTINCT 
            ...
            LEFT JOIN tr_group_barang g ON (g.i_kode_group_barang = e.i_kode_group_barang AND e.id_company = g.id_company)
            WHERE
                a.f_status = 't'
                AND a.id_company = '$idcompany'
            ...
            GROUP BY 1,e.i_material,e.e_material_name,n_bisbisan,c.e_jenis_potong,a.e_bagian,e_nama_group_barang
            ORDER BY
    ...
    {
        $idcompany  = $this->session->userdata('id_company');
        return $this->db->query("SELECT i_material, d.e_nama_group_barang AS e_bagian_name/* string_agg(trim(e_bagian_name),', ') AS e_bagian_name */  
            FROM tr_material a
            LEFT JOIN tr_bagian_kelompokbarang b ON (b.i_kode_kelompok = a.i_kode_kelompok AND a.id_company = b.id_company)
            LEFT JOIN tr_group_barang d ON (d.i_kode_group_barang = a.i_kode_group_barang AND a.id_company = d.id_company)
    ...
    public function insertdetail($imaterial, $vgelar, $vset, $iproductwip, $icolor, $bagian, $bis3, $id_bisbisan, $f_cutting, $autocutter, $badan, $print, $bordir, $quilting, $id_type_makloon, $f_kain_utama, $f_budgeting, $f_jahit, $f_packing, $id_marker, $f_marker_utama)
    {
        if ($bis3 == '') {
            $bis3 = 0;
        }
        ...
        INTO tr_polacutting_new
                (id_company,id_product_wip,id_material,e_bagian,v_gelar,v_set,v_bisbisan,id_bisbisan,id_type_makloon,f_kain_utama,f_budgeting, f_jahit, f_packing, id_marker, f_marker_utama) 
        VALUES ('$idcompany','$iproductwip','$imaterial','$bagian','$vgelar','$vset','$bis3',$id_bisbisan,$id_type_makloon,'$f_kain_utama','$f_budgeting', '$f_jahit', '$f_packing', '$id_marker','$f_marker_utama')
        ");
    }

    public function updatedetail($imaterial, $vtoset, $vgelar, $vset, $fbis, $iproductwip, $icolor, $n_bagibis, $bagian, $bis3, $bis4, $id_bisbisan, $f_cutting, $autocutter, $badan, $print, $bordir, $quilting, $id_type_makloon, $f_kain_utama, $f_budgeting, $f_jahit, $f_packing, $id_marker, $f_marker_utama)
    {
        if ($bis3 == '') {
            $bis3 = 0;
        }
        ...
        $data = array(
            ...
            'id_marker'         => $id_marker,
            'f_marker_utama'    => $f_marker_utama
        );
        $this->db->insert('tr_polacutting_new', $data);
    }

    public function insertdetailall($imaterial, $vtoset, $vgelar, $vset, $fbis, $iproductwip, $icolor, $n_bagibis, $bagian, $bis3, $bis4, $id_bisbisan, $f_cutting, $autocutter, $badan, $print, $bordir, $quilting,$id_type_makloon, $f_kain_utama, $f_budgeting, $f_jahit, $f_packing, $id_marker,$f_marker_utama)
    {
        if ($bis3 == '') {
            $bis3 = 0;
        }
    ...
    public function updatedetailall($imaterial, $vtoset, $vgelar, $vset, $fbis, $iproductwip, $icolor, $n_bagibis, $bagian, $bis3, $bis4, $id_bisbisan, $f_cutting, $autocutter, $badan, $print, $bordir, $quilting, $id_type_makloon, $f_kain_utama, $f_budgeting, $id_marker, $f_marker_utama)
    {
        $idcompany  = $this->session->userdata('id_company');
        $id_type_makloon = "{" . to_pg_array($id_type_makloon) . "}";
        $query = $this->db->query("SELECT id FROM tr_product_wip WHERE i_product_wip = '$iproductwip' AND id_company = '$idcompany' ", FALSE);
    ...
    public function datawip($iproduct, $icolor, $id_marker)
    {
        $idcompany  = $this->session->userdata('id_company');
        $this->db->select('b.i_product_wip, b.id, e_product_wipname, b.i_color, e_color_name, id_marker, e_marker_name, f_marker_utama')->distinct();
        ...
        $this->db->join('tr_marker d', 'd.id = a.id_marker', 'left');
        /*$this->db->where('a.id', $id);*/
        $this->db->where('b.i_product_wip', $iproduct);
        $this->db->where('b.i_color', $icolor);
        $this->db->where('a.id_marker', $id_marker);
        $this->db->where('b.id_company', $idcompany);
        return $this->db->get();
    }

    public function checkmarkerutama($iproductwip)
    {
        return $this->db->query("SELECT * FROM tr_polacutting_new WHERE id_product_wip = '$iproductwip' AND f_marker_utama IS TRUE");
    }

    public function detail(/*$id, */$iproduct, $icolor, $id_marker)
    {
        $idcompany  = $this->session->userdata('id_company');
        return $this->db->query("SELECT
                a .*,
            ...
            LEFT JOIN (SELECT id, initcap(REPLACE (e_type_makloon_name,'MAKLOON','')) e_type_makloon_name FROM tr_type_makloon j ORDER BY id) j ON (j.id = any(a.id_type_makloon))	
            LEFT JOIN tr_group_barang g ON (g.i_kode_group_barang = b.i_kode_group_barang AND b.id_company = g.id_company)
            WHERE
                i_product_wip = '$iproduct'
                AND i_color = '$icolor'
                AND id_marker = '$id_marker'
                AND a.id_company = '$idcompany'
            GROUP BY
                1,
                e_nama_group_barang,
    ...
    public function delete($iproduct, $icolor, $cek, $id_marker)
    {
        $idcompany  = $this->session->userdata('id_company');
        if ($cek != 'on') {
