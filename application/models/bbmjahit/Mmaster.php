...
class Mmaster extends CI_Model
{
    ...
    public function cek_kode($kode,$ibagian)
    {
        ...
        $this->db->from('tm_masuk_gudang_jadi');
        ...
        return $this->db->query("SELECT
            ...
            UNION ALL
                ...
                WHERE
                    ...
                    /* AND a.id_jenis_barang_keluar <> '3' */ ) AS x
            WHERE 
                id_bagian = '$ipengirim'
            ORDER BY 1
        ", FALSE);
    ...
    public function datareferensi_all($cari,$ipengirim,$ibagian)
    {
        $idcompany = $this->session->userdata('id_company');
    ...
    public function headerreferensi($id,$ipengirim,$ibagian)
    {
        $idcompany = $this->session->userdata('id_company');
        return $this->db->query("SELECT
                *
            ...
            UNION ALL
                ...
                INNER JOIN tr_bagian c ON
                    (c.i_bagian = a.i_bagian
                        AND a.id_company = c.id_company)
                LEFT JOIN tr_jenis_barang_keluar b ON (b.id = a.id_jenis_barang_keluar)
                WHERE
                    a.id_company = '$idcompany'
                    AND i_status = '6'
    ...
    public function headerreferensi_all($id,$ipengirim,$ibagian)
    {
        $idcompany = $this->session->userdata('id_company');
        $sql = "SELECT *
                FROM (
                        SELECT
                            ...
                            '' AS e_jenis_name,
                            a.id_bagian_tujuan AS id_bagian
                        FROM tm_keluar_gudang_jadi a INNER JOIN tr_bagian b ON b.id = a.id_bagian_tujuan
                        WHERE a.id_company = '$idcompany' AND a.i_status = '6'
                        UNION ALL
                        SELECT DISTINCT 
                            a.id,
                            a.id_jenis_barang_keluar, 
                            b.e_jenis_name,
                            c.id AS id_bagian
                        FROM tm_keluar_qc a INNER JOIN tr_bagian c ON c.i_bagian = a.i_bagian AND a.id_company = c.id_company
                        LEFT JOIN tr_jenis_barang_keluar b ON (b.id = a.id_jenis_barang_keluar)
                        WHERE /* a.id_company = '$idcompany' AND */ i_status = '6'
                            AND a.i_tujuan = '$ibagian'
                    ) AS x
    ...
    public function detailreferensi_all($id,$ipengirim,$ibagian)
    {
        $idcompany = $this->session->userdata('id_company');
        ...
        $sql = "SELECT *
                FROM
                    ...
                    WHERE a.id_company = '$idcompany' AND a.i_status = '6' AND c.n_quantity_sisa > 0
                    UNION ALL
                    SELECT
                        a.id,
                        c.id AS id_bagian,
                        ...
                        e_color_name,
                        n_sisa AS n_quantity_sisa
                    FROM tm_keluar_qc a INNER JOIN tm_keluar_qc_item b ON (b.id_keluar_qc = a.id)
                    INNER JOIN tr_bagian c ON (c.i_bagian = a.i_bagian AND a.id_company = c.id_company)
                    INNER JOIN tr_product_base d ON (d.id = b.id_product)
                    INNER JOIN tr_color e ON (e.i_color = d.i_color AND d.id_company = e.id_company)
                    WHERE /* a.id_company = '$idcompany' AND */ i_status = '6' AND b.n_sisa > 0 AND a.i_tujuan = '$ibagian'
                ) AS x
                WHERE id_bagian = '$ipengirim' AND id = '$id'
                ORDER BY 4";
    ...
    public function simpan($id,$idocument,$ddocument,$ibagian,$ipengirim,$ijenis,$ireff,$eremark)
    {
        ...
        $this->db->insert('tm_masuk_gudang_jadi', $data);
    }
    ...
    public function simpandetail($id,$ireff,$idproduct,$nquantity,$nquantityreff,$eremark)
    {
        ...
        $this->db->insert('tm_masuk_gudang_jadi_item', $data);
    }
    ...
    public function dataeditdetail($id)
    {
        return $this->db->query("SELECT
                ...
                a.n_quantity_reff
            FROM
                tm_masuk_gudang_jadi_item a
            INNER JOIN tm_masuk_gudang_jadi b ON
                (b.id = a.id_document)
    ...
    public function update($id,$idocument,$ddocument,$ibagian,$ipengirim,$ijenis,$ireff,$eremark)
    {
        ...
        $this->db->where('id',$id);
        $this->db->update('tm_masuk_gudang_jadi', $data);
    }
    ...
    public function delete($id)
    {
        ...
        $this->db->delete('tm_masuk_gudang_jadi_item');
    }
        ...
        if ($istatus=='3' || $istatus== '6') {
            ...
            if ($istatus == '3') {
            	...
            	if ($awal->i_approve_urutan + 1 > $awal->n_urut ) {
                    // $this->updatesisa($id);
                    $this->db->query("UPDATE tm_keluar_qc_item query
                    SET n_quantity_product = subquery.n_quantity,
                        n_sisa = 0,
                        f_item_complete = 't'
                    FROM (SELECT id_document_reff,id_product_base, n_quantity 
                    FROM tm_masuk_gudang_jadi_item WHERE id_document = '$id') AS subquery
                    WHERE query.id_keluar_qc = subquery.id_document_reff AND subquery.id_product_base = query.id_product");
                    $data = array(
	                    'i_status'  => $istatus,
	                    'i_approve_urutan'  => $awal->i_approve_urutan + 1,
            	...
            	$this->db->query("INSERT INTO tm_menu_approve (i_menu,i_level,i_document,e_approve,d_approve,e_database) VALUES
					('$this->i_menu','$this->i_level','$id','$this->username','$now','tm_masuk_gudang_jadi');", FALSE);
                }
        }else{
            $data = array(
                'i_status'  => $istatus,
