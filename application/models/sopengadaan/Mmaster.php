...
class Mmaster extends CI_Model {
        ...
        $id_company = $this->session->userdata('id_company');
          ...
          $datatables->add('action', function ($data) {
              ...
              if(check_role($i_menu, 2)){
                  $data     .= "<a href=\"#\" title='Detail' onclick='show(\"$folder/cform/view/$id\",\"#main\"); return false;'><i class='ti-eye text-success fa-lg mr-3'></i></a>";
              }
    ...
    public function runningid() {
        ...
        $this->db->from('tm_stockopname_pengadaan');
    ...
    /* 
        return $this->db->query(" SELECT 
            ...
            INNER JOIN 
                tm_stockopname_pengadaan_item aa
                ON
                (a.id = aa.id_document AND a.id_company = aa.id_company)
            INNER JOIN 
                tr_product_wip x
                ON
                (aa.id_product_wip = x.id AND aa.id_company = x.id_company)
            INNER JOIN
                tr_satuan b 
                ON
    ...
    */
        ...
        return $this->db->query(" SELECT 
                ...
                a.i_satuan_code,
                b.e_satuan_name,
                x.n_saldo_akhir as saldo_akhir
                FROM
                   f_mutasi_saldoawal_pengadaan_newbie('$id_company', '$i_periode', '$d_jangka_awal', '$d_jangka_akhir', '$dfrom', '$dto', '$ibagian') x
                INNER JOIN 
                    tr_product_wip a
                    ON
                    (a.id = x.id_product_wip AND a.id_company = $id_company)
                INNER JOIN
                    tr_satuan b 
                    ON
    ...
    public function barang($cari, $ibagian, $ddocument) {
        ...
        */
        ...
        AND (a.i_product_wip ILIKE '%$cari%' OR a.e_product_wipname ILIKE '%$cari%')
        ORDER BY 1 
        ", FALSE); 
    }
    ...
    public function simpan($id,$idcompany,$ibagian,$idocument,$ddocument,$iperiode, $eremarkh){
          ...
          $data = array(
                        ...
                        'e_remark'     => $eremarkh,
                        'd_entry'      => $dentry,
                        'id_jenis_barang_keluar' => 1,
          );
          $this->db->insert('tm_stockopname_pengadaan', $data);
    }
    ...
    public function simpandetail($idcompany, $id, $idmaterial, $qty, $eremark){
          $data = array(
                        'id_company'      => $idcompany,
                        'id_document'     => $id,
                        'id_product_wip'     => $idmaterial,
                        'n_quantity'      => $qty,
                        'e_remark'        => $eremark,
          );
      $this->db->insert('tm_stockopname_pengadaan_item', $data);
    }
    ...
    public function changestatus($id, $istatus){
        ...
        $this->db->where('id', $id);
        $this->db->where('id_company', $this->session->userdata('id_company'));
        $this->db->update('tm_stockopname_pengadaan', $data);
    }
    */
        ...
        if ($istatus=='3' || $istatus== '6') {
            ...
            if ($istatus == '3') {
                ...
                $this->db->query("INSERT INTO tm_menu_approve (i_menu,i_level,i_document,e_approve,d_approve,e_database) VALUES
                    ('$this->i_menu','$this->i_level','$id','$this->username','$now','tm_stockopname_pengadaan');", FALSE);
            }
        } else{
            $data = array(
                'i_status'  => $istatus,
       ...
       /* 
        return $this->db->query("
                                  SELECT
                                     ...
                                     a.e_remark 
                                  FROM
                                     tm_stockopname_pengadaan_item a 
                                     INNER JOIN
                                        tr_material b 
       ...
       */
        ...
        return $this->db->query(" SELECT 
            ...
            INNER JOIN 
                tr_product_wip x
                ON
                (a.id_product_wip = x.id AND a.id_company = x.id_company)
            INNER JOIN
                tr_satuan b 
                ON
                (a.id_company = b.id_company AND x.i_satuan_code = b.i_satuan_code)
    ...
    {
        $this->db->select("a.id, i_product_wip i_product_base, e_product_wipname e_product_basename, e_color_name");
