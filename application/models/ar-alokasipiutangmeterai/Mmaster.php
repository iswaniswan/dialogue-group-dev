    ...
    public function data($folder,$i_menu,$dfrom,$dto)
    {
        ...
        if ($this->departement=='1') {
            ...
            if ($cek->num_rows()>0) {                
                ...
                $bagian = "AND bag.i_bagian = '$i_bagian' ";
            }else{
                $bagian = "AND bag.i_bagian IN ($sql_in_bagian) ";
            }
        }
        ...
        $sql = "SELECT DISTINCT 
                ...
                '$dfrom' AS dfrom,
                '$dto' AS dto 
                FROM tm_alokasimt tam
                INNER JOIN tm_alokasimt_item tami ON tami.i_alokasimt = tam.i_alokasimt
                INNER JOIN tr_status_document tsd ON tsd.i_status = tam.i_status
                inner join tr_bagian bag on bag.id = tam.id_bagian and bag.id_company = tam.id_company 
                LEFT JOIN tr_menu_approve tma ON (tma.n_urut = tam.i_approve_urutan AND tma.i_menu = '$i_menu')
                LEFT JOIN public.tr_level tl ON tl.i_level = tma.i_level
                LEFT JOIN tr_customer tc ON tc.id = tam.id_customer
                LEFT JOIN tr_area ta ON ta.id = tam.id_area
                LEFT JOIN tm_rv tr ON tr.i_rv = tam.i_rv
                WHERE tam.id_company = '$id_company' AND
                    tam.i_status <> '5'AND
                    tam.d_alokasimt BETWEEN to_date('$dfrom','yyyy-mm-dd') AND to_date('$dto','yyyy-mm-dd')
                    $bagian
                ORDER BY tam.i_alokasimt DESC";
    ...
    */
    ...
    public function get_data_referensi($dfrom, $dto, $folder, $id_menu)
    {
        ...
        if ($this->departement=='1') {
            ...
            if ($cek->num_rows()>0) {                
                $i_bagian = $cek->row()->i_bagian;
                $bagian = "AND tr.i_bagian = '$i_bagian' ";
            }else{
                $bagian = "AND tr.i_bagian IN ($sql_in_bagian) ";
            }
        }
        $datatables = new Datatables(new CodeigniterAdapter);
        ...
        $sql = "SELECT 
                    ...
                    tc.e_coa_name,
                    tc2.e_coa_name as e_coa_name_ref,
                    tr.i_rv_id,
                    tri.i_area,
                    ta.e_area,
                ...
                INNER JOIN tr_coa tc2 ON tc2.id = tri.i_coa AND tc2.f_alokasi_meterai = 't'
    ...
    public function insert_alokasimt($i_alokasi_id, $i_rv, $i_rv_item, $d_alokasi, $e_bank_name, $v_jumlah, $v_lebih,
                                        $id_company=null, $id_area, $id_customer, $id_bagian)
    {
        $id_company = $this->session->userdata('id_company');

        $data = [
            'i_alokasimt_id' => $i_alokasi_id,
            'i_rv' => $i_rv,
            'i_rv_item' => $i_rv_item,
            'd_alokasimt' => $d_alokasi,
        ...
        $this->db->insert('tm_alokasimt', $data);
    }

    /** untuk detail update keterangan saja */
    public function update_alokasimt_item($id, $e_remark)
    {
        $data = [
            'e_remark' => $e_remark
        ]; 

        $this->db->where('i_alokasimt_item', $id);
        $this->db->update('tm_alokasimt_item', $data);
    }
    ...
    public function insert_alokasimt_item($i_alokasi, $i_alokasi_item=null, $i_rv_item, $id_nota, $d_nota, 
                                    $v_jumlah, $v_sisa, $n_item_no=null, $e_remark, $id_company=null, $id_area)
    {
        ...
        $this->db->insert('tm_alokasimt_item', $data);
    }
    ...
    public function data_header($id)
    {
        $sql = "SELECT tap.*,
                ...
                FROM tm_alokasimt tap
                INNER JOIN tm_alokasimt_item tapi ON tapi.i_alokasimt = tap.i_alokasimt
    ...
    public function data_detail($id)
    {
        $sql = "SELECT tapi.*,
                tnp.i_document,
                tnp.d_document,
                tnp.v_meterai_sisa AS v_nilai,
                tap.v_lebih,
                tnp.v_meterai_sisa + tapi.v_Jumlah AS v_nota
                FROM tm_alokasimt_item tapi
                INNER JOIN tm_alokasimt tap ON tap.i_alokasimt = tapi.i_alokasimt
                INNER JOIN tm_nota_penjualan tnp ON tnp.id = tapi.id_nota
                WHERE tapi.i_alokasimt = $id";
    ...
    public function update_header($id, $d_alokasi)
    {
        ...
        $this->db->where('i_alokasimt', $id);
        $this->db->update('tm_alokasimt', $data);
    }
    ...
    private function change_status_insert_menu_approve($id, $i_menu, $i_level, $username)
    {
        ...
        $sql = "INSERT INTO tm_menu_approve 
                ...
                VALUES
                    ('$i_menu','$i_level','$id','$username','$now','tm_alokasimt')";
    ...
    private function update_action_approve($id)
    {
        ...
        foreach($all_item->result() as $item) {
            ...
            $this->db->query("UPDATE tm_nota_penjualan SET v_meterai_sisa = v_meterai_sisa - '$v_jumlah', e_remark = '$e_remark' WHERE id = '$id_nota' ", FALSE);
    ...
    public function generate_nomor_dokumen($id_bagian) {
        ...
        $sql = "SELECT count(*) 
                ...
                WHERE tb.id = '$id_bagian' 
                    and i_status <> '5'
                    AND to_char(d_alokasimt, 'yyyy-mm') = to_char(now(), 'yyyy-mm')";
