...
class Mmaster extends CI_Model {
    ...
    public function baca($id){   
        $this->db->select("
            ...
            FROM
                dgu.tm_op
            INNER JOIN dgu.tr_supplier ON
                (tm_op.i_supplier = tr_supplier.i_supplier)
            INNER JOIN dgu.tr_op_status ON
                (tm_op.i_op_status = tr_op_status.i_op_status)
            INNER JOIN dgu.tr_area ON
    ...
    public function bacadetail($id){
        ...
        if(substr($reff,0,3)=='SPB'){
            $this->db->select("a.*, b.e_remark from dgu.tm_op_item a, dgu.tm_spb_item b, dgu.tm_op c 
                where a.i_op='$id '
                and a.i_op=c.i_op
            ...
            $this->db->select("a.*, b.e_remark from dgu.tm_op_item a, dgu.tm_spmb_item b, dgu.tm_op c 
                where a.i_op='$id '
                and a.i_op=c.i_op
