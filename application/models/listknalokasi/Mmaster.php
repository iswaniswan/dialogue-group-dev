...
//    public function data($dfrom, $dto, $iarea, $folder, $iperiode, $title){
    ...
    public function baca($ialokasi,$iarea,$ikn){
        $this->db->select("
                tm_alokasikn.i_alokasi, tm_alokasikn.d_alokasi, tm_alokasikn.i_kn, tm_alokasikn.i_area, tm_alokasikn.v_jumlah, tm_alokasikn.i_customer,
            ...
            INNER JOIN tr_customer ON
                (tm_alokasikn.i_customer = tr_customer.i_customer)
            INNER JOIN tr_area ON
                (tm_alokasikn.i_area = tr_area.i_area)
            INNER JOIN tm_kn ON
                (tm_alokasikn.i_area = tm_kn.i_area and tm_alokasikn.i_kn = tm_kn.i_kn)
            WHERE
                tm_alokasikn.i_alokasi = '$ialokasi'
                AND tm_alokasikn.i_area = '$iarea'
                AND tm_alokasikn.i_kn = '$ikn'"
        , false);
        $query = $this->db->get();
        if ($query->num_rows() > 0){
            return $query->row();
