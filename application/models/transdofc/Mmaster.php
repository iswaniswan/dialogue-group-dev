...
class Mmaster extends CI_Model {
    ...
    function updateopdetail($iop,$iproduct,$iproductgrade,$iproductmotif,$ndeliver){
      $this->db->query("update tm_opfc_item set n_delivery=n_delivery+$ndeliver where i_op='$iop' and i_product='$iproduct' and   
    ...
    function updatespbdetail($iop,$iproduct,$iproductgrade,$iproductmotif,$ndeliver){
		  $this->db->select(" i_reff, i_area from tm_opfc
	...
	function updatesaldofc($iproduct,$jumlah,$emutasiperiode){
    	$this->db->select("  e_periode,i_product from tm_saldoawal_fc where i_product='$iproduct' and e_periode='$emutasiperiode'", false);
     	$query = $this->db->get();
     	foreach($query->result() as $row){
     	   $prod=$row->i_product;
     	}
     	if($prod){
    		$this->db->query("update tm_saldoawal_fc set n_sisa=n_sisa-$jumlah
                                 where i_product='$iproduct' and e_periode='$emutasiperiode'");              
     	}
	}
    ...
    function insertdetail($iop,$ido,$iproduct,$iproductgrade,$eproductname,$jumlah,
	...
	function updateopitem($iop,$iproduct,$iproductgrade,$iproductmotif,$jumlah,$ido){
		...
		$query=$this->db->query("select * from tm_opfc_item 
								 where i_op='$iop' 
	...
	function cekadaop($iop,$iproduct,$iproductmotif,$iproductgrade,$iarea){
		$que=$this->db->query("select * from tm_opfc where i_op like '%$iop%' and i_area='$iarea'", false);
		if($que->num_rows()>0){
			foreach($que->result() as $tmp){
				$query=$this->db->query("select i_op from tm_opfc_item 
	...
	function cekdataitem($ido,$isupplier,$iproduct,$iproductmotif,$iproductgrade,$thbl){
		$query=$this->db->query("select i_do from tm_dofc_item 
	...
	function cekdata($ido,$isupplier,$thbl){
		$query=$this->db->query("select i_do from tm_dofc where i_do='$ido' and i_supplier='$isupplier'", false);
		if($query->num_rows()>0){
			return $query->num_rows();
		}else{
			$ido='DO-'.$thbl.'-DT'.substr($ido,2,4);
    ...
    function inserttmp($ido,$iproduct,$iproductgrade,$eproductname,$jumlah,
		...
		$this->db->select(" n_delivery from tt_dofc where i_do='$ido' and i_product='$iproduct'
							and i_product_grade='$iproductgrade' and i_product_motif='00' 
							and i_area='$iarea' and i_op='$iop'", false);
		$query = $this->db->get();
		if ($query->num_rows() > 0){
			$this->db->query("	update tt_dofc set n_delivery=n_delivery+$jumlah
	...
	function cekono($iop,$iarea){
		$query= $this->db->query("select i_op from tm_opfc where i_op_old like '%$iop%' and i_area='$iarea'");
		if($query->num_rows()>0){
			foreach($query->result() as $row){
			  $no=$row->i_op;
			}
