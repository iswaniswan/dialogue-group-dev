...
class Mmaster extends CI_Model
{
    ...
    function data($i_menu,$folder,$dfrom,$dto)
    {
        ...
        $datatables->query("   
            ...
            LEFT JOIN tr_bagian h ON
                (h.id = a.id_partner)
            LEFT JOIN tm_konversi_pinjaman_bp i ON
                (i.id = a.id_document_reff)
            LEFT JOIN tm_memo_bp j ON
    ...
    public function partner($cari)
    {
        return $this->db->query("
            ...
            FROM
                ...
                FROM
                    tm_memo_bp a
                INNER JOIN tm_memo_bp_item b ON
                    (b.id_document = a.id)
                LEFT JOIN tr_supplier e ON
                    (e.id = a.id_partner)
                LEFT JOIN tr_customer f ON
            ...
            UNION ALL
                ...
                FROM
                    tm_konversi_pinjaman_bp a
                INNER JOIN tm_konversi_pinjaman_bp_item b ON
                    (b.id_document = a.id)
                LEFT JOIN tr_supplier e ON
                    (e.id = a.id_partner)
                LEFT JOIN tr_customer f ON
    ...
    public function simpan($id,$idocument,$ddocument,$ibagian,$ipartner,$etypepartner,$ireff,$etypereff,$eremark)
    {
        ...
        $this->db->insert('tm_penjualan_bp', $data);
    }
    ...
    public function simpandetail($id,$idreff,$idproduct,$nquantity,$eremark)
    {
        ...
        $this->db->insert('tm_penjualan_bp_item', $data);
    }
    ...
    public function dataeditdetail($id)
    {
        return $this->db->query("
            ...
            FROM
                tm_penjualan_bp_item a
            INNER JOIN tm_penjualan_bp b ON
                (b.id = a.id_document)
            INNER JOIN tr_material c ON
                (c.id = a.id_material)
            INNER JOIN tr_satuan d ON
                (d.i_satuan_code = c.i_satuan_code
                AND c.id_company = d.id_company)
            LEFT JOIN tm_memo_bp_item e ON
                (e.id_document = a.id_document_reff
                AND a.id_material = e.id_material)
            LEFT JOIN tm_konversi_pinjaman_bp_item f ON
    ...
    public function update($id,$idocument,$ddocument,$ibagian,$ipartner,$etypepartner,$ireff,$etypereff,$eremark)
    {
        ...
        $this->db->where('id',$id);
        $this->db->update('tm_penjualan_bp', $data);
    }
    ...
    public function delete($id)
    {
        ...
        $this->db->delete('tm_penjualan_bp_item');
    }
    ...
    public function simpanjurnal($id,$title)
    {
        $this->db->query("
            ...
            SELECT
                ...
                '110-81000' AS i_coa,
                'PENJUALAN BAHAN PEMBANTU' AS e_coa,
            ...
            FROM
                tm_penjualan_bp a
            INNER JOIN tm_penjualan_bp_item b ON
                (b.id_document = a.id)
            WHERE
                a.id = $id
        ", FALSE);
