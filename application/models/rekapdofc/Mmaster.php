...
class Mmaster extends CI_Model {
	function data($dfrom,$dto,$isupplier,$total){
		...
		$tahun = date('Y');
    	if($isupplier == 'AS'){
			$datatables->query("sselect row_number() over(order by idtap) as i,idtap, ddtap, cancel, spb, spbold, dtapc, supplier, area, spmb, ddo, ido, iop, istore, istoreloc, dspmb, spmbold, total
	...
	public function total(){
		return $this->db->query("select distinct f.i_dtap, f.d_dtap, a.f_do_cancel, a.i_do, a.d_do, a.i_op, a.i_area, c.i_spb, c.i_spb_old, d.i_spmb, d.i_spmb_old, 
								e.e_supplier_name, g.f_dtap_cancel, a.i_supplier, d.i_store, d.i_store_location, d.d_spmb from tr_supplier e,
  								...
  								left join tm_dtap g on (f.i_dtap=g.i_dtap and f.i_area=g.i_area and f.i_supplier=g.i_supplier 
  								and g.f_dtap_cancel='f')
  								where a.i_op=b.i_op and a.i_area=b.i_area and a.i_supplier=e.i_supplier
  								and a.i_area <> '00'
  								and not d.i_spmb isnull
   								and a.i_do not in(select substr(e_remark, 12, 15) from tm_sjp_item where e_remark like '%Dari DO FC%' )
