...
//    public function data($dfrom, $dto, $isupplier, $folder, $iperiode, $title){
    public function data($dfrom, $dto, $iarea, $folder, $iperiode, $title){
        ...
        $datatables->query("
            SELECT	distinct on (a.d_tunai, a.i_customer, a.i_tunai)  a.i_area as iarea, a.i_tunai as itunai, a.d_tunai as dtunai, '('||a.i_customer||') '||c.e_customer_name as icustomer, 
                  a.v_jumlah as vjumlah, a.e_remark as eremark
                  , '$folder' AS folder, '$dfrom' AS dfrom,
                  ...
                  left join tm_rtunai_item f on(a.i_tunai=f.i_tunai and a.i_area=f.i_area_tunai)
					        where a.f_tunai_cancel='f' and a.i_area='$iarea'
                  and a.d_tunai >= '$dfrom' and a.d_tunai <= '$dto'
                  ORDER BY a.d_tunai, a.i_customer, a.i_tunai ",false);
        ...
        $datatables->add('action', function ($data) {
            ...
            $title      = $data['title'];
            $data       = '';
            $data      .= "&nbsp;&nbsp;<a href=\"#\" title=\"Detail Nota\" onclick='window.open(\"$folder/cform/detail/$itunai/$iarea\",\"#main\"); return false;'><i class='fa fa-pencil'></i></a>";
            return $data;
        });
