    ...
    */
    ...
    public function data($folder, $i_menu, $dfrom, $dto)
    {
        ...
        $datatables->query("SELECT
            ...
            INNER JOIN tr_bagian b ON 
                (b.i_bagian = a.i_bagian 
                    AND a.id_company_bagian = b.id_company)
            ...
            left join public.company m on (m.id = a.id_company_bagian)
            WHERE
                a.i_status <> '5'
        ...
        $datatables->add('action', function ($data) {
            ...
            $e_jenis_bagian = ($data['e_jenis_bagian']) ? $data['e_jenis_bagian'] : 'null';
            ...
            if (check_role($i_menu, 2)) {
                $data .= "<a href=\"#\" title='Detail' onclick='show(\"$folder/cform/view/$id/$dfrom/$dto/$e_jenis_bagian/$id_referensi\",\"#main\"); return false;'><i class='ti-eye text-success fa-lg mr-3'></i></a>";
            }

            if (check_role($i_menu, 3)) {
                if ($i_status == '1' || $i_status == '2' || $i_status == '3' || $i_status == '7') {
                    $data .= "<a href=\"#\" title='Edit' onclick='show(\"$folder/cform/edit/$id/$dfrom/$dto/$e_jenis_bagian/$id_referensi\",\"#main\"); return false;'><i class='ti-pencil-alt fa-lg mr-3'></i></a>";
                }
            }

            if (check_role($i_menu, 7) && ($i_status == '2')) {
                if (($i_level == $this->session->userdata('i_level')) || $this->session->userdata('i_level') == 1) {
                    $data .= "<a href=\"#\" title='Approve' onclick='show(\"$folder/cform/approval/$id/$dfrom/$dto/$e_jenis_bagian/$id_referensi\",\"#main\"); return false;'><i class='ti-check-box text-primary fa-lg mr-3'></i></a>";
                }
            }
    ...
    public function get_data_referensi()
    {
        ...
        return $this->db->query("SELECT DISTINCT 
            ...
            INNER JOIN tm_fcjahit_item b ON (b.id_forecast = a.id)
            LEFT JOIN (
                SELECT DISTINCT
                    id_referensi, b.id_product_wip, sum(b.n_quantity) n_quantity
                FROM tm_uraianjahit a, tm_uraianjahit_item b
                WHERE a.id = b.id_document AND i_status IN ('1', '2', '3', '6')
                AND b.n_quantity > 0
                GROUP BY 1,2
            ) d ON (d.id_referensi = a.id AND b.id_product_wip = d.id_product_wip)
            WHERE
                a.i_status = '6'
                AND a.id_company = '$this->id_company'
                AND (b.n_quantity - COALESCE (d.n_quantity,0)) > 0");
    }
    ...
    public function datadetail_header($id)
    {
        return $this->db->query("SELECT
            a.i_document,
            b.e_bagian_name,
            to_char(a.d_document, 'DD MONTH YYYY') AS tgl_dokumen,
            c.i_document as no_dokumen_ref,
            to_char(c.d_document, 'MONTH YYYY') AS tgl_dokumen_ref,
            d.e_status_name, a.e_remark
        FROM
            tm_uraianjahit a
        INNER JOIN tr_bagian b ON
            (b.i_bagian = a.i_bagian
                AND a.id_company_bagian = b.id_company)
        INNER JOIN tm_fcjahit c ON
            (c.id = a.id_referensi
    ...
    public function datadetail_edit($id)
    {
        return $this->db->query("SELECT
            ...
            b.i_periode,
            c.i_product_wip,
            upper(c.e_product_wipname) AS e_product_name,
            a.n_quantity as quantity_urai,
            g.n_fc as quantity_forecast,
            a.e_remark,
            a.n_quantity_sisa,
            trim(d.e_color_name) AS e_color_name,
            e.e_style_name,
            f.e_brand_name
        FROM
        ...
        INNER JOIN tr_style e ON
            (e.i_style = c.i_style
                AND c.id_company = e.id_company)
        INNER JOIN tr_brand f ON
            (f.i_brand = c.i_brand
                AND f.id_company = c.id_company)
        INNER JOIN tm_fcjahit_item g ON
            (g.id_forecast = b.id_referensi
                AND g.id_company = b.id_company
                AND g.id_product_wip  = a.id_product_wip)
        WHERE a.id_document = '$id'
        ORDER BY a.id_product_wip ASC, 
                c.e_product_wipname ASC,
                d.e_color_name ASC;");
    }
    ...
    public function get_detail_referensi($id,$ibagian)
    {
        ...
        return $this->db->query("SELECT
                ...
                a.n_quantity - COALESCE(f.n_quantity, 0) AS n_quantity_sisa
            FROM
                tm_fcjahit_item a
            ...
            WHERE
                a.id_forecast = '$id'
                AND (a.n_quantity - COALESCE(f.n_quantity, 0)) > 0
            ORDER BY
                e_type_name,
                d.i_product_wip,
                e_product_name,
    ...
    public function bagian($cari)
    {
        ...
        select * FROM (
            SELECT DISTINCT a.id, a.i_bagian, e_bagian_name, d.i_departement, e.id as id_company, e.name AS company_name FROM tr_bagian a 
            INNER JOIN tr_departement_cover b ON b.i_bagian = a.i_bagian AND a.id_company = b.id_company 
            left join tr_type c on (a.i_type = c.i_type)
            left join public.tm_menu d on (d.i_menu = '$this->i_menu' and c.i_departement  = d.i_departement)
            LEFT JOIN public.company e ON (e.id = a.id_company)
            ...
            FROM
                tr_bagian f
            LEFT JOIN tr_type h on (f.i_type = h.i_type)
            LEFT JOIN public.tm_menu i on (i.i_menu = '$this->i_menu' AND h.i_departement = i.i_departement)
            LEFT JOIN public.company j on (j.id = f.id_company)
            WHERE /*f.id_company = '21' and*/ f.f_status = 't' AND f.i_type = '10' and (select array_agg(id) from tr_type_makloon where e_type_makloon_name ilike '%MAKLOON JAHIT%') @> f.id_type_makloon
        ) as x
        where e_bagian_name ilike '%$cari%' OR company_name ilike '%$cari%'
    ...
    public function insertheader($id,$idocument,$ddocument,$ibagian,$idreferensi,$iperiode,$eremark,$idcompanybagian)
    {
        $data = array(
            'id'               => $id,
            ...
            'i_bagian'         => $ibagian,
            'id_referensi'     => $idreferensi,
            'i_periode'        => $this->db->query("SELECT i_periode FROM tm_fcjahit WHERE id = '$idreferensi'")->row()->i_periode,
            'e_remark'         => $eremark,
            'i_status'         => 1,
            'id_company_bagian'=> $idcompanybagian
        );
    ...
    public function editheader($id)
    {
        return $this->db->query("SELECT
            ...
            FROM
                tm_uraianjahit a
            INNER JOIN tr_bagian b ON (b.i_bagian = a.i_bagian AND a.id_company_bagian = b.id_company)
            LEFT JOIN tm_fcjahit c ON (c.id = a.id_referensi)
            WHERE
                a.id = '$id'
        ", FALSE);
    ...
    {        
        return $this->db->query("SELECT
                ...
                END,
                0)  */
                c.n_quantity n_quantity_fc,
                COALESCE(f.n_quantity, 0) n_quantity_uraian,
                initcap(trim(h.e_color_name)) AS e_color_name,
                regexp_replace(regexp_replace(t.e_type_name, '[^\w]+', ''),' ','') grup,	
                /* COALESCE (CASE
                ...
                END,
                0)  */
                COALESCE(c.n_quantity, 0) - COALESCE(f.n_quantity, 0) + a.n_quantity n_quantity_sisa
            FROM
                tm_uraianjahit_item a
            INNER JOIN tr_product_wip d ON
                (d.id = a.id_product_wip
    ...
    public function edititem_approve($id,$e_jenis, $id_referensi)
    {        
        return $this->db->query("SELECT
                DISTINCT d.id AS id_product_wip,
                d.i_product_wip,
