...
class Mmaster extends CI_Model
{
    ...
    public function getpartner($cari, $i_menu) {
        return $this->db->query("
                                ...
                                WHERE
                                	...
                                	AND a.id_company = '".$this->session->userdata('id_company')."' 
                                    AND a.i_status = '6'
                                	AND a.id NOT IN (SELECT id_referensi FROM tm_notamakloonpacking WHERE id_company = '".$this->session->userdata('id_company')."')
                                ORDER BY
                                	b.e_supplier_name   
                                ", FALSE);
    }
        ...
        return $this->db->query("
                                ...
                                FROM
                                	...
                                	LEFT JOIN 
                                		...
                                		AND a.id_company = b.id_company)
                                	LEFT JOIN 
                                		tm_keluar_makloon_packing c
                                		ON (b.id_document_reff = c.id 
                                		AND b.id_company = c.id_company)
                                WHERE 
                                    a.i_status = '6'
                                    AND a.id_supplier = '$idpartner'
                                    AND c.id NOT IN (SELECT id_referensi FROM tm_notamakloonpacking WHERE i_status <> '5' AND id_company = '".$this->session->userdata('id_company')."')
                                    AND a.id_company = '".$this->session->userdata('id_company')."'
                                ", FALSE);
    }
    ...
    public function getdetailref($id, $idpartner, $i_menu){
        return $this->db->query("
                                SELECT DISTINCT
                                	...
                                	a.id_product as produk_masuk,
                                	b.id_product_base as produk_keluar,
                                    c.i_product_base as i_product_wip,
                                    c.e_product_basename as e_product_wipname,
                                	a.n_quantity,
                                	a.n_quantity_sisa,
                                    b.v_price,
                                    d.i_status,
                                ...
                                FROM
                                	...
                                	LEFT JOIN 
                                		...
                                		AND a.id_product = b.id_product_base
                                		AND a.id_company = b.id_company)
                                	INNER JOIN 
                                		tr_product_base c
                                        ON (a.id_product = c.id
                                        AND b.id_product_base = c.id
                                		AND a.id_company = c.id_company)
                                	LEFT JOIN 
                                ...
                                WHERE
                                	d.id_company='".$this->session->userdata('id_company')."'
                                	AND d.i_status = '6'
                                	AND a.id_document_reff = '$id'
                                	AND e.id_type_makloon IN (SELECT id_makloon FROM tr_makloon_menu WHERE i_menu = '$i_menu' AND id_company='".$this->session->userdata('id_company')."')
    ...
    public function cekkode($kode,$ibagian)
    {
        ...
        $this->db->from('tm_notamakloonpacking');
    ...
    public function insertheader($id, $ibagian, $inota, $dnota, $dreceivefaktur, $ipartner, $ireferensi, $dreferensi, $ifaktursup, $dfaktursup, $ipajak, 
        ...
        $this->db->insert('tm_notamakloonpacking', $data);
    }
    ...
    public function insertdetail($id, $idreffmasuk, $idwip, $nquantity, $harga, $hargatotal, $edesc) {
        ...
        $this->db->insert('tm_notamakloonpacking_item', $data);
    }
    ...
    public function send($kode){
        ...
        $this->db->update('tm_notamakloonpacking', $data);
    }
    ...
    public function bacadetail($id){
        return $this->db->query("
                                SELECT 
                                    a.*,
                                    a.id_product_base as id_product_wip,
                                    c.i_document,
                                    d.i_product_base as i_product_wip,
                                    d.e_product_basename as e_product_wipname,
                                    d.i_color,
                                	e.e_color_name 
                                FROM
                                	tm_notamakloonpacking_item a
                                	LEFT JOIN 
                                		tm_notamakloonpacking b 
                                		ON (a.id_document = b.id
                                		AND a.id_company = b.id_company)
                                	LEFT JOIN 
                                		tm_masuk_makloon_packing c
                                		ON (a.id_referensi_item = c.id
                                		AND a.id_company = c.id_company)
                                	INNER JOIN 
                                		tr_product_base d
                                		ON (a.id_product_base = d.id
                                		AND a.id_company = d.id_company)
                                	INNER JOIN 
                                		tr_color e 
    ...
    public function updatedetail($id, $idreffmasuk, $idwip, $nquantity, $harga, $hargatotal, $edesc) {
        ...
        $this->db->where('id_product_base', $idwip);
        $this->db->where('id_company', $this->session->userdata('id_company'));
        $this->db->update('tm_notamakloonpacking_item', $data);
    }
