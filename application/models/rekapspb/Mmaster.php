...
class Mmaster extends CI_Model {
    ...
    public function total($username, $idcompany, $status){
        if($status=='1'){
            $stokdaerah='f';
        }else{
            $stokdaerah='t';
        ...
        return $this->db->query("       
            ...
            ORDER BY
                a.i_spb) AS i,
                a.i_spb,
                TO_CHAR(d_spb, 'dd-mm-YYYY') AS d_spb,
            ...
            LEFT JOIN tm_dkb_item h ON
                (h.i_sj = a.i_sj
                AND a.i_area = h.i_area)
            INNER JOIN tr_product_group g ON
                (a.i_product_group = g.i_product_group) ,
                tr_area c
            WHERE
                a.i_store ISNULL
                AND a.i_area = c.i_area
                AND a.f_spb_cancel = 'f'
                AND a.f_spb_stockdaerah = '$stokdaerah'
                AND a.i_area IN (
                SELECT
                    i_area
                FROM
    ...
    public function data($folder, $total, $username, $idcompany, $status){
        if($status=='1'){
            $stokdaerah='f';
        }else{
            $stokdaerah='t';
                ...
                if($query->num_rows()>0){
                    ...
                    $this->db->query("
                        UPDATE
                            tm_spmb_item
                        SET
                            n_order = n_order + $row->n_order,
                            n_acc = n_acc + $row->n_order,
                            n_saldo = n_saldo + $row->n_order,
