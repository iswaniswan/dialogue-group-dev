...
class Cform extends CI_Controller {
    ...
    public function refeksternal(){
        ...
        if($this->input->get('ipartner') != '' && $this->input->get('itype') != ''){
            ...
            if ($data->num_rows()>0) {
             ...
             $filter[] = array(
                'id'   => null,
                'text' => "Partner & Tipe Makloon Belum dipilih"
            );
        }
            echo json_encode($filter);
    }
    ...
    public function simpan()
    {
        ...
        $dsup       = $this->input->post('dsupplier', TRUE);
        ...
        if ($dsup != '') {
            $dsup = date('Y-m-d', strtotime($dsup));
        }
        ...
        if ($idocument!='' && $jml>0) {
            ...
            $this->mmaster->simpan($id,$idocument,$ddocument,$ibagian,$itype,$ipartner,$idocumentsup,$eremarkh,$dsup);
            ...
            for ($i = 1; $i <= $jml; $i++) {
                ...
                if ($nquantity > 0 && $nquantity != null) {
                    $this->mmaster->simpandetail($id, $id_document_reff, $idpanel, $nquantity, $eremark, $idmarker);
                }
    ...
    public function update(){
        ...
        if ($idocument!='' && $jml>0) {
            $this->db->trans_begin();
            $this->mmaster->update($id,$idocument,$ddocument,$ibagian,$itype,$ipartner,$idocumentsup,$eremarkh,$dsup);
