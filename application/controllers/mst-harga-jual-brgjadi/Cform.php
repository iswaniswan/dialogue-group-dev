...
class Cform extends CI_Controller
{
    ...
    public function getproduct()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $spb = $query->result();
            foreach ($spb as $row) {
                $c .= "<option value=" . $row->id_product . " >" . $row->i_product_base . " - " . $row->e_product_basename . " - " . $row->e_color_name . "</option>";
            }
    ...
    public function proses()
    {
        ...
        $data = array(
            ...
            'iproduct'      => $iproduct,
            'proses'        => $this->mmaster->getinput($ikodekelompok, $ikodejenis, $iproduct, $idcompany),
            'kodeharga'     => $this->mmaster->getkodeharga($idcompany),
            'grade'         => $this->db->get_where('tr_jenis_barang_keluar', ['id <> ' => '2']),
        );
        ...
        if ($ikodekelompok != '') {
            for ($i = 1; $i <= $jml; $i++) {
                if ($this->input->post('cek' . $i) == 'cek') {
                    ...
                    $id_jenis_barang_keluar = $this->input->post('id_jenis_barang_keluar' . $i, TRUE);
                    ...
                    $this->mmaster->insert($kodebrg, $harga, $dateberlaku, $ikodeharga, $id_jenis_barang_keluar);
                }
            }
            $data = array(
                'sukses'    => true,
    ...
    public function edit()
    {
        ...
        $data = array(
            ...
            'kodeharga'     => $this->mmaster->getkodeharga($idcompany),
            'jenisbarang'   => $this->db->get_where('tr_jenis_barang_keluar', ['id <> ' => '2']),
        );
    ...
    public function ubahtanggalberlaku()
    {
        ...
        $this->db->trans_begin();
        $data =  $this->mmaster->update($id, $kodebrg, $ikodeharga, $harga, $dateberlaku, $idcompany, $id_jenis_barang_keluar);
        if ($this->db->trans_status() === false) {
            $this->db->trans_rollback();
        } else {
            $this->db->trans_commit();
    ...
    public function inserttanggalberlaku()
    {
        ...
        $this->db->trans_begin();
        $data =  $this->mmaster->updatetglakhir($id, $kodebrg, $ikodeharga, $harga, $dateberlaku, $dateberlakusebelum, $idcompany, $id_jenis_barang_keluar);
        if ($this->db->trans_status() === false) {
            $this->db->trans_rollback();
        } else {
            $this->db->trans_commit();
