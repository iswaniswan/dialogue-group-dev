...
class Cform extends CI_Controller {
    ...
    public function partner(){
        ...
        if ($data->num_rows()>0) {
            ...
            $filter[] = array(
                'id'   => null,
                'text' => "Tidak Ada Data Partner di Memo"
            );
        }
    ...
    public function referensi(){
        $filter = [];
        $partner     = $this->input->get('ipartner', TRUE);        
        if ($partner != '' || $partner != null) {
            $data   = $this->mmaster->referensi(str_replace("'","",$this->input->get('q')), $partner);            
            if ($data->num_rows()>0) {
                foreach($data->result() as $key){
                    $filter[] = array(
                        'id'   => $key->id,  
            ...
            $filter[] = array(
                'id'   => null,
                'text' => "Pilih Partner Terlebih Dahulu!"
            );   
        }
        echo json_encode($filter);
    }
    ...
    public function simpan(){
        ...
        if ($ikeluar!='' && $jml>0) {        
            $this->db->trans_begin();
            $this->mmaster->insertheader($id, $ibagian, $ikeluar, $datekeluar, $imemo, $partner, $eremark);
    ...
    public function update(){
        ...
        $this->db->trans_begin();
        $this->mmaster->updateheader($id, $ibagian, $ikeluar, $datekeluar, $imemo,  $partner, $eremark);
