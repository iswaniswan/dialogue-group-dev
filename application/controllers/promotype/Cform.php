...
class Cform extends CI_Controller {
    ...
    public function simpan(){
        ...
        if ($ipromotype != '' && $epromotypename != ''){
                $cekada = $this->mmaster->cek_data($ipromotype);
                if($cekada->num_rows() > 0){
                    $data = array(
                        'sukses' => false
                    );
                }else{
                    $this->Logger->write('Simpan Data '.$this->global['title'].' Kode : '.$ipromotype);
    ...
    public function edit(){
        ...
        $data = array(
            ...
            'title_list' => 'List '.$this->global['title'],
            'data' => $this->mmaster->cek_data($ipromotype)->row()
        );
    ...
    public function update(){
        ...
        if ($ipromotype != '' && $epromotypename != ''){
            $cekada = $this->mmaster->cek_data($ipromotype);
            if($cekada->num_rows() > 0){ 
                $this->Logger->write('Update Data '.$this->global['title'].' Kode : '.$ipromotype);
