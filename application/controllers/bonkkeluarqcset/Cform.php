...
class Cform extends CI_Controller
{
    ...
    public function detailproduct()
    {
        ...
        $query  = array(
            'detail' => $this->mmaster->detailproduct($this->input->post('id', TRUE), $this->input->post('i_bagian', TRUE), $this->input->post('id_marker', TRUE))->result_array()
        );
                ...
                if ($idproduct != "" || $idproduct != NULL) {
                    foreach ($this->input->post("idproductwip[]", TRUE) as $idproductwip) {
                        if ($idproduct == $idproductwip) {
                            ...
                            $npenyusun      = str_replace(",", ".", $this->input->post("npenyusun[]", TRUE)[$i]);
                            $nqtysisa       = str_replace(",", ".", $this->input->post("nqtysisa[]", TRUE)[$i]);
                            $idpanel        = str_replace(",", ".", $this->input->post("idpanel[]", TRUE)[$i]);
                            $edesc          = $this->input->post("eremark[]", TRUE)[$i];
                            if ($nqtysisa > 0) {
                                $this->mmaster->insertdetail($id, $idproductwip, $idmarker, $idpanel, $nquantitywip, $npenyusun, $nqtysisa, $edesc);
                            }
    ...
    public function cetak()
    {
        ...
        $no_urut = $this->generate_nomor_urut_cetak($_data->i_keluar_qcset, $ibagian);
        ...
        $data = [            
            ...
            'dto'           => $dto,
            'bagian'        => $this->mmaster->bagian()->result(),
            'data'          => $this->mmaster->dataedit_print($id)->row(),
