...
class Cform extends CI_Controller {
    ...
    public function tambah()
    {
        ...
        $data = array(
            ...
            'jenis'         => $this->mmaster->jenis()->result(),
            'number'        => "KNA-".date('ym')."-123456"            
        );
    ...
    {
        $filter = [];
        if ($this->input->get('ijenis')!='' || $this->input->get('ijenis')!=null) {
            $data   = $this->mmaster->referensi(str_replace("'", "", $this->input->get('q')),$this->input->get('ijenis'));
            if ($data->num_rows()>0) {
                foreach ($data->result() as $row) {
                    $filter[] = array(
                        'id'    => $row->id,
    ...
    public function getreferensi()
    {
        ...
        $data = array(
            'data' => $this->mmaster->getreferensi($this->input->post('ireferensi'),$this->input->post('ijenis'))->result_array(),
        );
    ...
    public function simpan()
    {
        ...
        if($ibagian != ''  && $ijenis != ''  && $ireferensi != ''  && $idocument != '' && $ddocument != '' && $ikasbank != ''){
            ...
            if($cekkode->num_rows()>0){
                ...
                $this->mmaster->insertheader($id,$idocument,$datedocument,$ibagian,$idkasbank,$ibank,$ijenis,$ireferensi,$vnilai,$eremark);
                for($i = 0; $i < $jml; $i++){
                    $idreferensi    = $this->input->post('idreferensi'.$i, TRUE);
                    $vvalue         = str_replace(',','',$this->input->post('v_nilai'.$i,TRUE));
                    $edesc          = $this->input->post('edesc'.$i, TRUE);
                    if ($vvalue>0 && $idreferensi!='') {
                        $this->mmaster->insertdetail($id,$idreferensi,$vvalue,$edesc);
                    }
                }
    ...
    public function update()
    {
        ...
        if($id != ''  && $ibagian != ''  && $ijenis != ''  && $ireferensi != ''  && $idocument != '' && $ddocument != '' && $ikasbank != ''){
            ...
            if($cekkode->num_rows()>0){
                ...
                $this->db->trans_begin();
                $this->mmaster->updateheader($id,$idocument,$datedocument,$ibagian,$idkasbank,$ibank,$ijenis,$ireferensi,$vnilai,$eremark);
