...
class Cform extends CI_Controller
{
    ...
    public function simpan()
    {
        ...
        $i_kum_id = $this->input->post('i_kum_id');
        $d_kum = $this->input->post('d_kum');
        if (strlen($d_kum) > 0) {
            $d_kum = formatYmd($d_kum);
        } else {
            $d_kum = date('Y-m-d');
        }
        ...
        if ($i_bagian != '' && $d_kum != '' && $i_customer != '' && $i_area != '' && $v_jumlah > 0) {
            $this->db->trans_begin();
            $id = $this->mmaster->runningid();
            $this->mmaster->create($id, $i_kum_id, $i_bagian, $i_area, $i_customer, $i_salesman, $i_bank, $i_dt, $d_kum, $v_jumlah, $e_remark, $e_send_name);
    ...
    public function update()
    {
        ...
        if ($i_bagian != '' && $d_kum != '' && $i_customer != '' && $i_area != '' && $v_jumlah > 0) {
            $this->db->trans_begin();
            $this->mmaster->update($id, $i_kum_id, $i_bagian, $i_area, $i_customer, $i_salesman, $i_bank, $i_dt, $d_kum, $v_jumlah, $e_remark, $e_send_name);
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                $data = array(
                    'sukses' => false,
