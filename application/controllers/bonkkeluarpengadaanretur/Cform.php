...
class Cform extends CI_Controller {
    ...
    public function simpan()
    {
        ...
        if ($ibonk!='' && $datebonk!='' && $ibagian != '' && $jml>0) {
            ...
            $this->mmaster->insertheader($id, $ibonk, $ibagian, $datebonk, $itujuan, $eremarkh);

            for ($x=0; $x <= $jml ; $x++) { 
                $idproduct = $this->input->post('idproduct'.$x, TRUE);
                $nquantitywip = str_replace(",",".",$this->input->post('nquantity'.$x, TRUE));
                $eremarkwip   = $this->input->post("eremark".$x, TRUE);

                if ($nquantitywip>0 && ($idproduct!=null || $idproduct!='')) {
                    $this->mmaster->insertdetail($id, $idproduct, $nquantitywip, $eremarkwip);
                }
                // $i  = 0;
                ...
                //         if ($idproduct == $idproductwip) {
                            ...
                            // $nquantitymat   = str_replace(",",".",$this->input->post("nquantity".$x, TRUE));
                            // $edesc     = $this->input->post("eremark".$x, TRUE);
                            // $nquantitymat   = str_replace(",",".",$this->input->post("nquantity${i}", TRUE));
                            // $edesc     = $this->input->post("eremark${i}", TRUE);
    ...
    public function update() {
        ...
        if ($ibonk!='' && $datebonk!='' && $ibagian != '' && $jml>0) {
            ...
            if ($cekdata->num_rows()==0) {
                ...
                $this->db->trans_begin();
                $this->mmaster->updateheader($id, $ibonk, $datebonk, $ibagian, $itujuan, $eremarkh);
