...
class Cform extends CI_Controller {
    ...
    public function barangskin() {
        $filter = [];
        $data = $this->mmaster->barangskin(str_replace("'","",$this->input->get('q')) );
        if ($data->num_rows()>0) {
            foreach ($data->result() as $row) {
                $filter[] = array(
                    'id'   => $row->id,
    ...
    public function simpan(){
        ...
        for ($i = 1; $i <= $jml; $i++) {
            ...
            $skinm      = $this->input->post('skin' . $i, TRUE);
            $statusbarang   = $this->input->post('statusbarang' . $i, TRUE);

            // if ($idproduct[0]!=null || $idproduct[0]!='' || $skinm!=null || $skinm!='') {
            //     $this->mmaster->simpandetail($idcompany, $id, $idproduct[0], $idcolor, $qty, $eremark, $skinm, $statusbarang);
            // }
            if ($idproduct[0]!=null || $skinm!='') {
                $this->mmaster->simpandetail($idcompany, $id, $idproduct[0], $idcolor, $qty, $eremark, $skinm, $statusbarang);
            }
        }
    ...
    public function update(){
        ...
        for ($i = 1; $i <= $jml; $i++) {
            ...
            //     $this->mmaster->simpandetail($idcompany, $id, $idproduct[0], $idcolor, $qty, $eremark);
            // }
            if ($idproduct[0]!=null || $skinm!='') {
                $this->mmaster->simpandetail($idcompany, $id, $idproduct[0], $idcolor, $qty, $eremark, $skinm, $statusbarang);
            }
