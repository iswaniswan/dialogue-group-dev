...
class Cform extends CI_Controller {
    ...
    public function getnota(){
        ...
        if($this->input->get('q') != '' && $this->input->get('icustomer') !='') {
            ...
            $data       = $this->mmaster->getnota($cari,$igroup);
            foreach($data->result() as $kuy){
                $filter[] = array(
                    'id'    => $kuy->i_nota,  
    ...
    public function getdetailnota(){
        ...
        $data      = $this->mmaster->getdetailnota($inota,$igroup);
    ...
    public function simpan(){
        ...
        if (($dkn != '') && ($dalokasi != '') && ($ikn != '') && ($vjumlah != '') && ($vjumlah != '0') && ($jml != '0') && ($ijenisbayar != '') && ($icustomer != '')) {
            ...
            if (!$ada) {
                ...
                $this->mmaster->insertheader($ialokasi, $ikn, $iarea, $ijenisbayar, $icustomer, $dkn, $dalokasi, $vjumlah, $vlebih, $idt, $areadt);
                $asal = 0;
                $pengurang = $vjumlah;
                ...
                for ($i = 1; $i <= $jml; $i++) {
                    ...
                    if ($inota!=''||$inota!=null) {
                        ...
                        $this->mmaster->insertdetail($ialokasi, $ikn, $iarea, $inota, $dnota, $dkn, $vjumlah, $vsiso, $x, $ipelunasanremark, $eremark);
            ...
            if(($this->db->trans_status()=== False)){
                ...
                $this->Logger->write('Input knalokasi No:' . $ialokasi . ' Area:' . $iarea);
                $data = array(
                    'sukses'    => true,
                    'kode'      => $ialokasi
                );
