...
class Cform extends CI_Controller {
    ...
    public function simpan()
    {
        ...
        $idreff      = $this->input->post('ireferensi', TRUE);
        ...
        if($idocument != '' && $ddocument != '' && $ibagian != '' && $itujuan != '' && $idreff != '' && $jml > 0) {
            ...
            for($i=0; $i < $jml; $i++){
                ...
                if (($idmaterial != '' || $idmaterial != null) && ($idproductwip != '' || $idproductwip != null) && ($qtywip > 0 || $qtywip > '') && ($qty > 0 || $qty > '')) {
                    $this->mmaster->simpandetail($id,$idreff,$idproductwip,$qtywip,$idmaterial,$qtyreff,$qty,$eremarkitem);
                }
            }
    ...
    public function pending()
    {
        ...
        $data = array(
            'folder'        => $this->global['folder'],
            'title'         => "Tambah Pending ".$this->global['title'],
            'title_list'    => 'List '.$this->global['title'],
            'gudang'        => $this->mmaster->gudang(),
            'tujuan'        => $this->mmaster->tujuan($this->i_menu),
            'dfrom'         => $this->uri->segment(4),
            'dto'           => $this->uri->segment(5),
            'number'        => "BBK-".date('ym')."-123456-01",
        );

        $this->Logger->write('Membuka Menu Tambah Pending '.$this->global['title']);

        $this->load->view($this->global['folder'].'/vformaddpending', $data);
    }

    /****** UNTUK KELUAR PENDINGAN ******/

    public function referensip()
    {
        $filter = [];        
        $data = $this->mmaster->datareferensip(str_replace("'", "", $this->input->get('q')));
        if ($data->num_rows()>0) {
            foreach($data->result() as $key){
                $filter[] = array(
                    'id'   => $key->id,  
    ...
    public function referensidetailp()
    {
        header("Content-Type: application/json", true);
        $query  = array(
            'number' => $this->mmaster->numberpending($this->input->post('id',TRUE)),
            'head'   => $this->mmaster->dataheaderp($this->input->post('id',TRUE))->row(),
            'detail' => $this->mmaster->datadetailp($this->input->post('id',TRUE))->result_array()
        );
        echo json_encode($query);  
    }
    ...
    public function edit()
    {
        ...
        if ($this->uri->segment(7) != null || $this->uri->segment(7) != "") {
            $this->load->view($this->global['folder'].'/vformeditpending', $data);
        }else{
