...
class Cform extends CI_Controller {
    ...
    public function view(){
        ...
        $data = array(
            ...
            'dto'           => $dto,
            'iarea'          => $area,
            'isi'           => $this->mmaster->bacaperiode($area,$dfrom,$dto)->result()
        );
    ...
    public function editpromo(){
        ...
        if(($this->uri->segment(4)!='') && ($this->uri->segment(5)!='')){
            ...
            $query1 = $this->db->query("select * from tm_spb_item where i_spb = '$ispb' and i_area='$area'");
            $this->db->select(" * from tm_spb where i_spb = '$ispb' and i_area='$area'");
			$query = $this->db->get();
            ...
            $data   = array(
                ...
                'pesan'         => $pesan,
                'ipromo'        => $ipromo,
                'isi'           => $this->mmaster->bacapromo($ispb,$area)->row(),
                'detail'        => $this->mmaster->bacadetailpromo($ispb,$area)->result()
            );   
        }
    ...
    public function editspb(){
        ...
        if(($this->uri->segment(4)!='') && ($this->uri->segment(5)!='')){
            ...
            $query = $this->db->query("select * from tm_spb_item where i_spb = '$ispb' and i_area='$area'");
            ...
            $data   = array(
                ...
                'ispb'          => $ispb,
                'area'          => $area,
                'isi'           => $this->mmaster->baca($ispb,$area),
                'detail'        => $this->mmaster->bacadetail($ispb,$area,$ipricegroup)
            );   

            $qnilaispb  = $this->mmaster->bacadetailnilaispb($ispb,$area,$ipricegroup);
            ...
            $qnilaiorderspb   = $this->mmaster->bacadetailnilaiorderspb($ispb,$area,$ipricegroup);
            ...
            $qeket   = $this->db->query(" SELECT e_remark1 as keterangan from tm_spb where i_spb ='$ispb' and i_area='$area' ");
            if($qeket->num_rows()>0){
               $row_eket   = $qeket->row();
    ...
    public function export(){
       ...
       // var_dump($dto);
        ...
        foreach($query as $row) {
          ...
          $nama_file = "LaporanSPBPerDivisi-Area:".$iarea.".xls";
