...
class Cform extends CI_Controller {
    ...
    public function update(){
        ...
        if ((isset($irefference) && $irefference != '') && (isset($dkn) && $dkn != '') && (isset($iarea) && $iarea != '') && (isset($icustomer) && $icustomer != '') && ($finsentif != 'f')){
            ...
            if(($this->db->trans_status()=== False)){
                ...
                $this->Logger->write('Update KN Non Retur:'.$iarea.' No:'.$ikn);
                $data = array(
                    'sukses'    => true,
                    'kode'      => $ikn
                );
    ...
    public function getreferensi(){
        ...
        if($this->input->get('q') != '' && $this->input->get('iarea') !='') {
            ...
            foreach($data->result() as $kuy){
                $filter[] = array(
                    'id'    => $kuy->i_bbm,  
                    'text'  => $kuy->i_bbm." - ".$kuy->i_customer." - ".$kuy->e_area_name
                );
            }
            echo json_encode($filter);
        } else {
    ...
    public function delete(){
        ...
        if(($this->db->trans_status()=== False)){
            ...
            $this->db->trans_commit();
            $this->Logger->write('Mengcancel KN Non Retur Area '.$iarea.' No:'.$ikn);
