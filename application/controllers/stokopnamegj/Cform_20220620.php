...
class Cform extends CI_Controller {
    ...
    public function barang() {
        ...
        if ($data->num_rows()>0) {
            foreach ($data->result() as $row) {
                $filter[] = array(
                    'id'   => $row->id_product.'|'.$row->jenis_barang.'|'.$row->id_color.'|'.$row->e_color_name,
                    'text' => $row->i_product_base.' - '.$row->e_product
                );
            }
        }else{
            $filter[] = array(
    ...
    public function simpan(){
        ...
        for ($i = 1; $i <= $jml; $i++) {
            ...
            if ($idproduct[0]!=null || $idproduct[0]!='') {
                $this->mmaster->simpandetail($idcompany, $id, $idproduct[0], $jenisbarang, $idcolor, $qty, $eremark);
            }
        }
    ...
    public function update(){
        ...
        for ($i = 1; $i <= $jml; $i++) {
            ...
            $eremark        = $this->input->post('eremark' . $i, TRUE);
            if ($idproduct[0]!=null || $idproduct[0]!='') {
                $this->mmaster->simpandetail($idcompany, $id, $idproduct[0], $idproduct[1], $idcolor, $qty, $eremark);
            }
        }
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
