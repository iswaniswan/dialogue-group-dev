...
class Cform extends CI_Controller {
    ...
    public function simpan()
    {
        ...
        $edept = $this->input->post('edept', TRUE);  

        if ($idept != '' && $edept != ''){
            $cekada = $this->mmaster->cek_data($idept);
            if($cekada->num_rows() > 0){
                ...
                $this->mmaster->insert($idept, $edept);
                $data = array(
                    'sukses'    => true,
                    'kode'      => $edept
                );
    ...
    public function edit()
    {
        ...
        $idept = $this->uri->segment('4');

        $data = array(
            'folder'        => $this->global['folder'],
            'title'         => "Edit ".$this->global['title'],
            'title_list'    => 'List '.$this->global['title'],
            'data'          => $this->mmaster->cek_data($idept)->row(),
        );
    ...
    public function update()
    {
        ...
        if ($idept != '' && $edept != ''){
            $cekada = $this->mmaster->cek_data($idept);
            if($cekada->num_rows() > 0){                
                $this->mmaster->update($idept, $edept);
                $data = array(
                    'sukses'    => true,
                    'kode'      => $idept
                );
