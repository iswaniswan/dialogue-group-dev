...
class Cform extends CI_Controller {
    ...
    public function getdetailref(){
        ...
        $reff    = explode('|', $this->input->post('id'));
        $id      = $reff[0];
        $ibagian = $reff[1];
        ...
        $ntop      = $partner[4];
        $ndiskon   = $partner[5];
        $jml    = $this->mmaster->getdetailref($id, $idpartner, $this->i_menu, $ibagian);
        $query  = array(
            'head'   => $this->mmaster->getheadref($id, $ipajak, $fpkp, $ntop, $ndiskon)->row(),
            'jmlitem'=> $jml->num_rows(),
            'detail' => $this->mmaster->getdetailref($id, $idpartner, $this->i_menu, $ibagian)->result_array()
        );
    ...
    public function simpan() {
        ...
        $reff           = explode('|', $this->input->post('ireferensi', TRUE));
        $ireferensi     = $reff[0];
        $ibagianreff    = $reff[1];
        ...
        if ($inota!='' && $dnota!='' && $ibagian!='' && $partner!='' && $ireferensi!='') {
            ...
            if($cekkode->num_rows()>0){
                ...
                $this->mmaster->insertheader($id, $ibagian, $inota, $dnota, $dreceivefaktur, $ipartner, $ireferensi, $dreferensi, $ifaktursup, $dfaktursup, $ipajak, $dpajak, $vdiskon, $vtotaldiskon, $diskonsup, $vtotaldpp, $vtotalppn, $vtotalbruto, $vtotalnetto, $vtotalfa, $eremark, $djatuhtempo, $ibagianreff);
                $x = 0;
                for($i=1;$i<=$jml;$i++){
    ...
    public function view() {
        ...
        $ibagianreff = $this->uri->segment(8);
        $ada = false;
        ...
        $data = array(
            ...
            'ada'        => $ada,
            'bagian'     => $this->mmaster->bagian()->result(),
            'data'       => $this->mmaster->bacaheader($id, $ipartner, $this->i_menu, $ibagianreff)->row(),
            'detail'     => $this->mmaster->bacadetail($id, $this->i_menu, $ibagianreff)->result(),
