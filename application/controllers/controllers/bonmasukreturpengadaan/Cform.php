...
class Cform extends CI_Controller
{
    ...
    {
        $filter = [];
        $data   = $this->mmaster->referensi(strtoupper($this->input->get('q')),$this->input->get('pengirim'));
        if ($data->num_rows() > 0) {
            ...
            if ($idproduct != "" || $idproduct != NULL) {
                foreach ($this->input->post("idproductwip[]", TRUE) as $idproductwip) {
                    if ($idproduct == $idproductwip) {
                        $idproduct          = $this->input->post("idproductwip[]", TRUE)[$i];
                        $idpanelitem        = $this->input->post("iditem[]", TRUE)[$i];
                        $idmaterial         = $this->input->post("idmaterial[]", TRUE)[$i];
                        $idpanel            = $this->input->post("idpanel[]", TRUE)[$i];
                        $nqtyretur          = $this->input->post("nquantityretur[]", TRUE)[$i];
                        $nqtyterima         = $this->input->post("nquantityterima[]", TRUE)[$i];
                        $edesc              = $this->input->post("edesc[]", TRUE)[$i];

                        $this->mmaster->insertdetail($id, $idproduct, $idpanel, $idmaterial, $nqtyretur, $nqtyterima, $edesc);
