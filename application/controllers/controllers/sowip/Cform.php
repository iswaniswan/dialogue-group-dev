...
class Cform extends CI_Controller {
    ...
    public function barang() {
        ...
        if ($data->num_rows()>0) {
            foreach ($data->result() as $row) {
                $filter[] = array(
                    'id'   => $row->id.'|'.$row->e_color_name.'|'.$row->id_color,
    ...
    public function barangall() {
        $filter = [];
        $data = $this->mmaster->barangall($this->input->get('q'));
        if ($data->num_rows()>0) {
            foreach ($data->result() as $row) {
                $filter[] = array(
                    'id'   => $row->id.'|'.$row->e_color_name.'|'.$row->id_color,
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)->setCellValue("A1", strtoupper($this->session->e_company_name))
            ->setCellValue("A2", "FORMAT UPLOAD STOCKOPNAME WIP");
        $spreadsheet->getActiveSheet()->setTitle('Format Stockopname');
        $h = 3;
        $header = ['#', 'ID BARANG', 'KODE BARANG', 'NAMA BARANG', 'WARNA', 'SO BAGUS', 'SO REPAIR','KETERANGAN'];
        ...
        if ($sql->num_rows() > 0) {
            foreach ($sql->result() as $row) {
                $no++;
                $isi = [
                    $no, $row->id, $row->i_product_base, $row->e_product_basename, $row->e_color_name, 0, 0,
                ];
    ...
    public function load()
    {
        ...
        $config = array(
            'upload_path'   => "./import/soproduksi/wip/",
            'allowed_types' => "xls",
            'file_name'     => $filename,
            'overwrite'     => true
        );
        ...
        if ($this->upload->do_upload("userfile")) {
            ...
            $this->Logger->write('Upload File SO WIP : ' . $i_so);
    ...
    public function loadview()
    {
        ...
        $inputFileName = "./import/soproduksi/wip/" . $filename;
        ...
        for ($n = 4; $n <= $hrow; $n++) {
            ...
            $qty_bagus  = (int)$spreadsheet->getActiveSheet()->getCell('F' . $n)->getValue();
