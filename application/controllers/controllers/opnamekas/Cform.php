...
class Cform extends CI_Controller {
    ...
    public function index(){
        $data = array(
            ...
            'hariini'   => $this->mmaster->cekoptoday()->row()
        );
    ...
    public function simpan(){
        ...
        $dfrombank          = $this->input->post('dfrombank',TRUE);    
        $dfrombank          = date('Y-m-d', strtotime($dfrombank));
        $counter            = 0;
        for ($i = 1; $i<= 5; $i++){
            $rekbank    = $this->input->post('rekbank'.$i,TRUE);
            $pecah1     = explode("+", $rekbank);
            $icoa1      = $pecah1[0]; 
            if((!empty($coa1)) or ($icoa1 != null) or ($icoa1 != '')){
                $counter +=1;
            }else{
                $counter +=0;
            } 
        ...
        $saldobank1         = $this->input->post('saldobank1',TRUE);
        $saldobank2         = $this->input->post('saldobank2',TRUE);
        $saldobank3         = $this->input->post('saldobank3',TRUE);
        $saldobank4         = $this->input->post('saldobank4',TRUE);
        $saldobank5         = $this->input->post('saldobank5',TRUE);
        ...
        $nama               = ''; 
        $this->db->trans_begin();
        $rm                 = $this->mmaster->runningnumber($thbl);
        for ($i = 1; $i<= $counter; $i++){
            $rekbank    = $this->input->post('rekbank'.$i,TRUE);
            if($rekbank!="+"){
                $pecah  = explode("+", $rekbank);
                $icoa   = $pecah[0]; 
                $nama   = $pecah[1]; 
            }
            $saldobank  = intval($this->input->post('saldobank'.$i,TRUE));
            $this->mmaster->opnamebank($rm, $thbl,$icoa,$nama, $saldobank,$dfrombank);
        }
        $iarea = '00';
        ...
        if(($this->db->trans_status()=== False)){
            ...
            $this->Logger->write('Input KAS Opname 00-Pusat No:'.$rm);
            $data = array(
                'sukses'    => true,
                'kode'      => $rm
            );
