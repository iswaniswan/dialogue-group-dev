...
class Cform extends CI_Controller {
    ...
    {
        $filter = [];
        if ($this->input->get('idjenis')!='' || $this->input->get('idjenis')!=null) {
            $data   = $this->mmaster->area(str_replace("'","",$this->input->get('q')),$this->input->get('idjenis'));            
            if ($data->num_rows()>0) {
                foreach($data->result() as $key){
                    $filter[] = array(
                        'id'   => $key->id,  
    ...
    public function customer()
    {
        ...
        if (($ijenis != '' || $ijenis != null) && ($iarea != '' || $iarea != null)) {
            ...
            $filter[] = array(
                'id'   => null,
                'text' => "Jenis SPB dan Area Harus Dipilih!"
            );   
        }
        echo json_encode($filter);
    }
    ...
    public function referensi()
    {
        ...
        $ijenis      = $this->input->get('ijenis', TRUE);
        $iarea       = $this->input->get('iarea', TRUE);
        if (($ijenis != '' || $ijenis != null) && ($icustomer != '' || $icustomer != null) && ($iarea != '' || $iarea != null)) {
            $data   = $this->mmaster->referensi(str_replace("'","",$this->input->get('q')),$icustomer,$ijenis,$iarea);            
            if ($data->num_rows()>0) {
                foreach($data->result() as $key){
                    $filter[] = array(
                        'id'   => $key->id,  
            ...
            $filter[] = array(
                'id'   => null,
                'text' => "Jenis SPB, Area dan Pelanggan Harus Dipilih!"
            );   
        }
        echo json_encode($filter);
    }
    ...
    public function simpan()
    {
        ...
        if ($idharga!='' && $ibagian!='' && $icustomer!='' && $jml>0) {
            ...
            if ($cekkode->num_rows()>0) {
                ...
                $this->mmaster->insertheader($id,$isj,$datedocument,$ibagian,$ireferensi,$icustomer,$ecustomer,$ncustop,$idarea,$ijenis,$idharga,$eremark);
                $no = 0;
    ...
    public function update()
    {
        ...
        if ($id!='' && $idharga!='' && $ibagian!='' && $icustomer!='' && $jml>0) {
            ...
            if ($cekkode->num_rows()>0) {
                ...
                $this->db->trans_begin();
                $this->mmaster->updateheader($id,$isj,$datedocument,$ibagian,$ireferensi,$icustomer,$ecustomer,$ncustop,$idarea,$ijenis,$idharga,$eremark);
