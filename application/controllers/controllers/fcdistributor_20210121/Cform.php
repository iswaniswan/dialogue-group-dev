...
class Cform extends CI_Controller
{
    ...
    public function loadview()
    {
        ...
        for ($n = 2; $n <= $hrow; $n++) {
            ...
            $rata2  = $spreadsheet->getActiveSheet()->getCell('J' . $n)->getValue();
            $qty  = $spreadsheet->getActiveSheet()->getCell('K' . $n)->getValue();
            /* $v_harga  = $spreadsheet->getActiveSheet()->getCell('J' . $n)->getValue(); */
            $e_remark  = $spreadsheet->getActiveSheet()->getCell('L' . $n)->getValue();
            ...
            if ($qty > 0 && $idproduct != "") {
                ...
                if ($cek_produk->num_rows() > 0) {
                    $aray[] = array(
                        ...
                        'e_class_name'          => $spreadsheet->getActiveSheet()->getCell('I' . $n)->getValue(),
                        'v_harga'               => $v_harga,
                        'n_rata2'               => $rata2,
                        'n_quantity'            => $qty,
                        'n_quantity_sisa'       => $qty,
                ...
                $aray_notfound[] = array(
                    ...
                    'i_product_base'        => $i_product,
                    'e_product_basename'    => $spreadsheet->getActiveSheet()->getCell('C' . $n)->getValue(),
                    'e_class_name'          => $spreadsheet->getActiveSheet()->getCell('G' . $n)->getValue(),
                    'v_harga'               => $v_harga,
                    'n_rata2'               => $rata2,
                    'n_quantity'            => $qty,
                    'n_quantity_sisa'       => $qty,
    ...
    public function export()
    {
        ...
        if ($query) {
            ...
            $spreadsheet->setActiveSheetIndex(0)
                ...
                ->setCellValue('F1', 'Sub Kategori')
                ->setCellValue('G1', 'Brand')
                ->setCellValue('H1', 'Series')
                ->setCellValue('I1', 'Kategori Penjualan')
                ->setCellValue('J1', 'Rata-rata OP')
                ->setCellValue('K1', 'QTY FC')
            ...
            foreach ($query as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('G' . $kolom, $row->e_brand_name)
                    ->setCellValue('H' . $kolom, $row->e_style_name)
                    ->setCellValue('I' . $kolom, $row->e_class_name)
