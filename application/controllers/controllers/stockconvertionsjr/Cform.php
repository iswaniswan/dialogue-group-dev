...
class Cform extends CI_Controller {
    ...
    public function getsjr(){
        $filter = [];
        if($this->input->get('q') != '') {
            $filter     = [];
            $cari       = strtoupper($this->input->get('q'));
            $tgl1       = date('Ym');
            $periode    = date('Ym', strtotime('-3 month', strtotime($tgl1))); //operasi penjumlahan tanggal sebanyak 6 hari
            $data       = $this->mmaster->getsjr($cari, $periode);
            foreach($data->result() as $kuy){
                $filter[] = array(
                    'id'    => $kuy->i_sjr,  
                    'text'  => $kuy->i_area." - ".$kuy->e_area_name." - ".$kuy->i_sjr." - ".$kuy->dsjr
                );
            }
    ...
    public function getdetailitem(){
        ...
        $isjr   = $this->input->post('isjr', FALSE);
        $query  = array(
            'detail' => $this->mmaster->bacadetail($isjr)->result_array()
        );
    ...
    public function simpan(){
        ...
        if(($iicconvertion=='') && ($dicconvertion!='') && ($irefference!='') && ($drefference!='') && (($jml!='')&&($jml!='0'))){
            ...
            if(($this->db->trans_status()=== False)){
                ...
                $this->Logger->write('Input Konversi Stock SJR No:'.$iicconvertion);
                $data = array(
                    'sukses'    => true,
                    'kode'      => $iicconvertion
                );
