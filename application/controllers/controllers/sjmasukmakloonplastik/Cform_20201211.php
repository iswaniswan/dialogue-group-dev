...
class Cform extends CI_Controller {
    ...
    function getmaterial(){
      ...
      public function simpan(){
        ...
        $this->db->trans_begin();
        $this->mmaster->insertheader($nosjmasuk, $datesjk,$nosjkeluar,$istore,$supplier, $remark);
        for($i=1;$i<=$jml;$i++){
    ...
    public function update(){
        ...
        for($i=1;$i<=$jml;$i++){
            $cek=$this->input->post('chk'.$i, TRUE);
            $i_2material = $this->input->post('i_2material'.$i, TRUE);
                if($cek=='on'){
                    $imaterial_reff = $this->input->post('i_material'.$i, TRUE);
                    $imaterial      = $i_2material;
                    $nquantity      = $this->input->post('n_2qty'.$i, TRUE);
                    $isatuan        = $this->input->post('i_2satuan'.$i, TRUE); 
                    $edesc          = $this->input->post('edesc'.$i, TRUE);
                    $this->mmaster->insertdetail($nosjmasuk,$istore,$nosjkeluar, $imaterial_reff, $imaterial, $nquantity,$isatuan, $edesc, $i);
            ...
            if ($this->db->trans_status() === FALSE)
            {
                ...
                $this->db->trans_commit();
                $this->Logger->write('Update Data '.$this->global['title'].' No SJ : '.$nosjmasuk.' Gudang :'.$istore);
                $data = array(
                    'sukses' => true,
                    'kode'      => $nosjmasuk,
                );
