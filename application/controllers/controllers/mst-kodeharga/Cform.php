...
class Cform extends CI_Controller {
    ...
    public function simpan()
    {
        ...
        if ($iharga != '' && $eharga != ''){
            ...
            if($cekada->num_rows() > 0){
                ...
                $this->db->trans_begin();
                $this->mmaster->insert($id, $iharga, $eharga);
                if($this->db->trans_status() === False){
                    $this->db->trans_rollback();
                    $data = array(
                        'sukses' => false,
                    ...
                    $this->db->trans_commit();
                    $this->Logger->write('Simpan Data '.$this->global['title'].' Kode : '.$iharga);
                    $data = array(
                        'sukses'  => true,
                        'kode'    => $iharga
                    );
    ...
    public function update()
    {
        ...
        if ($id != '' && $iharga != '' && $eharga != ''){            
            $this->db->trans_begin();       
            $this->mmaster->update($id, $iharga, $eharga);
            if($this->db->trans_status() === False){
                $this->db->trans_rollback();
                $data = array(
                    'sukses' => false,
                ...
                $this->db->trans_commit();
                $this->Logger->write('Update Data '.$this->global['title'].' Kode : '.$iharga);
                $data = array(
                    'sukses' => true,
                    'kode'   => $iharga
                );
