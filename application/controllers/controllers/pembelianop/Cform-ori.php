...
class Cform extends CI_Controller {
    ...
    public function proses1(){
        ...
        $data = array(
            ...
            'data'          => $this->mmaster->get_harga1($ipp, $isupplier)->row(),
            'data2'         => $this->mmaster->get_harga1($ipp, $isupplier)->result(),
    ...
    public function simpan(){
        ...
        $this->mmaster->insertheader($id, $iop, $dateop, $ibagian, $isupplier, $ntop, $ndiskon, $itypepajak, $importantstatus, $datedeliv, $eremark, $fpkp, $esuppliername);
            for($i=1;$i<=$jml;$i++){   
    ...
    public function update(){
        ...
        $this->db->trans_begin();
        $this->Logger->write('Simpan Data '.$this->global['title'].' Kode : '.$iop);
        $this->mmaster->update($id, $iop, $dateop, $ibagian, $isupplier, $ntop, $ndiskon, $itypepajak, $importantstatus, $datedeliv, $eremark, $fpkp, $esuppliername);
            ...
            for($i=1;$i<=$jml;$i++){   
                ...
                $imaterial      = $this->input->post('imaterial'.$i, TRUE);             
                $nquantity      = $this->input->post('nquantity'.$i, TRUE);
                $qty_now        = $this->input->post('nquantity_now'.$i, TRUE);
                $nsisa          = $nquantity;
                $vprice         = $this->input->post('vprice'.$i, TRUE);
                $remark         = $this->input->post('eremark'.$i, TRUE);
    ...
    public function cetak(){
        ...
        $data = array(
            ...
            'iop'           => $iop,
            'data'          => $this->mmaster->cetakop($iop)->result(),
            'data2'         => $this->mmaster->get_item($idpp,$idop, $idcompany)->result()
        );
