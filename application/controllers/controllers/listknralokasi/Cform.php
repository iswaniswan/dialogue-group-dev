...
//        echo $this->mmaster->data($dfrom, $dto, $iarea, $this->global['folder'], $iperiode, $this->global['title']);
    ...
    public function proses(){
        ...
        $ikn        = $this->uri->segment(5);
        $iarea      = $this->uri->segment(6);
        $dfrom      = $this->uri->segment(7);
        $dto        = $this->uri->segment(8);
        $query      = $this->mmaster->cekedit($ialokasi,$ikn,$iarea);
        if($query->num_rows()>0){
            $edit = false;
        }else{
            $edit = true;
        ...
        $data = array(
            ...
            'bisaedit'  => $edit,
            'isi'       => $this->mmaster->baca($ialokasi,$iarea,$ikn),
            'detail'    => $this->mmaster->bacadetail($ialokasi,$iarea,$ikn),
            'area'      => $this->mmaster->bacaarea($username, $idcompany),
            'iperiode'  => $this->mmaster->cekperiode()
        );
