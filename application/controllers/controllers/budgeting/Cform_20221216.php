        ...
        if ($sql->num_rows() > 0) {
            foreach ($sql->result() as $row) {
                ...
                $isi = [
                    ...
                    0, "=(F$j-H$j)+G$j+I$j", ucwords(strtolower($row->e_satuan_name)), $hitungan, $row->estimasi, "=L$j-M$j", 0, 
    ...
    public function load()
    {
        ...
        if ($this->upload->do_upload("userfile")) {
            ...
            for ($n = 5; $n <= $hrow; $n++) {
                ...
                $n_op_sisa = $spreadsheet->getActiveSheet()->getCell('M' . $n)->getValue();
                $n_up_qty = $spreadsheet->getActiveSheet()->getCell('O' . $n)->getValue();
                $e_remark = $spreadsheet->getActiveSheet()->getCell('R' . $n)->getValue();
