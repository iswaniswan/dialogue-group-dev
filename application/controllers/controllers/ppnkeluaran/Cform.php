      ...
      $sharedStyle1->applyFromArray(
        ...
        foreach(range('A','S') as $columnID) {
          ...
          $spreadsheet->setActiveSheetIndex(0)
                      ->setCellValue('A1', 'LAPORAN PPN KELUARAN')
                      ->setCellValue('A2', "Laporan PPN Keluaran Periode : $periode")
                      ->setCellValue('A5', 'KODETRAN,C,2')
                      ->setCellValue('B5', 'NAMADGU,C,40')
                      ->setCellValue('C5', 'NPWPDGU,C,20')
                      ->setCellValue('D5', 'NODOK,C,7')
                      ->setCellValue('E5', 'TGLDOK,D')
                      ->setCellValue('F5', 'KODELANG,C,5')
                      ->setCellValue('G5', 'NAMALANG,C,25')
                      ->setCellValue('H5', 'KODESALES,C,2')
                      ->setCellValue('I5', 'NOFAKTUR,C,6')
                      ->setCellValue('J5', 'NOSERI,C,20')
                      ->setCellValue('K5', 'TGLPJK,D')
                      ...
                      ->setCellValue('Q5', 'NAMAPKP,C,40')
                      ->setCellValue('R5', 'ALAMATPKP,C,90')
                      ->setCellValue('S5', 'NPWP');
          ...
          foreach($query as $row) {
            ...
            $spreadsheet->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('B' . $kolom, NmPerusahaan)
                        ->setCellValue('C' . $kolom, NPWPPerusahaan)
                        ->setCellValue('D' . $kolom, $row->i_nota)
                        ->setCellValue('E' . $kolom, $row->d_nota)
                        ->setCellValue('F' . $kolom, $row->i_customer)
                        ->setCellValue('G' . $kolom, $row->e_customer_name)
                        ->setCellValue('H' . $kolom, $row->i_salesman)
                        ->setCellValue('I' . $kolom, $row->i_faktur_komersial)
                        ->setCellValue('J' . $kolom, $row->i_seri_pajak)
                        ->setCellValue('K' . $kolom, $row->d_pajak)
                        ->setCellValue('L' . $kolom, $row->v_nota_gross)
                        ->setCellValue('M' . $kolom, $row->v_nota_discount)
                        ->setCellValue('N' . $kolom, $row->v_nota_dpp)
                        ->setCellValue('O' . $kolom, $row->v_nota_ppn)
                        ->setCellValue('P' . $kolom, $row->v_nota_netto)
                        ->setCellValue('Q' . $kolom, $row->e_customer_pkpname)
                        ->setCellValue('R' . $kolom, $row->e_customer_pkpaddress)
                        ->setCellValue('S' . $kolom, $row->e_customer_pkpnpwp);
        ...
        $nama_file = "PPN Keluaran".$periode."_".$area.".xls";
