...
class Cform extends CI_Controller
{
    ...
    {
        $filter = [];
        if ($this->input->get('ireferensi') != '') {
            $data   = $this->mmaster->product(str_replace("'", "", $this->input->get('q')), $this->input->get('ireferensi'));
            if ($data->num_rows() > 0) {
                foreach ($data->result() as  $key) {
                    $filter[] = array(
                        'id'   => $key->id,
                        'text' => $key->i_product . ' - ' . $key->e_product,
                    );
                }
            } else {
                $filter[] = array(
            ...
            $filter[] = array(
                'id'   => null,
                'text' => 'No Referensi Tidak Boleh Kosong!',
            );
        }
        echo json_encode($filter);
    }
    ...
    {
        header("Content-Type: application/json", true);
        echo json_encode($this->mmaster->getproduct($this->input->post('idproduct'), $this->input->post('id'))->result_array());
    }
        ...
        if ($idocument != '' && $ddocument != '' && $ibagian != '' && $idcustomer != '' && $jml > 0) {
            ...
            if ($cekkode->num_rows() > 0) {
                ...
                for ($i = 0; $i < $jml; $i++) {
                    ...
                    $idproductttb = $this->input->post('idproductttb' . $i, TRUE);
                    $idproductbbm = $this->input->post('idproductbbm' . $i, TRUE);
                    ...
                    if ($nquantity > 0 && ($idproductttb != null || $idproductttb != '') && ($idproductbbm != null || $idproductbbm != '') && ($iddocument != null || $iddocument != '')) {
                        $this->mmaster->insertdetail($id, $iddocument, $idproductbbm, $nquantity, $vprice, $ndiskon1, $ndiskon2, $ndiskon3, $vdiskon1, $vdiskon2, $vdiskon3, $vdiskonplus, $vtotaldiskon, $vtotal, $eremark);
                        $this->mmaster->updatettb($iddocument, $idproductttb, $idproductbbm, $nquantity);
                    }
                }
