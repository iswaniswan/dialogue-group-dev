...
class Cform extends CI_Controller {
    ...
    public function prosesdata() 
    {
        ...
        $data = array(
            ...
            // 'pembuat'    => $this->db->order_by('e_bagian_name')->get_where("tr_bagian",["f_status"=>'t', "id_company"=>$this->id_company,"i_type"=>"10"]),
            'datadetail' => $this->mmaster->datadetail($this->uri->segment(4)),
        ...
        if ($idocument!='' && $ddocument!='' && $ibagian!='' && $idreferensi!='' && $jml > 0) {
            ...
            if ($cekkode->num_rows()>0) {
                ...
                for ($i = 0; $i <= $jml; $i++) {
                    ...
                    $n_pengadaan            = str_replace(",","",$this->input->post('n_pengadaan'.$i, TRUE));
                    $n_fcjahit              = str_replace(",","",$this->input->post('n_fcjahit'.$i, TRUE));
                    $n_fcjahit_internal     = str_replace(",","",$this->input->post('n_fcjahit_internal'.$i, TRUE));
                    $n_fcjahit_eksternal    = str_replace(",","",$this->input->post('n_fcjahit_eksternal'.$i, TRUE));
                    $n_fcjahit_ade          = str_replace(",","",$this->input->post('n_fcjahit_ade'.$i, TRUE));
                    $e_remark               = $this->input->post('e_remark'.$i, TRUE);

                    if (/* $n_fcjahit > 0 &&  */($idproduct!=null || $idproduct!='')) {
                        $this->mmaster->insertdetailbase($id,$idproduct,$n_quantity,$n_pengadaan,$n_fcjahit,$n_fcjahit_internal,$n_fcjahit_eksternal,$n_fcjahit_ade,$e_remark);
                    }
                }
    ...
    public function edit()
    {
        ...
        $data = array(
            ...
            'datadetail' => $this->mmaster->edititembase($this->uri->segment(4)),
        ...
        if ($id!='' && $idocument!='' && $ddocument!='' && $ibagian!='' && $idreferensi!='' && $jml > 0) {
            ...
            if ($cekkode->num_rows()>0) {
                ...
                for ($i = 0; $i <= $jml; $i++) {
                    ...
                    if (/* $n_fcjahit > 0 &&  */($idproduct!=null || $idproduct!='')) {
                        // $this->mmaster->insertdetailbase($id,$idproduct,$n_quantity,$n_pengadaan,$n_fcjahit,$e_remark);
                        $this->mmaster->insertdetailbase($id,$idproduct,$n_quantity,$n_pengadaan,$n_fcjahit,$n_fcjahit_internal,$n_fcjahit_eksternal,$n_fcjahit_ade,$e_remark);
                    }
                }
