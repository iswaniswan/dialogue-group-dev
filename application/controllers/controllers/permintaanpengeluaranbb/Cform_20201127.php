...
class Cform extends CI_Controller {
    ...
    public function index(){
        //$ijenisgudang     = $this->session->userdata('i_jenis_gudang');
...
//var_dump($ikodejenis);
    ...
    function datamaterial(){
        ...
        if($this->input->get('q') != '') {
            ...
            $ikodekelompok =  $this->uri->segment(5);
           // var_dump($ikodekelompok);
            $data = $this->mmaster->product($cari, $gudang, $ikodekelompok);
     ...
     public function getpic(){
        ...
        if($jenis == 'JK00001'){
            ...
            if($query->num_rows()>0) {
                ...
                $spb = $query->result();
                foreach($spb as $row) {
                    $c.="<option value=".$row->partner." >".$row->epartner."</option>";
                }
            ...
            $query = $this->mmaster->getpicEKs($group_supplier);
            if($query->num_rows()>0) {
                $c  = "";
                $spb = $query->result();
                foreach($spb as $row) {
            ...
            if($query->num_rows()>0) {
                ...
                $query = $this->mmaster->getpicEKste();
                if($query->num_rows()>0) {
                    $c  = "";
                    $spb = $query->result();
                    foreach($spb as $row) {
    ...
    public function edit(){
        ...
        $partner          = $this->mmaster->partner($tujuankeluar, $jenis, $group_supplier)->result();
    ...
    public function update(){
        ...
        $this->db->trans_begin();
        $this->mmaster->updateheader($dbonk, $istore, $tujuankeluar, $pic, $dept, $remark, $nobonkeluar, $epic, $jenispengeluaran, $memo, $dmemo);
