...
class Cform extends CI_Controller
{
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)->setCellValue("A1", strtoupper($this->session->e_company_name))
            ->setCellValue("A2", "FORMAT UPLOAD STOCKOPNAME GUDANG JADI");
        $spreadsheet->getActiveSheet()->setTitle('Format Stockopname');
        $h = 3;
        $header = ['#', 'ID BARANG', 'KODE BARANG', 'NAMA BARANG', 'WARNA', 'SO BAGUS', 'SO REPAIR', 'SO GRADE B', 'KETERANGAN'];
        ...
        if ($sql->num_rows() > 0) {
            foreach ($sql->result() as $row) {
                $no++;
                $isi = [
                    $no, $row->id, $row->i_product_base, $row->e_product_basename, $row->e_color_name, 0, 0, 0,
                ];
    ...
    public function load()
    {
        ...
        $config = array(
            'upload_path'   => "./import/soproduksi/gudangjadi/",
            'allowed_types' => "xls",
            'file_name'     => $filename,
            'overwrite'     => true
        );
        ...
        if ($this->upload->do_upload("userfile")) {
            ...
            $this->Logger->write('Upload File SO Gudang Jadi : ' . $i_so);
    ...
    public function loadview()
    {
        ...
        $inputFileName = "./import/soproduksi/gudangjadi/" . $filename;
        ...
        for ($n = 4; $n <= $hrow; $n++) {
            ...
            $qty_grade  = (int)$spreadsheet->getActiveSheet()->getCell('H' . $n)->getValue();
            if (($qty_bagus > 0 || $qty_repair > 0 || $qty_grade > 0) && $id_product != "") {
                $aray[] = array(
                    'id'                => $id_product,
                    'i_product'         => strtoupper($spreadsheet->getActiveSheet()->getCell('C' . $n)->getValue()),
