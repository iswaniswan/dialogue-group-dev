...
class Cform extends CI_Controller {
    ...
    public function tambah(){
        ...
        $data = array(
            ...
            'jenis'=> $this->mmaster->get_jenis()->result(),
            'salesman'=> $this->mmaster->get_salesman()->result()
        );
	...
	public function simpan(){
        ...
        if ($iarea != '' && $icustomer != '' && $isalesman != '' && $iperiode != ''){
                $cekada = $this->mmaster->cek_data($iarea, $icustomer, $iproductgroup);
                if($cekada->num_rows() > 0){
                    ...
                    $this->db->trans_begin();
                    $this->mmaster->insert($icustomer, $isalesman, $iarea, $esalesman, $iproductgroup, $iperiode);
    ...
    public function edit(){
        ...
        $data = array(
            ...
            'jenis'         => $this->mmaster->get_jenis()->result(),
            'periode'       => $iperiode,
            'data'          => $this->mmaster->cek_data($iarea, $icustomer, $iproductgroup)->row()
        );
    ...
    public function update(){
        ...
        if ($iarea != '' && $icustomer != '' && $iproductgroup != ''){
            $cekada = $this->mmaster->cek_data($iarea, $icustomer, $iproductgroup);
            if($cekada->num_rows() > 0){ 
                $this->db->trans_begin();

                $this->mmaster->update($icustomer, $isalesman, $iarea, $esalesman, $iproductgroup, $iperiode);
    ...
    function data_salesman(){
        $filter = [];

        if($this->input->get('q') != '') {
            $filter = [];
            $cari = strtoupper($this->input->get('q'));
            $this->db->select("i_salesman, e_salesman_name");
            $this->db->from("tr_salesman");
            $this->db->where("f_salesman_aktif = 't'");
            $this->db->like("i_salesman", $cari);
            $this->db->or_like("UPPER(e_salesman_name)", $cari);
            $data = $this->db->get();
            foreach($data->result() as $row){
                    $filter[] = array(
                    'id'    => $row->i_salesman,
                    'text'  => $row->i_salesman.' - '.$row->e_salesman_name
                );
            }
