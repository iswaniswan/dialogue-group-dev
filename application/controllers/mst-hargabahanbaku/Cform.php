...
class Cform extends CI_Controller {
    ...
    public function status(){
        ...
        if ($id!='') {
            $this->db->trans_begin();
            $data = $this->mmaster->status($isupplier, $imaterial, $id);
        ...
        if ($isupplier != ''){
            ...
            for($i=1;$i<=$jml;$i++){ 
                //if($this->input->post('cek'.$i)=='cek'){
                    ...
                    $this->mmaster->insert($isupplier, $barang, $harga, $isatuansupp, $hargakonversi, $norder, $imaterialsupplier, $dateberlaku, $fppn);   
                //}
                $id = $this->mmaster->getId($isupplier, $barang, $harga, $isatuansupp, $hargakonversi, $norder, $imaterialsupplier, $dateberlaku)->row();
                array_push($tampungId, $id->id);
            } 
            if($this->db->trans_status() === False){
                $this->db->trans_rollback();
                $data = array(
    ...
    public function edit(){
        ...
        $data = array(
            ...
            'isupplier'     => $isupplier,
            'id'            => $id,
            'data'          => $this->mmaster->cek_data($kodebrg,$isupplier, $id, $idcompany)->row(),
            'satuan'        => $this->mmaster->getsatuan($idcompany),
        );
    ...
    public function ubahtanggalberlaku(){
        ...
        $imaterialsupplier = $this->input->post('imaterialsupplier', TRUE);   
        $isatuansupp     = $this->input->post('isatuansupp', TRUE);   
        ...
        $this->db->trans_begin();
        $data =  $this->mmaster->update($id, $isupplier, $kodebrg, $harga, $isatuansupp, $hargakonversi, $norder, $imaterialsupplier, $dateberlaku, $fppn, $idcompany);
        if ($this->db->trans_status() === false) {
            $this->db->trans_rollback();
        }else {
            $this->db->trans_commit();
            $this->Logger->write('Ubah Harga Bahan Baku dan Bahan Pembantu by Supplier ' . $kodebrg);
            echo json_encode($data);
        }
    }
    ...
    public function inserttanggalberlaku(){
        ...
        $this->db->trans_begin();
        $data =  $this->mmaster->updatetglakhir($id, $isupplier, $kodebrg, $harga, $isatuansupp, $hargakonversi, $norder, $imaterialsupplier, $fppn, $dateberlaku, $dateberlakusebelum, $idcompany);
        if ($this->db->trans_status() === false) {
            $this->db->trans_rollback();
        }else {
            $this->db->trans_commit();
    ...
    public function update(){
            ...
            $norder         = $this->input->post('norder', TRUE);
            $imaterialsupplier = $this->input->post('imaterialsupplier', TRUE);
            $isatuansupp    = $this->input->post('isatuansupplier', TRUE);
            $hargakonversi  = $this->input->post('hargakonversi', TRUE);
            $hargakonversi  = str_replace(',','',$hargakonversi);
            ...
            if ($isupplier != '' && $harga != ''){
                ...
                if($dateberlaku == $dateberlakusebelum){
                    $this->mmaster->update($id, $isupplier, $kodebrg, $harga, $isatuansupp, $hargakonversi, $norder, $imaterialsupplier, $dateberlaku, $fppn, $idcompany);
                }else{
                    $this->mmaster->insert($isupplier, $kodebrg, $harga, $isatuansupp, $hargakonversi, $norder, $imaterialsupplier, $dateberlaku, $fppn);  
                    $this->mmaster->updatetglakhir($id, $isupplier, $kodebrg, $dateberlakusebelum, $dateberlaku, $idcompany);
                }
                $data = array(
                    'sukses'    => true,
                    'kode'      => $kodebrg
    ...
    public function changestatusarray() {
        $id = explode(',',$this->input->post('id', true));
        $istatus = $this->input->post('istatus', true);
        $estatus = $this->mmaster->estatus($istatus);
        $this->db->trans_begin();
        foreach($id as $i) {
            $this->mmaster->changestatus($i, $istatus);
        }
        if ($this->db->trans_status() === false) {
            $this->db->trans_rollback();
            echo json_encode (false);
            ...
            $this->db->trans_commit();
            $this->Logger->write('Update Status ' . $this->global['folder'] . ' Menjadi : ' . $estatus . ' Id : ' . $this->input->post('id', true));
            echo json_encode(true);
        }
    }
