...
class Cform extends CI_Controller {
    ...
    public function giro(){
        $filter = [];
        if($this->input->get('q') != '' && $this->input->get('iarea') != '' && $this->input->get('tgl') != '') {
            $filter     = [];
            $cari       = strtoupper($this->input->get('q'));
            ...
            $data       = $this->mmaster->bacagiro($cari, $iarea, $xtgl);
            foreach($data->result() as $kuy){
                $filter[] = array(
                    'id'    => $kuy->bayar,  
                    'text'  => $kuy->bayar." - ".$kuy->v_jumlah
                );
            }
            echo json_encode($filter);
        } else {
    ...
    public function simpan(){
        ...
        $adaket=false;
        for ($a=1;$a<=$jml;$a++) {
            $ket=$this->input->post('edescription'.$a,TRUE);
            if($ket==''){
                $adaket=false;
                break;
            }else{
        ...
        if ((isset($iperiode) && $iperiode != '') && (isset($ibank) && $ibank != '') && (isset($iarea) && $iarea != '') && (isset($icoabank) && $icoabank != '') && (isset($dbank) && $dbank != '') && (isset($jml) && $jml != '' && $jml != '0') && ($adaket)){
            $this->db->trans_begin();
            $tot  = 0;
            $irvb = $this->mmaster->runningnumberrvb($tah,$bul,$icoabank,$iarea);
            $irv  = $this->mmaster->runningnumberrv($tah,$bul,$iarea,$irvtype,$icoabank);            
            for ($a=1;$a<=$jml;$a++) {
                ...
                $igiro        = $this->input->post('igiro'.$a, TRUE);
                if($dbukti!=''){
                    $tmp=explode("-",$dbukti);
                    $xth=$tmp[2];
                    $xbl=$tmp[1];
                ...
                $this->mmaster->insert( $iareax,$ikode,$iperiode,$icoa,$vbank,$dbukti,$ecoaname,$edescription,$fdebet,$icoabank,$igiro);
                ...
                $this->mmaster->insertgldebet($accdebet,$ikode,$namadebet,'t',$iareax,$vbank,$dbank,$eremark,$icoabank);
                $this->mmaster->insertglkredit($acckredit,$ikode,$namakredit,'f',$iareax,$vbank,$dbank,$eremark,$icoabank);
                $this->mmaster->insertrvitem( $irv,$iarea,$icoa,$ecoaname,$vbank,$edescription,$ikode,$irvtype,$iareax,$icoabank);
            }
            $this->mmaster->insertrv( $irv,$iarea,$iperiode,$icoabank,$drv,$tot,$eremark,$irvtype);
            $this->mmaster->insertrvb( $irvb,$icoabank,$irv,$iarea,$irvtype);
            if(($this->db->trans_status()=== False)){
                ...
                $this->Logger->write('Input Bank Masuk No:'.$ikode.' Periode:'.$iperiode.' Area:'.$iarea);
                $kode = array(
                    'sukses'    => true,
                    'kode'      => $ikode
                );
