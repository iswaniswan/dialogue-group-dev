...
class Cform extends CI_Controller
{
    ...
    public function barang()
    {
        $filter = [];
        $q = $this->input->get('q');
        $data = $this->mmaster->barang_grb05(str_replace("'", "", $q), $this->input->get('ibagian'), $this->input->get('ddocument'));
        if ($data->num_rows() > 0) {
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)->setCellValue("A1", strtoupper($this->session->e_company_name))
            ->setCellValue("A2", "FORMAT UPLOAD STOCKOPNAME PENGADAAN MATERIAL");
        ...
        $x = 4;
        $no = 0;
        $sql = $this->mmaster->get_export_so_material($i_bagian);
        ...
        $nama_file = "SO_Pengadaan_Material.xls";
