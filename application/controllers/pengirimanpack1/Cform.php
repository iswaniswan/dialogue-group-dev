...
class Cform extends CI_Controller {
    ...
    function getdetailbar(){
        header("Content-Type: application/json", true);
        $iproduct = $this->input->post('imaterial');
        $this->db->select(" a.*, b.v_price from tr_material a 
        left join tr_supplier_materialprice b on a.i_material = b.i_material where a.i_material = '$iproduct'", false);
    ...
    function databrg(){
        ...
        if($this->input->get('q') != '') {
            $filter = [];
            $cari = strtoupper($this->input->get('q'));
            // $this->db->select("* select * from tr_material where i_kode_kelompok = 'KTB0001'");
            $this->db->select("* "); 
            ...
            foreach($data->result() as  $product){       
                    ...
                    'text' => $product->i_material.' - '.$product->e_material_name//.' - '.$product->e_color_name
                );
            }   
            echo json_encode($filter);
        }else{
