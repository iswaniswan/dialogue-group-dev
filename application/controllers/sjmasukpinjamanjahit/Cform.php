...
class Cform extends CI_Controller
{
    ...
    {
        $filter = [];
        $iasal        = $this->input->get('iasal', TRUE);
        $iasal        = explode('|', $iasal);
        $idpartner    = $iasal[0];
        $ipartner     = $iasal[1];
    ...
    public function getdataitem()
    {
        ...
        $idpartner = $iasal[0];
        $ipartner  = $iasal[1];
        $jml = $this->mmaster->getdataitem($idreff, $idpartner, $ipartner);
        $data = array(
            'datahead'   => $this->mmaster->getdataheader($idreff, $idpartner, $ipartner)->row(),
            'jmlitem'    => $jml->num_rows(),
            'dataitem'   => $this->mmaster->getdataitem($idreff, $idpartner, $ipartner)->result_array()
        );
    ...
    public function simpan()
    {
        ...
        $n_quantity = $this->input->post('nquantityterima[]', TRUE);
        ...
        if($idocument != ''  && $ddocument != '' && $ibagian != '' && $iasal != '' && $ireff != ''){
            ...
            if($cekdata->num_rows()>0){
                ...
                $this->mmaster->insertheader($id, $idocument, $ddocument, $ibagian, $ipartner, $ireff, $eremark);
                $no = 0;
                foreach ($id_product as $idproduct) {
                    $idproduct = $idproduct;
                    $nquantity = $n_quantity[$no];
                    $edesc     = $e_desc[$no];    
                    $this->mmaster->insertdetail($id, $idproduct,$nquantity, $edesc, $ireff);
                    $no++;
                }
    ...
    public function update()
    {
        ...
        if($idocument != ''  && $ddocument != '' && $ibagian != ''){
            $cekkode = $this->mmaster->cek_kodeedit($idocument, $kodeold, $ibagian);
            if($cekkode->num_rows()>0){
                ...
                $this->db->trans_begin();
                $this->Logger->write('Simpan Data ' . $this->global['title'] . ' Kode : ' . $idocument);
                $this->mmaster->updateheader($id, $idocument, $ddocument, $ibagian, $ipartner, $ireff, $eremark);
