...
class Cform extends CI_Controller {
    ...
    public function lokasigudang(){
        ...
        if($this->input->get('q') != '') {
            ...
            $this->db->join("tr_master_gudang b", "a.i_kode_lokasi=b.i_kode_lokasi");
            
            $this->db->like("UPPER(a.i_kode_lokasi)", $cari);
            $this->db->or_like("UPPER(a.e_nama_lokasi)", $cari);
            $this->db->where("b.i_kode_master in('G08','G13')");
            $data = $this->db->get();
            foreach($data->result() as  $isupplier){
                    $filter[] = array(
                    'id'   => $isupplier->i_kode_lokasi, 
                    'text' => $isupplier->e_nama_lokasi.'-'.$isupplier->i_kode_master.'-'.$isupplier->e_nama_master,
                );
            }          
    ...
    public function add(){
        ...
        $data =array(
                        ...
                        'data2' => $this->mmaster->getproduct($ilokasigudang)->result(),
