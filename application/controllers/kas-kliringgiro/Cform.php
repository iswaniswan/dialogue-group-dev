...
class Cform extends CI_Controller {
    ...
    public function penyetor(){
        $filter = [];
        $data   = $this->mmaster->penyetor(strtoupper($this->input->get('q')));
        if ($data->num_rows()>0) {
            foreach ($data->result() as $row) {
                $filter[] = array(
                    'id'    => $row->id,
    ...
    public function getgiro(){
        $filter = [];
        $data   = $this->mmaster->getgiro(strtoupper($this->input->get('q')), $this->input->get('ibank'));
        if ($data->num_rows()>0) {
            foreach ($data->result() as $row) {
                $filter[] = array(
                    'id'    => $row->id, 
    ...
    function getitemgiro(){
        ...
        $data = $this->mmaster->getitemgiro($ireferensigiro, $ibank);
        $dataa = array(
            'data'       => $data->result_array(),
            'dataitem'   => $this->mmaster->getitemgiro($ireferensigiro, $ibank)->result_array(),
        );
    ...
    public function simpan(){
        ...
        $ipenyetor    = $this->input->post('ipenyetor', TRUE);
        ...
        $this->mmaster->insertheader($id, $idocument, $ibagian, $datedocument, $ikasbank, $ipenyetor, $ibank, $eremark);

        for($i=1;$i<=$jml;$i++){ 
            if($this->input->post('cek'.$i)=='checked'){
                $idgiro         = $this->input->post('idgiro'.$i, TRUE);
                $penerima       = $this->input->post('penerima'.$i, TRUE);
                $pelanggan      = $this->input->post('pelanggan'.$i, TRUE);
                $jumlah         = str_replace(',','',$this->input->post('jumlah'.$i,TRUE));

                $this->mmaster->insertdetail($id, $idgiro, $penerima, $pelanggan, $jumlah);
            }
        }
    ...
    public function edit(){
        ...
        $data = array(
            ...
            'bagian'        => $this->mmaster->bagianpembuat()->result(),
            'number'        => "GRK-".date('ym')."-123456",   
            'giro'          => $this->mmaster->get_giro($id, $idcompany)->result(),
    ...
    public function update(){
        ...
        $this->db->trans_begin();
        $this->Logger->write('Simpan Data ' . $this->global['title'] . ' Kode : ' . $idocument);
        $this->mmaster->updateheader($id, $idocument, $ibagian, $datedocument, $ikasbank, $ipenyetor, $ibank, $eremark);
        $this->mmaster->deletedetail($id);
        // var_dump($id, $idocument, $ibagian, $datedocument, $ikasbank, $ipenyetor, $ibank, $eremark, $jml);
        // die();
        for($i=1;$i<=$jml;$i++){ 
