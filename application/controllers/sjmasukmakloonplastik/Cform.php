...
class Cform extends CI_Controller {
    ...
    public function simpan()
    {
        ...
        $idreff       = implode(",",$this->input->post('idreff', TRUE));
        $eremark      = $this->input->post('eremark', TRUE);
        $jml          = $this->input->post('jml', TRUE);
        if ($ibagian!='' && $idtype!='' && $idreff!=''  && $idpartner!='' && $jml > 0) {
            $this->db->trans_begin();
            $id = $this->mmaster->runningid();
            $this->mmaster->simpan($id,$idocument,$ddocument,$ibagian,$idtype,$idpartner,$idreff,$eremark);
            for($i=0;$i<$jml;$i++){
                ...
                if (($iddocument!='' || $iddocument!=null) && ($idmaterial!='' || $idmaterial!=null) && $nqty > 0 && ($idmateriallist!='' || $idmateriallist!=null) && $nqtylist > 0) {
                    $this->mmaster->simpandetail($id,$iddocument,$idmaterial,$nqty,$idmateriallist,$nqtylist,$eremark);
                }
            } 
    ...
    public function update()
    {
        ...
        if ($id!='' && $ibagian!='' && $idtype!='' && $idreff!=''  && $idpartner!='' && $jml > 0) {
            $this->db->trans_begin();
            $this->mmaster->update($id,$idocument,$ddocument,$ibagian,$idtype,$idpartner,$idreff,$eremark);
