...
class Cform extends CI_Controller {
    ...
    public function barang() {
        $filter = [];
        $data = $this->mmaster->barang($this->input->get('q'),$this->input->get('ibagian'), $this->input->get('ddocument') );
        if ($data->num_rows()>0) {
            foreach ($data->result() as $row) {
                $filter[] = array(
                    'id'   => $row->id.'|'.$row->e_color_name.'|'.$row->id_color.'|'.$row->e_company_name,
                    'text' => $row->e_company_name.' - '.$row->i_product_wip.' - '.$row->e_product_wipname.' ('.$row->e_color_name.')'
                );
            }
        }else{
            $filter[] = array(
    ...
    public function barangall() {
        $filter = [];
        $data = $this->mmaster->barangall($this->input->get('q'), $this->input->get('ibagian'));
        if ($data->num_rows()>0) {
            foreach ($data->result() as $row) {
                $filter[] = array(
                    'id'   => $row->id.'|'.$row->e_color_name.'|'.$row->id_color.'|'.$row->e_company_name,
    ...
    public function simpan(){
        ...
        for ($i = 1; $i <= $jml; $i++) {
            ...
            $qtyrepair = str_replace(",","",$this->input->post('nquantityrepair' . $i, TRUE));
            $idcolor   = $this->input->post('idcolor' . $i, TRUE);
            $eremark   = $this->input->post('eremark' . $i, TRUE);
            if ($idproduct[0]!=null || $idproduct[0]!='') {
                $this->mmaster->simpandetail($idcompany, $id, $idproduct[0], $idcolor, $qty, $eremark, $qtyrepair);
            }
        }
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)->setCellValue("A1", strtoupper($this->session->e_company_name))
            ->setCellValue("A2", "FORMAT UPLOAD STOCKOPNAME JAHIT");
        $spreadsheet->getActiveSheet()->setTitle('Format Stockopname');
        $h = 3;
        $header = ['#', 'PERUSAHAAN', 'ID BARANG', 'KODE BARANG', 'NAMA BARANG', 'WARNA', 'SO BAGUS', 'SO REPAIR','KETERANGAN'];
        ...
        $x = 4;
        $no = 0;
        $sql = $this->mmaster->get_export_so($ibagian, $ddocument);
        if ($sql->num_rows() > 0) {
            foreach ($sql->result() as $row) {
                $no++;
                $isi = [
                    $no, $row->e_company_name, $row->id, $row->i_product_base, $row->e_product_basename, $row->e_color_name, 0, 0,
                ];
    ...
    public function load()
    {
        ...
        $config = array(
            'upload_path'   => "./import/soproduksi/unitjahit/",
            'allowed_types' => "xls",
            'file_name'     => $filename,
            'overwrite'     => true
        );
        ...
        if ($this->upload->do_upload("userfile")) {
            ...
            $this->Logger->write('Upload File SO Unit Jahit : ' . $i_so);
    ...
    public function loadview()
    {
        ...
        $inputFileName = "./import/soproduksi/unitjahit/" . $filename;
        ...
        $aray = array();
        for ($n = 4; $n <= $hrow; $n++) {
            $id_product = strtoupper($spreadsheet->getActiveSheet()->getCell('C' . $n)->getValue());
            $qty_bagus  = (int)$spreadsheet->getActiveSheet()->getCell('G' . $n)->getValue();
            $qty_repair = (int)$spreadsheet->getActiveSheet()->getCell('H' . $n)->getValue();
            if (($qty_bagus > 0 || $qty_repair > 0) && $id_product != "") {
                ...
                $aray[] = array(
                    'e_company_name'    => strtoupper($spreadsheet->getActiveSheet()->getCell('B' . $n)->getValue()),
                    'id'                => $id_product,
                    'i_product_wip'     => strtoupper($spreadsheet->getActiveSheet()->getCell('D' . $n)->getValue()),
                    'e_product_wipname' => strtoupper($spreadsheet->getActiveSheet()->getCell('E' . $n)->getValue()),
                    'e_color_name'      => strtoupper($spreadsheet->getActiveSheet()->getCell('F' . $n)->getValue()),
                    'qty_bagus'         => $qty_bagus,
                    'qty_repair'        => $qty_repair,
                    'id_color'          => $id_color,
                    'e_remark'          => $spreadsheet->getActiveSheet()->getCell('I' . $n)->getValue(),
                );
            }
        }
