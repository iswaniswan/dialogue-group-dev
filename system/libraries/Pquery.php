...
class JQuery  {
	...
	function form_remote_tag($options)
	{
		...
		return '<form class="'.$options['class'].'" action="'.$options['url'].'" '.$id_string.' onsubmit=\''.$this->remote_function($options).'; return false;\' method="'.(isset($options['type'])?$options['type']:'GET').'"  >';			
	...
	function link_to_function($name,$function,$html_options=null)
	{
		...
		$id_string = 'id="'.$uid.'"';
		return '<a class="waves-effect" href="'.((isset($html_options['href']))?$html_options['href']:'#').'" onclick=\''.((isset($html_options['onclick']))?$html_options['onclick'].';':'').$function.'; return false;\'><span class="hide-menu">'.$name.'</span></a>';
...
class CI_PQuery extends JQuery{
}
